...
if ! [ -f $BitRockDir/output/$NewGDKPackage ] ; then
  cat /tmp/masterbuild_error | mail -s "6.0 GDK Build FAILED in `hostname` - $DATE" asoleymanzadeh@groundworkopensource.com
  exit -1
fi
...
ssh root@morat mv -f $MoratDir/builds/BitRock/6.0/groundworkgdk-6.0-*-linux-$bitrock_os-installer.bin /DailyBuilds/BitRock/$qasubdir
