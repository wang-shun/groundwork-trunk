...
if ! [ -f $BitRockDir/output/$NewPackage ] ; then
  cat /tmp/masterbuild_error | mail -s "6.0 CE Build FAILED in `hostname` - $DATE" asoleymanzadeh@groundworkopensource.com
  exit -1
fi

# Copy package into morant
ssh root@morat rm -f $MoratDir/builds/BitRock/6.0/groundwork-6.0-*-linux-$bitrock_os-installer.bin
ssh root@morat rm -f $MoratDir/builds/BitRock/last_good_build/groundwork-6.0-*-linux-$bitrock_os-installer.bin
