...
# Copyright (c) 2009-2012 GroundWork Open Source Solutions info@groundworkopensource.com
...
svn_co () {
	...
	if [ $SVN_EXIT_CODE -eq 0 ]; then
	    ...
	    echo "BUILD FAILED: There has been a problem trying to checkout groundwork files." | mail -s "6.7.0 Ent Build FAILED in  `hostname` - $DATE" build-info@gwos.com
	    exit 1
	fi
	sleep 30
    done
...
svn_co --username build --password bgwrk http://archive.groundworkopensource.com/groundwork-opensource/trunk/foundation/collage/database/schema/postgresql
...
$RUN_DIR/makesymlinks nagios nagios $GW_HOME/profiles SYMLINKS
...
#Copy gwcollagedb migration script for gdma plugins fix (GWMEE-10084)
...
cp -p $GW_HOME/foundation/container/webapps/nms-rstools*.war $BR_HOME/foundation/container/webapps

echo "Copy the Cloudhub Web application into the GroundWork deployment"
scp root@morat:/var/www/html/cloudhub/vmware/1.0/*.war $BR_HOME/foundation/container/webapps
...
# Noma configuration
echo "Copy Noma configuration files"
