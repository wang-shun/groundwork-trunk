...
#!/bin/bash -x
#Copyright (C) 2012  GroundWork Inc. info@groundworkopensource.com
#
...
# No need to build CE files Changed Nov 15 2010
date
...
if [ $GREP_ERROR -eq 0 ]; then
  ...
  grep -v "grep" $RUN_DIR/logs/build.log | grep -A 15 -B 45 "BUILD FAILED" | mail -s "6.7.0 Enterprise Build FAILED in `hostname` - $DATE" build-info@gwos.com
  exit -1
elif [ -f $BitRockDir/output/$NewPackage ] ; then
    $BitRockDir/output/$NewPackage --version | mail -s "6.7.0 Build OK in `hostname` - $DATE" build-info@gwos.com
else
    echo "Unknown error" | mail -s "6.7.0 Enterprise Build FAILED in `hostname` - $DATE" build-info@gwos.com
    exit -1
fi

# Copy package into morat
ssh root@morat rm -f $MoratDir/builds/BitRock/6.0/groundworkenterprise-6.7.0-*-linux-$bitrock_os-installer.run
ssh root@morat mv $MoratDir/builds/BitRock/last_good_build/groundworkenterprise-6.7.0-*-linux-$bitrock_os-installer.run /tmp
...
ssh root@morat $MoratDir/builds/BitRock/comparer-linux.bin /tmp/groundworkenterprise-6.7.0-*-linux-$bitrock_os-installer.run $MoratDir/builds/BitRock/last_good_build/groundworkenterprise-6.6.0-*-linux-$bitrock_os-installer.bin > $RUN_DIR/logs/comparer-EE-$bitrock_os.log
scp -p $RUN_DIR/logs/comparer-EE-$bitrock_os.log root@morat:$MoratDir/logs
ssh root@morat rm -f /tmp/groundworkenterprise-6.7.0-*-linux-$bitrock_os-installer.run
