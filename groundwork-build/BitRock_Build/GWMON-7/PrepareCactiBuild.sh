...
PRO_ARCHIVE=http://geneva/groundwork-professional/$PRO_ARCHIVE_BRANCH
          ...
          CACTI_SVN_BASE=$PRO_ARCHIVE/monitor-core/cacti/postgresql/cacti
   CACTI_PORTAL_SVN_BASE=$PRO_ARCHIVE/monitor-portal/applications/nms/installer/cacti
  CACTI_SCRIPTS_SVN_BASE=$PRO_ARCHIVE/monitor-portal/applications/nms/conf/cacti_updates
 CACTI_DATABASE_SVN_BASE=$PRO_ARCHIVE/monitor-portal/applications/nms/conf/database
  CACTI_PREDICT_SVN_BASE=$PRO_ARCHIVE/monitor-core/cacti/postgresql/predict
CACTI_AGGREGATE_SVN_BASE=$PRO_ARCHIVE/monitor-core/cacti/postgresql/aggregate
 CACTI_REALTIME_SVN_BASE=$PRO_ARCHIVE/monitor-core/cacti/postgresql/realtime
   CACTI_NECTAR_SVN_BASE=$PRO_ARCHIVE/monitor-core/cacti/postgresql/nectar
...
svn_export $SVN_CREDENTIALS $CACTI_SVN_BASE/lib-functions.php.php56.patch
svn_export $SVN_CREDENTIALS $CACTI_SVN_BASE/lib-html.php.patch
svn_export $SVN_CREDENTIALS $CACTI_SVN_BASE/lib-ping.php-ipv6.patch
svn_export $SVN_CREDENTIALS $CACTI_SVN_BASE/lib-ping.php.php56.patch
svn_export $SVN_CREDENTIALS $CACTI_SVN_BASE/lib-rrd.php-r7393.patch
svn_export $SVN_CREDENTIALS $CACTI_SVN_BASE/lib-snmp.php-ipv6.patch
...
svn_export $SVN_CREDENTIALS $CACTI_SVN_BASE/thold-0.4.2-includes-polling.php-query.patch
svn_export $SVN_CREDENTIALS $CACTI_SVN_BASE/thold-0.4.2-includes-settings.php.initial_diff
svn_export $SVN_CREDENTIALS $CACTI_SVN_BASE/thold-0.4.2-includes-settings.php.php56.patch
...
svn_export $SVN_CREDENTIALS $CACTI_SVN_BASE/thold-0.4.2-thold_functions.php.php56.patch
...
svn_export $SVN_CREDENTIALS $CACTI_AGGREGATE_SVN_BASE/plugin:aggregate-v0.75.tgz
svn_export $SVN_CREDENTIALS $CACTI_AGGREGATE_SVN_BASE/aggregate-0.75-patch-for-postgres9.tar.gz

svn_export $SVN_CREDENTIALS $CACTI_REALTIME_SVN_BASE/plugin:realtime-v0.5-2.tgz
svn_export $SVN_CREDENTIALS $CACTI_REALTIME_SVN_BASE/realtime-0.5-2-patch-for-postgres9.tar.gz

svn_export $SVN_CREDENTIALS $CACTI_NECTAR_SVN_BASE/plugin:nectar-v0.35a.tgz
svn_export $SVN_CREDENTIALS $CACTI_NECTAR_SVN_BASE/nectar-0.35a-patch-for-postgres9.tar.gz
svn_export $SVN_CREDENTIALS $CACTI_NECTAR_SVN_BASE/GROUNDWORK_NOTES
...
tar xzf $CACTI_BUILD_TREE/aggregate-0.75-patch-for-postgres9.tar.gz
tar xzf $CACTI_BUILD_TREE/realtime-0.5-2-patch-for-postgres9.tar.gz
tar xzf $CACTI_BUILD_TREE/nectar-0.35a-patch-for-postgres9.tar.gz
...
# The official release files for the Cacti "aggregate", "nectar", and "realtime"
# plugins include a colon in the filename, so we use an extra "tar" option to disable
# its standard interpretation of such a character in the middle of a tarball filename.
# (That might not actually be necessary given that we use a preceding pathname rather
# than just the naked filename, but it's better to play safe anyway.)
tar xzf $CACTI_BUILD_TREE/plugin:aggregate-v0.75.tgz --force-local
tar xzf $CACTI_BUILD_TREE/plugin:realtime-v0.5-2.tgz --force-local
tar xzf $CACTI_BUILD_TREE/plugin:nectar-v0.35a.tgz   --force-local
...
patch -b -V simple -z .pre_pg -p0 < $CACTI_BUILD_TREE/aggregate-0.75-patch-for-postgres9
patch -b -V simple -z .pre_pg -p0 < $CACTI_BUILD_TREE/realtime-0.5-2-patch-for-postgres9
patch -b -V simple -z .pre_pg -p0 < $CACTI_BUILD_TREE/nectar-0.35a-patch-for-postgres9

echo === Include extra GroundWork-supplied files into the build.
cp -p $CACTI_BUILD_TREE/GROUNDWORK_NOTES plugins/nectar/
...
patch -b -V simple -z .pre_some_other_fix     -p2 < $CACTI_BUILD_TREE/thold-0.4.2-includes-polling.php-query.patch
...
patch -b -V simple -z .pre_url -p2 < $CACTI_BUILD_TREE/lib-html.php.patch
...
echo === Apply patches for allowing Cacti to run under PHP 5.6 or later releases.
# These patches must be applied at the end, after GroundWork's PostgreSQL-porting fixes.
cd $CACTI_BUILD_BASE/cacti-0.8.7g
patch -b -V simple -z .pre_php56 -p2 < $CACTI_BUILD_TREE/lib-functions.php.php56.patch
patch -b -V simple -z .pre_php56 -p2 < $CACTI_BUILD_TREE/thold-0.4.2-includes-settings.php.php56.patch
patch -b -V simple -z .pre_php56 -p2 < $CACTI_BUILD_TREE/thold-0.4.2-thold_functions.php.php56.patch
patch -b -V simple -z .pre_php56 -p2 < $CACTI_BUILD_TREE/lib-ping.php.php56.patch

echo === Apply patches for fixing problems with IPv6 support.
# These patches must be applied at the end, after GroundWork's PostgreSQL-porting fixes.
cd $CACTI_BUILD_BASE/cacti-0.8.7g
patch -b -V simple -z .pre_ipv6 -p2 < $CACTI_BUILD_TREE/lib-ping.php-ipv6.patch
patch -b -V simple -z .pre_ipv6 -p2 < $CACTI_BUILD_TREE/lib-snmp.php-ipv6.patch
...
# Fix modes on certain plugin files and directories whose original source code
# inappropriately ships with permissions which are completely or partly open.
find \
    $CACTI_BUILD_BASE/htdocs/plugins/discovery	\
    $CACTI_BUILD_BASE/htdocs/plugins/settings	\
    $CACTI_BUILD_BASE/htdocs/plugins/thold	\
    $CACTI_BUILD_BASE/htdocs/plugins/aggregate	\
    $CACTI_BUILD_BASE/htdocs/plugins/nectar	\
    -type f -perm /go+w -print | xargs chmod 644
find \
    $CACTI_BUILD_BASE/htdocs/plugins/aggregate	\
    $CACTI_BUILD_BASE/htdocs/plugins/nectar	\
    -type d -perm /go+w -print | xargs chmod 755
...
echo === Modify Perl scripts to refer to GroundWork Perl.
cd $CACTI_BUILD_TREE/cacti/htdocs/scripts
for perl_script in *.pl; do
   perl -pe 's{#!/usr/bin/perl}{#!/usr/local/groundwork/perl/bin/perl}' -i $perl_script
done
