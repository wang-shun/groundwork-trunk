...
my $version  = "0.2";
my $PROGNAME = "assign_hostgroup.pl";
my $debug    = 0;
...
while ( my $line = <DATA> ) {
    ...
    ## host name \t hostgroup
    chomp $line;
    my ( $hostname, $hostgroup ) = split( /\t/, $line );
    $host_hashref->{$hostname}->{hostgroup} = $hostgroup;

    if ( $debug > 1 ) {
	print "debug:   file entry - $line\n";
	print "debug: Hostname $hostname Hostgroup $hostgroup\n";
    }
    if ( !$monarchapi->hostgroup_exists( $hostgroup ) ) {
	print "skipping $hostgroup does not exist.\n";
	next;
    }
    ...
    if ( !$monarchapi->assign_hostgroup( $hostname, $hostgroup ) ) {
	print "assign_hostgroup  Failed to add host $hostname to group $hostgroup\n";
    }
}
close(DATA);
