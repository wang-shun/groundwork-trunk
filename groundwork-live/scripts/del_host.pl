...
while ( my $line = <DATA> ) {
	...
	print "debug:   host entry - $hostname\n";
    }
}
close(DATA);
my $count;
...
foreach (@hostnames) {
    my $result = $monarchapi->delete_host($_);
    $count++;
    if ( !$result ) {
	print "delete_host exception for host $_\n";
    }
}

# Finish with summary statistics.
print "Completed processing of " . $count . " host records in " . sprintf( '%0.3f', ( Time::HiRes::time() - $start_time ) ) . " seconds.\n";
