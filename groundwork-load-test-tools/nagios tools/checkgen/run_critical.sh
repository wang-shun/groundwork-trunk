...
do
    while [ "$number" -le $2 ]
    do
        echo "Setting Critical for host $number"
        if test $number -le 9
        then
            exec `./gen_passive_service_nsca.pl  -c ./input_conf_blast_CRITICAL_changes.cfg -S random_counter_host_0$number -l ./log_host_0$number.log > /dev/null` &
        else
