...
public abstract class WorkloadMessage implements IWorkloadMessage
{
	...
	private static final String PSTMT_INSERT_MESSAGE_BATCH = 
		"INSERT INTO MessageBatches (WorkloadID, WorkloadBatchId, MessageName, Threshold, BatchStartTime, BatchEndTime, TimeRecorded) " +
		"VALUES(?, ?, ?, ?, ?, ?, ?)";	
	...
	public void run() 
	{
		try 
		{
			...
			while (isUpdateComplete() == false) {
				...
				if (log.isDebugEnabled())
					log.debug("Checking Message Completion - Message: " + _name + 
			...
			log.info("Successfully captured metrics for: " + toString());
	...
	throws SQLException
	{
		...
		pstmt.setTimestamp(7, timeRecorded);
