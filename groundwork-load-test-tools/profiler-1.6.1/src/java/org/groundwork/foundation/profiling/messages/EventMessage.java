...
public class EventMessage extends WorkloadMessage
{
	// SQL Statements
	...
	// Also note, we have removed consolidation (consolidation='NAGIOSEVENT') for these messages in order to verify update based on
	// text message of the event.  If consolidation was on then the text message does not change
	// 
	private static final String MSG_LOG_MESSAGE_NO_CONSOLIDATION =
		"<NAGIOS_LOG MonitorServerName=\"localhost\"" +
		" Host=\"%1$s\" Identification=\"%1$s\" Severity=\"OK\"" +
		" MonitorStatus=\"UP\" TextMessage=\"%2$s\"" +
		" ReportDate=\"%3$s\" LastInsertDate=\"\"" +
	...
	private static final String MSG_LOG_MESSAGE_WITH_CONSOLIDATION =
		"<NAGIOS_LOG consolidation='NAGIOSEVENT' MonitorServerName=\"localhost\"" +
		" Host=\"%1$s\" Identification=\"%1$s\" Severity=\"OK\"" +
		" MonitorStatus=\"UP\" TextMessage=\"Profiler Log Message - Consolidation\"" +
		" ReportDate=\"%2$s\" LastInsertDate=\"\"" +
		" SubComponent=\"UNDEFINED\" ErrorType=\"HOST ALERT\" />";	
	 ...
	 */
	private EventMessage (int workloadId,
									 int batchCount,
									 int numDevices, 
	 ...
	 */
	public EventMessage(Node messageNode, NamedNodeMap attributes) throws ProfilerException
	{     
        ...
        {
        	throw new ProfilerException("numDevices attribute is missing from EventMessage.");
        }
        ...
        {
        	throw new ProfilerException("EventMessage.ctor - Invalid numDevices setting." 
        								+ numDevices);
        }   		
	...
	{
		StringBuilder sb = new StringBuilder(MSG_LOG_MESSAGE_NO_CONSOLIDATION.length() * _numDevices);
		...
		for (int i = 0; i < _numDevices; i++)
		{
			...
			{
				logMsg = String.format(MSG_LOG_MESSAGE_WITH_CONSOLIDATION, deviceName, _lastInsertDate);
			}
			else 
			{
				logMsg = String.format(MSG_LOG_MESSAGE_NO_CONSOLIDATION, deviceName, textMessage, _lastInsertDate);
			}
	...
	public IWorkloadMessage getRunnableInstance(int workloadId, 
												...
												long deltaTime)
	{
		return new EventMessage(workloadId, 
										   batchCount, 
										   _numDevices,
										   _consolidation,
										   _name, 
