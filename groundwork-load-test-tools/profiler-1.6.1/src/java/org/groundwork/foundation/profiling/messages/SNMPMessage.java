...
public class SNMPMessage extends WorkloadMessage
{
	// SQL Statements
	...
	// Also note, we have removed consolidation (consolidation='SNMPTRAP') for these messages in order to verify update based on
	// text message of the event.  If consolidation was on then the text message does not change
	// 
	private static final String MSG_SNMP_MESSAGE_NO_CONSOLIDATION =
		"<SNMPTRAP MonitorServerName=\"localhost\"" +
		...
		" ipaddress=\"%1$s\" Event_OID_numeric=\"1\" Event_OID_symbolic=\"1\" Event_Name=\"SNMP EVENT\"" +
		" Category=\"SNMP CATEGORY\" Variable_Bindings=\"SNMP BINDINGS\" />";	

	// Since we are using consolidation we have to check if the last check time is updated to the one in the message we sent
	private static final String MSG_SNMP_MESSAGE_WITH_CONSOLIDATION =
		"<SNMPTRAP consolidation='SNMPTRAP' MonitorServerName=\"localhost\"" +
		" Host=\"%1$s\" Device=\"%1$s\" Severity=\"OK\"" +
		" MonitorStatus=\"UP\" TextMessage=\"Profiler SNMP Message - Consolidation\"" +
		" ReportDate=\"%2$s\" LastInsertDate=\"%2$s\"" +
		" SubComponent=\"UNDEFINED\" ErrorType=\"HOST ALERT\"" +
		" ipaddress=\"%1$s\" Event_OID_numeric=\"1\" Event_OID_symbolic=\"1\" Event_Name=\"SNMP EVENT\"" +
		" Category=\"SNMP CATEGORY\" Variable_Bindings=\"SNMP BINDINGS\" />";	
	
	private static final String DEVICE_NAME_FORMAT = "SNMP_DVC_%1$s_%2$d_%3$d";
	...
	// Log	
	protected static Log log = LogFactory.getLog(SNMPMessage.class);
	 ...
	 */
	private SNMPMessage (int workloadId,
									 int batchCount,
									 int numDevices, 
									 ...
									 long deltaTime)
	{
		super(workloadId, batchCount, name, threshold, messageSocketInfo, dbProfilerConnection, dbSourceConnection, deltaTime);
	 ...
	 */
	public SNMPMessage(Node messageNode, NamedNodeMap attributes) throws ProfilerException
	{     
        ...
        {
        	throw new ProfilerException("numDevices attribute is missing from SNMPMessage.");
        }
        ...
        {
        	throw new ProfilerException("SNMPMessage.ctor - Invalid numDevices setting." 
        								+ numDevices);
        }   		
	...
	{
		StringBuilder sb = new StringBuilder(MSG_SNMP_MESSAGE_NO_CONSOLIDATION.length() * _numDevices);
		...
		for (int i = 0; i < _numDevices; i++)
		{
			...
			{
				snmpMsg = String.format(MSG_SNMP_MESSAGE_WITH_CONSOLIDATION, deviceName, _lastInsertDate);
			}
			else 
			{
				snmpMsg = String.format(MSG_SNMP_MESSAGE_NO_CONSOLIDATION, deviceName, textMessage, _lastInsertDate);
			}
	...
	public IWorkloadMessage getRunnableInstance(int workloadId, 
												...
												long deltaTime)
	{
		return new SNMPMessage(workloadId, 
										   batchCount, 
										   _numDevices,
										   _consolidation,
										   _name, 
