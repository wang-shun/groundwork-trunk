...
public class ToggleServiceStatusMessage extends WorkloadMessage {
	...
	private static final String MSG_SERVICE_STATUS = "<SERVICE_STATUS MonitorServerName=\"localhost\" Host=\"%1$s\""
			+ " Identification=\"%1$s\" ServiceDescription=\"%2$s\""
	...
	private static final String HOST_DEVICE_FORMAT = "TSSM_DEV_%1$s_%2$d";
	...
	// Host and Device ID used to relate the services too. We use the
	// workload ID so the host and device will only be created once for each
	// workload
	...
	private ToggleServiceStatusMessage(int workloadId, int batchCount,
		...
		_hostName = String.format(HOST_DEVICE_FORMAT, _name, workloadId);
	...
	public String buildMessage() throws ProfilerException {
		...
		for (int i = 0; i < _numServices; i++) {
			...
			msg = String.format(MSG_SERVICE_STATUS, _hostName, serviceName,
