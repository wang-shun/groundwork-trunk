...
public class ProfileFoundation 
{
	...
	public ProfileFoundation (String[] args, Object tg)
	throws InvalidConfigFileException, ConfigFileParseException, ProfilerException
	{
		...
		WorkloadMgr.startWorkloads(configFile, sessionName, tg);
	}
	...
	{		
		try {
			new ProfileFoundation(args, null);
		}
		catch (Exception e)
		{
	...
	public static String startProfiler(String[] args, Object tg) 
	{		
		try {
			new ProfileFoundation(args, tg);
		}
		catch (Exception e)
		{
			log.error(e + " - " + ((e.getCause() == null) ? "" : e.getCause().toString()));
			e.printStackTrace();
			return "error: Foundation has a problem (did you start Foundation?)!";
		}
		return "ok";
	}
