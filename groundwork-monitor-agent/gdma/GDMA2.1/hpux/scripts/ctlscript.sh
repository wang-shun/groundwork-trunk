#!/bin/sh
#
# HP-UX version of GDMA start/stop script.
# Installed as:
...
user=gdma
PERL_PATH=/opt/groundwork/perl/bin/perl
POLLER_PATH=/opt/groundwork/gdma/bin/gdma_poll.pl
SPOOLER_PATH=/opt/groundwork/gdma/bin/gdma_spool_processor.pl
unset ENV
...
status_poller() {
    pids=`ps -e -x | grep -F .perl.bin | grep -F $POLLER_PATH | awk '{print $1}'`
    if [ -n "$pids" ]; then
	return 0
    else
	return 1
...
status_spooler() {
    pids=`ps -e -x | grep -F .perl.bin | grep -F $SPOOLER_PATH | awk '{print $1}'`
    if [ -n "$pids" ]; then
	return 0
    else
	return 1
