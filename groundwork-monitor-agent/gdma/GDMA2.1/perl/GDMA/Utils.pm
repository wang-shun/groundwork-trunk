...
our @ISA = ('Exporter');
our $VERSION = '0.8.0';
our @EXPORT = qw(&read_config &read_multiple_config &validate_config &spool_results
  &get_spool_filename &try_lock &get_lock &get_current_time_str
  &load_hires_timer $utilsdebug);

# FIX LATER:  Should this be declared "our"?
my $utilsdebug = 0;

my $force_CRL_check = 0;
my %basic_ssl_opts  = ();

# We use the SSL_check_crl flag here (see IO::Socket::SSL), to ensure that the capability of checking a
# Certificate Revocation List (CRL) is available to customers who desire to use it.  If we set $force_CRL_check
# above to a true value, that would (in conjunction with the GDMA::Utils::gdma_client_ssl_opts() function below)
# require everyone else who wants to use SSL to have at least put in place a long-expiring, effectively-empty
# CRL file on the client machine.  But since it can be difficult for some customers to generate an empty CRL,
# we're not demanding that such a file be present.  If a CRL file is present, we will force the verification to
# use it; if not, we don't complain, and the verification will proceed without such additional checking.
#
# We no longer use "SSL_version => 'TLSv1'," here within %standard_ssl_opts, since we wish to force
# the use of the upgraded TLSv1_2 protocol and the TLSv1 setting did not allow such connections.
# The point is, we would prefer to disable TLS1.0 and earlier, which are somewhat insecure, at least in
# some contexts.  And there seems to be no point in supporting SSLv2 or SSLv3, which also have known
# vulnerabilities, since we control the server and ought to be able to bump up the security level there.
# Previously, a critical issue was that some browsers such as Firefox didn't yet support TLS1.2, so we
# wouldn't be able to protect the entire GroundWork site at that level yet.  But that problem should be
# long solved now.  If it were not, we would need to see if we could protect just the URLs that a GDMA
# client would access, using various Apache facilities.  In any case, to make this all work, we must now
# be running a version of OpenSSL on the server, underneath Apache, that supports TLS1.2.
#
...
sub read_multiple_config {
    ...
    $$errstr = "Ok";
    if ( GDMA::Utils::read_config( $configfile, $config, $errstr, 0 ) ) {
	my $config_dir;
	my $DIR;
	my $CONFIG;
	my $file;
...
sub validate_config {
	...
	"Auto_Setup_Instructions_Dir" =>
	  { 'requirement' => 'optional', 'regex' => '\\S+', 'message' => "Auto_Setup_Instructions_Dir must be a non-empty string", },
	"Auto_Setup_Trigger_Dir" =>
	  { 'requirement' => 'optional', 'regex' => '\\S+', 'message' => "Auto_Setup_Trigger_Dir must be a non-empty string", },
	"Enable_Auto" =>
	  { 'requirement' => 'optional', 'regex' => '(ON|On|on|oN|off)', 'message' => "Enable_Auto must be ON, On, on, oN, or off", },
	"Enable_Auto_Setup" =>
	  { 'requirement' => 'optional', 'regex' => '(ON|On|on|oN|off)', 'message' => "Enable_Auto_Setup must be ON, On, on, oN, or off", },
	"Spooler_Status" => {
	    'requirement' => 'optional',
	...
	"Poller_Pull_Failure_Interval" =>
	  { 'requirement' => 'optional', 'regex' => '[0-9]+', 'message' => "Poller_Pull_Failure_Interval must be an unsigned integer", },
	...
	"Sleep_Interval" =>
	  { 'requirement' => 'optional', 'regex' => '[0-9]+([.][0-9]*)?', 'message' => "Sleep_Interval must be an unsigned positive float", },
...
sub spool_results {
	...
	## Log error and move on.
	$$errstr = "Could not open the spool file $spool_filename ($!).";
	$ret_val = 0;
    }
    umask $old_umask;
    return $ret_val;
...
################################################################################
# Routines which are common to many GDMA daemons and standalone programs.
################################################################################
...
#   get_headpath()
#
#   Returns the head path for the agent, based on platform information.
#   The software is supposed to be installed within the headpath file tree.
#
################################################################################
sub get_headpath {
    my $my_headpath = "NULL";
	...
	$my_headpath = "/usr/local/groundwork/gdma";
    }
    elsif ( ( $my_osname eq 'solaris' ) or ( $my_osname eq 'hpux' ) ) {
	$my_headpath = "/opt/groundwork/gdma";
    }
    elsif ( $my_osname eq 'MSWin32' ) {
	$my_headpath = "";
	if ( defined $ENV{GDMA_BASE_DIR} ) {
	    $my_headpath = $ENV{GDMA_BASE_DIR};
	}
	else {
	    die "ERROR:  The GDMA_BASE_DIR environment variable is not defined.\n        Perhaps you are running as the wrong user.\n";
	}
    }
...
sub initialize_ssl_opts {
    my $client_ssl_opts_ref    = shift;
    my $client_force_CRL_check = shift;
    %basic_ssl_opts  = %$client_ssl_opts_ref;
    $force_CRL_check = $client_force_CRL_check;
}
...
################################################################################
sub get_ca_path {
    my $head_path = get_headpath();
    my $my_osname = $^O;
    my $ca_path   = '';

    if ( $my_osname eq 'MSWin32' ) {
	$ca_path = "$head_path\\certs";
    }
    else {
	$ca_path = "$head_path/certs";
    }
...
################################################################################
#
#   gdma_client_ssl_opts()
#
################################################################################
sub gdma_client_ssl_opts {
    my $logger   = shift;
    my %ssl_opts = %basic_ssl_opts;
    $ssl_opts{SSL_check_crl} = 1;

    if ( not $force_CRL_check ) {
	## Check for the existence of at least one seemingly valid CRL file.
	## If we find we cannot check properly, or if we find one, we leave the
	## SSL_check_crl flag enabled.  If we can check but we find no such file,
	## we disable the SSL_check_crl flag.
	my $ca_path = get_ca_path();
	if ( !opendir( CAPATH, $ca_path ) ) {
	    $logger->debug("ERROR:  Cannot open CA directory \"$ca_path\" ($!).");
...
#   filesystem can change over time.
#
################################################################################
sub is_valid_ca_path {
    ...
    $$errormsg = '';
    if ( $my_osname eq 'MSWin32' ) {
	# FIX MAJOR:  verify the following conditions; if any of them fail, set $$errormsg
	# and return false:
	# (*) The directory must be owned by some kind of trusted administrative entity.
	# (*) The directory must be readable by the GDMA user, and perhaps not by any other
	#     users other than administrators.
	# (*) Permissions on the directory must disallow anyone but administrators
	#     and possibly (but preferably not) the GDMA user itself from writing into
	...
	if ( $uid != $> and $uid != 0 ) {
	    ## The "gdma" username is selectable at install time, so we cannot pretend it will
	    ## necessarily be that string now.  So we do our best to tell the user that the
	    ## directory must be owned by either the gdma user or by root.
	    ...
	    $$errormsg =
	      ...
	      . "    it must be owned by either the GDMA user you specified at install time,\n"
	      . "    and under which you should always run the poller, or by root.\n"
	      . "    You are currently running the poller as $running_as_username.";
	    return 0;
	}
	elsif ($mode & 022) {
	...
	if ( !opendir( CAPATH, $ca_path ) ) {
		...
		if ( -l $ca_filepath ) {
			...
			## The "gdma" username is selectable at install time, so we cannot pretend it will
			## necessarily be that string now.  So we do our best to tell the user that the
			## filepath must be owned by either the gdma user or by root.
			my $running_as_username = getpwuid $>;
			my $file_owner_username = getpwuid $uid;
			$running_as_username = defined($running_as_username) ? "\"$running_as_username\"" : "an unknown user (UID $>)";
			$file_owner_username = defined($file_owner_username) ? "\"$file_owner_username\"" : "an unknown user (UID $uid)";
