...
Function CheckCounter( strComputer, strClass, strWhere, strProperty )
    ...
    Set colOS = objWMIService.ExecQuery("Select " & strProperty & " From " & strClass & " WHERE Name LIKE '" & strWhere & "'")
    if (Err.Number <> 0) then
...
Function CheckCounterEx( strComputer, strClass, strWhere, strProperty, strUser, strPassword )
    ...
    Set colOS = objWMIService.ExecQuery("Select Name, " & strProperty & " From " & strClass & " WHERE Name LIKE '" & strWhere & "'")
    if (Err.Number <> 0) then
	f_Error("Query failed with error number " & Err.Number & ":  " & Err.Description)
    end if
    for Each objOS In colOS
...
Function f_help()
    Dim str
    str="This script checks a WMI Property.  If your Local Machine has the same Administrator account"&vbCrlF
    str=str&"and password as the Remote Machine, you don't have to specify the last two parameters."&vbCrlF
    str=str&vbCrlF
    ...
    str=str&vbCrlF
    str=str&"cscript //nologo check_counter2.vbs -h hostname -class WMI_class -where clause -prop Property"&vbCrlF
    ...
    str=str&"-class WMI_class    WMI Class, e.g., Win32_PerfFormattedData_PerfOS_Processor."&vbCrlF
    str=str&"-where clause       WHERE clause (pattern for the Name field), e.g., _Total."&vbCrlF
    str=str&"-prop Property      WMI Propery, e.g., PercentProcessorTime."&vbCrlF
    ...
    str=str&vbCrlF
    str=str&"cscript //nologo check_counter2.vbs -h . -class Win32_PerfFormattedData_PerfOS_Processor -where _Total"&vbCrlF
    str=str&"    -prop PercentProcessorTime -w 80 -c 90 [-user .\Administrator -pass password] "
    Wscript.Echo str
End Function
...
elseif Wscript.Arguments.Count=1 and (UCase(arg(0))="-H" Or UCase(arg(0))="--HELP") then
    f_help()
elseif _
	Wscript.Arguments.Count=12 And _
	UCase(arg(0))="-H"         And _
	UCase(arg(2))="-CLASS"     And _
	UCase(arg(4))="-WHERE"     And _
	UCase(arg(6))="-PROP"      And _
	UCase(arg(8))="-W"         And _
	UCase(arg(10))="-C" _
    then
	...
	WScript.Quit(intOK)
    elseif (CLng(result) < CLng(arg(11))) then
	prefix = "Warning - "
	Wscript.Echo prefix & sProperty & " = " & result & " | '" & sProperty & "'=" & result & ";" & CLng(arg(9)) & ";" & CLng(arg(11)) & ";;"
...
elseif _
	Wscript.Arguments.Count=16 And _
	UCase(arg(0))="-H"         And _
	UCase(arg(2))="-CLASS"     And _
	UCase(arg(4))="-WHERE"     And _
	UCase(arg(6))="-PROP"      And _
	UCase(arg(8))="-W"         And _
	UCase(arg(10))="-C"        And _
	UCase(arg(12))="-USER"     And _
	UCase(arg(14))="-PASS" _
    then
    ...
    f_Error("Bad usage; try ""cscript //nologo check_counter2.vbs --help""")
end if
