...
Function LogGrowths(strComputer , strDatabase)	
	...
	for Each objOS In colOS
		...
		if (result < CDbl(arg(5))) then
          ...
          str2 = str2 & "'" & objOS.Name & " Log Growth' = "&result&";" & arg(5) & ";" & arg(7) & ";;"
          key = intOK          
        else
...
Function LogGrowthsEx( strComputer , strDatabase , strUser , strPassword)
...
Function Do_CheckEx(sComputer , strDatabases , strUser , strPass)
	...
	do while (pos > 0)
		...
		Else
			...
			strDatabases = Mid(strDatabases , pos + 1 , len(strDatabases))				
		End If
		result = LogGrowthsEx(sComputer , database , strUser , strPass)						
		if result > tmp then
			tmp = result
		End If	
	Loop
...
Function f_help()
  ...
  str=str&"cscript nsclient_mssql_log_growth1.vbs -h hostname -d <Database> -w warning_level -c critical_level [-user username -pass password]"
  str=str&vbCrlF
  ...
  str=str&vbCrlF
  str=str&"Example: cscript nsclient_mssql_log_growth1.vbs -h Ser1 -d * -w 30 -c 70 [-user Ser1\Administrator -pass password]"
  wscript.echo str
end function
'******************************
'** Main program
