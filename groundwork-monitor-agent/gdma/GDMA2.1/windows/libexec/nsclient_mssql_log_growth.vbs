...
Dim returnError
returnError = 0
Function LogGrowths(strComputer , strDatabase)	
	...
	for Each objOS In colOS
		'LogGrowths = objOS.LogGrowths
		result = objOS.LogGrowths
		if (result < CDbl(arg(5))) then
          prefix = " OK - " & "'" & objOS.Name
          str1 = str1 & prefix & "' Log Growth : " & result & " | "
          str1 = str1 & "'Log Growth' = "&result&";" & arg(5) & ";" & arg(7) & ";;	"
          key = intOK          
        else
    ...
    LogGrowths = temp
    Else
       	LogGrowths = returnError
    End If	
End Function

' Get Memory Pages. Input Computer Name + User Name + Password
Function LogGrowthsEx( strComputer , strDatabase , strUser , strPassword, strDomain)
	...
	for Each objOS In colOS					
		if strComp ( objOS.Name , strComputer , 1) = 0 then
			LogGrowthsEx = LogGrowths("." , strDatabase)
			exit Function
		end if
	next
...
Function Do_Check(sComputer , StrDatabases)
	...
	do while (pos > 0)
		...
		End If
		result = LogGrowths(sComputer , database)
		if result > tmp then
			tmp = result
		End If	
	Loop
...
Function Do_CheckEx(sComputer , strDatabases , strUser , strPass, strDomain)
	...
	do while (pos > 0)
		...
		Else
			...
			strDatabases = Mid(strDatabases , pos + 1 , len(strDatabases))				
		End If
		result = LogGrowthsEx(sComputer , database , strUser , strPass, strDomain)						
		if result > tmp then
			tmp = result
		End If	
	Loop
...
Function f_help()
  Dim str
  str="Check Log Growths of MS SQL. If your Local Machine has the same Administrator account and password as the Remote Machine then you don't have to use the two last parameters."&vbCrlF&vbCrlF
  str=str&"cscript nsclient_mssql_log_growth.vbs -h hostname -d database -w warning_level -c critical_level [-user username -pass password [-domain domain]]"
  str=str&vbCrlF
  ...
  str=str&"-domain domaim              Netbios Domain Name of Remote Machine."&vbCrlF
  str=str&vbCrlF
  str=str&"Example: cscript nsclient_mssql_log_growth.vbs -h Ser1 -d * -w 30 -c 70 [-user Ser1\Administrator -pass password -domain ITSP]"
  wscript.echo str
end function
'******************************
'** Main program
...
if (Err.Number>0) Then
  ...
  ' Check for Case 1, local machine
    if (UCase(arg(0))="-H") And (UCase(arg(2))="-D") And (UCase(arg(4))="-W") And (UCase(arg(6))="-C") And (Wscript.Arguments.Count=8) then
		sComputer = arg(1)
		sDatabase = arg(3)
		'result = CDbl(LogGrowths(sComputer , sDatabase))				
