			...
			for j = 0 to sum - 1
				if(Ucase(OldArrDatabaseName(i)) = Ucase(NewArrDatabaseName(j)) And (len(NewArrDatabaseName(j)) > 0)) then ' Same Name
					...
					if (result < CDbl(arg(5))) then
			          ...
			          str1 = str1 & prefix & "' Transactions : "& result & "/s | "
				      str1 = str1 & "'Transactions' = "&result&";" & arg(5) & ";" & arg(7) & ";;	"
			          temp = intOK			          
			        else 
...
Function TransactionsEx( strComputer , strDatabases , strUser , strPassword, strDomain)
	Dim objWMIService , colOS , colOS1 , objOS , objOS1 , Sql , key , temp , pos , tmp , i , j , sum
	Dim NewArrDatabase(100) , NewArrDatabaseName(100) , OldArrDatabase(100) , OldArrDatabaseName(100)
	Dim database , return
	Sql = "Select * From Win32_PerfRawData_MSSQLSERVER_SQLServerDatabases"
...
Function f_help()
  ...
  str=str&"cscript nsclient_mssql_transactions.vbs -h hostname -d database -w warning_level -c critical_level [-user username -pass password [-domain domain]]"
  str=str&vbCrlF
  ...
  str=str&vbCrlF
  str=str&"Example: cscript nsclient_mssql_transactions.vbs -h Ser1 -d * -w 30 -c 70 [-user Ser1\Administrator -pass password -domain ITSP]"
  wscript.echo str
end function
'******************************
'** Main program
      ...
      if (UCase(arg(0))="-H") And (UCase(arg(2))="-D") And (UCase(arg(4))="-W") And (UCase(arg(6))="-C") And (UCase(arg(8))="-USER") And (UCase(arg(10))="-PASS") And (Wscript.Arguments.Count=12) then
        ...
        if (Err.Number <> 0) Then			
         ...
         if (UCase(arg(0))="-H") And (UCase(arg(2))="-D") And (UCase(arg(4))="-W") And (UCase(arg(6))="-C") And (UCase(arg(8))="-USER") And (UCase(arg(10))="-PASS") And (UCase(arg(12))="-DOMAIN") And (Wscript.Arguments.Count=14) then
	        sComputer = arg(1)        
    	    sDatabase = arg(3)
        	TransactionsEx sComputer , sDatabase , arg(9) , arg(11)	, arg(13)
        	if (Err.Number <> 0) Then			
				f_Error("")
        	else			
        	end if
