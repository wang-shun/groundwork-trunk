...
'-------------------------------------------------------------------------------------------------
	Function f_Help()
		...
		str=str&"cscript "&strCommandName&" -h hostname -class classname -prop property -inst instancename [-w warning_level -c critical_level] [-user username -pass password [-domain domain]]"&vbCrlF
		str=str&vbCrlF
		...
		str=str&vbCrlF
		str=str&"Example: cscript "&strCommandName&" -h Ser1 -class Win32_PerfRawData_PerfOS_Processor -prop InterruptsPerSec -inst ""*"" -w 30 -c 70 [-user Ser1\Administrator -pass password -domain ITSP] "&vbCrlF
		str=str&"Example: cscript "&strCommandName&" -h Ser1 -class Win32_PerfRawData_PerfOS_Processor -prop InterruptsPerSec -inst ""Name = '0'"" -w 30 -c 70 [-user Ser1\Administrator -pass password -domain ITSP] "
		wscript.echo str
...
'-------------------------------------------------------------------------------------------------
	Function f_GetData()
		...
		ReDim strPropNameArray(Int(colWMI.count))
		...
		For Each objWMI in colWMI
			...
			' Populate the Perfdata value in cases where the instance name is "*"
			if (strNameTemp = "") then
				strNameTemp = strPropNameTemp
			end if
			strNameArray(j) = strNameTemp
			strPropNameArray(j) = strPropNameTemp
		...
		for k = 0 to (Int(colWMI.count) - 1)
			...
			strPropName = strPropNameArray(k)
			if(strDSName = "") then
				strDSName = f_FormatDSName(strPropName)
			end if
			Dim intValue
...
'*************************************************************************************************
	...
	strCommandName="check_counter_counter.vbs"
	strDescription="Check WMI counters of type PERF_COUNTER_COUNTER (272696320)."
	strVersion="1.5"
	...
	if ((UCase(arg(0))="-H") Or (UCase(arg(0))="--HELP")) and (argcountcommand=1) then
		...
		if( ((argcountcommand Mod 2) = 0) and (3 < tempCount < 11)) then
			...
			if((strComputer = "") or (strClass = "") or (strProp = "") or (strInst = "")) then
				Wscript.Echo "Error! Arguments wrong, require verify -h -class -prop -inst parameters.  Try -h or --help."
				Wscript.Quit(intError)
			else
