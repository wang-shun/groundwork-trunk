' Copyright 2007-2009 GroundWork Open Source Inc.
'
...
'-------------------------------------------------------------------------------------------------
	Function f_Help()
		...
		str=str&"cscript "&strCommandName&" -h hostname -prop properties -inst DeviceID [-w warning_level -c critical_level] [-t thresoldpairs] [-user username -pass password [-domain domain]]"&vbCrlF
		str=str&vbCrlF
		str=str&"-h [--help]                 Help."&vbCrlF
		str=str&"-h hostname                 Host name."&vbCrlF
		str=str&"-inst instance              DeviceID Instance."&vbCrlF
		...
		str=str&"-t thresold pairs           Threshold pairs for each disk. Note: each threhsold pair separated by ':' characters, warning and critical values separated by ',' character"&vbCrlF
		...
		str=str&vbCrlF
		str=str&"Example: cscript "&strCommandName&" -h Ser1 -prop ""DiskReadBytesPerSec, DiskWriteBytesPerSec"" -inst ""C:""  -w 30 -c 70 [-user Ser1\Administrator -pass password -domain ITSP] " &vbCrlF
		str=str&"Example: cscript "&strCommandName&" -h Ser1 -prop ""DiskReadBytesPerSec, DiskReadsPerSec"" -inst ""C:, D:""  -t ""700,900:60,80"" [-user Ser1\Administrator -pass password -domain ITSP] " &vbCrlF
		wscript.echo str
...
'-------------------------------------------------------------------------------------------------
	Function f_Display()
		...
		if((warningValue <> "") Or (criticalValue <> "")) then
			...
			strDisplay2 =  "'" & strDSName & "-" & strPropName & "'" & "=" & strPerfValue & ";" & strWarnValue & ";" & strCritValue & ";; "
...
'*************************************************************************************************
		...
		strDescription="Checks logical disk IO parameters using WMI."
		strVersion="1.4"
