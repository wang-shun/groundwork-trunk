...
'-------------------------------------------------------------------------------------------------
	Function f_Help()
		...
		str=str&"cscript "&strCommandName&" -h hostname -inst memory [-w warning_level -c critical_level] [-t thresoldpairs] [-user username -pass password [-domain domain]]"&vbCrlF
		str=str&vbCrlF
		str=str&"-h [--help]                 Help."&vbCrlF
		str=str&"-h hostname                 Host name."&vbCrlF
		str=str&"-inst memory                Memory categories are RAM, PAGING, _TOTAL."&vbCrlF
		...
		str=str&"-t thresold pairs           Threshold pairs against each individual memory category. Note: each thresold pair is separated by a ':', warning and critical values separated by ',' character"&vbCrlF
		...
		str=str&vbCrlF
		str=str&"Example: cscript "&strCommandName&" -h Ser1 -inst ""*""  -w 30 -c 70 [-user Ser1\Administrator -pass password -domain ITSP] " &vbCrlF
		str=str&"Example: cscript "&strCommandName&" -h Ser1 -inst ""RAM, _Total""  -w 30 -c 70 [-user Ser1\Administrator -pass password -domain ITSP] " &vbCrlF
		str=str&"Example: cscript "&strCommandName&" -h Ser1 -inst ""RAM, _Total""  -t ""70,90:60,80"" [-user Ser1\Administrator -pass password -domain ITSP] " &vbCrlF
		wscript.echo str
...
'*************************************************************************************************
		...
		strDescription="Checks memory usage using WMI."
		strVersion="1.4"
	...
	if ((UCase(arg(0))="-H") Or (UCase(arg(0))="--HELP")) and (argcountcommand=1) then
		...
		if( ((argcountcommand Mod 2) = 0) and (1 < tempCount < 8)) then
			...
			if((strComputer = "") or (strInst = "")) then
				Wscript.Echo "Error! Arguments wrong, require verify -h -inst parameters.  Try -h or --help."
				Wscript.Quit(intError)
			else
