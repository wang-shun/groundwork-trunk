' Copyright 2007-2014 GroundWork Open Source Inc.
'
...
Dim intValue
Dim returnValue
Dim strSortInstances
Dim strThresholds
Dim strThresholdPair
Dim instanceArray()
Dim instanceArraySize
Dim thresholdArraySize
...
'-------------------------------------------------------------------------------------------------
	Function f_Help()
		...
		Dim str
		str=strCommandName&" version "&strVersion&vbCrlF&vbCrlF
		str=str&strDescription&"  If your Local Machine has the same Administrator account and password as the Remote Machine then you don't have to use the two (or three) last parameters."&vbCrlF&vbCrlF
		str=str&"cscript "&strCommandName&" -h hostname -prop properties -inst Names [-w warning_level -c critical_level] [-t thresholdpairs] [-user username -pass password [-domain domain]] [-s sort_instances]"&vbCrlF
		str=str&vbCrlF
		...
		str=str&"-w warning_level            Warning threshold as percentage."&vbCrlF
		str=str&"-c critical_level           Critical threshold as percentage."&vbCrlF
		str=str&"-t threshold pairs          Threshold pairs for each network interface. Note: each threshold pair separated by ':', warning and critical values separated by ',' character"&vbCrlF
		str=str&"-s sort_instances           sort_instances must be either 0 or 1.  If 1 (default), sort network interface names before comparison. This may help with recognizing shorter instance names."&vbCrlF
		...
		str=str&vbCrlF
		str=str&"Example: cscript "&strCommandName&" -h Ser1 -prop ""PacketsReceivedPerSec, PacketsSentPerSec"" -inst ""MS TCP Loopback interface""  -w 30 -c 70 [-user Ser1\Administrator -pass password -domain ITSP] " &vbCrlF
		str=str&"Example: cscript "&strCommandName&" -h Ser1 -inst -prop ""BytesReceivedPerSec, PacketsReceivedPerSec"" -inst ""LAN1, WAN1""  -t ""700,900:60,80"" [-user Ser1\Administrator -pass password -domain ITSP] " &vbCrlF
		wscript.echo str
...
'Descripton:        Get information at Local Host.
'Input:				instance.
'Output:			No.
'-------------------------------------------------------------------------------------------------
	Function f_GetInstance()
		...
		For Each objWMI in colWMI
			strName = objWMI.Name
			' Printing the interface names in the order we see them here may help with debugging ...
			' Wscript.Echo "Interface name is:  " & strName
...
'-------------------------------------------------------------------------------------------------
'Function Name:         f_FormatThresholdValues().
'Descripton:            Format threshold values to conform to their use in performance data.
'Input:                 ThresholdRange (see the Nagios Plugin Development Guidelines).
'Output:                String value of threshold range, normalized to the "en-us" locale. 
'-------------------------------------------------------------------------------------------------
        Function f_FormatThresholdValues(ThresholdRange)

                On Error Resume Next

                ' Implementing conversions await some future version.
                f_FormatThresholdValues = ThresholdRange
                Exit Function
...
'-------------------------------------------------------------------------------------------------
	Function f_Display()
		...
		if((warningValue <> "") Or (criticalValue <> "")) then
			...
			Dim strCritValue
			SetLocale("en-us")
			strPerfValue = FormatNumber(intValue,2,vbTrue,vbFalse,vbFalse)
			SetLocale(0)
			strWarnValue = f_FormatThresholdValues(warningValue)
			strCritValue = f_FormatThresholdValues(criticalValue)
			strDisplay1 = strName & " '" & strPropName & "'" & " = " & intValue & "; "
			strDisplay2 =  "'" & strDSName & "'" & "=" & strPerfValue & ";" & strWarnValue & ";" & strCritValue & ";; "
...
'-------------------------------------------------------------------------------------------------
	Function f_ExecQuery()
		...
		strQuery = "Select  BytesReceivedPerSec, PacketsReceivedPerSec, BytesSentPerSec, PacketsSentPerSec, TimeStamp_PerfTime, Frequency_PerfTime"  _
					& " from " & strClass
		if(strSortInstances > 0) then
			' The ORDER BY clause is only supported by the Extended WMI Query Language, which
			' as far as I can tell is only available in Microsoft Systems Management Server 2.0.
			' That probably means it won't be available in Windows 2008 or earlier, unless there
			' is some way to load SMS 2.0 on the machine.  So instead of using ORDER BY here,
			' we sort after the query, below.
		...
		' Since we cannot depend on having an ORDER BY clause available, we have to sort after the query is executed.
		if(strSortInstances > 0) then
			colWMITemp.Sort "Name"
			colWMI.Sort "Name"
		end if
...
'-------------------------------------------------------------------------------------------------
	...
	if(thresholdArraySize > 0 and (thresholdArraySize <> instanceArraySize * propArraySize)) then
		Wscript.Echo "Error! Arguments wrong, number of instances and thresholds do not match."
			Wscript.Quit(intError)
			Exit Function
	end if
...
'-------------------------------------------------------------------------------------------------
	...
	for i  = 0 to propArraySize -1
			...
			if ((StrComp(propTemp, "BytesReceivedPerSec") = -1) and (StrComp(propTemp, "PacketsReceivedPerSec") = -1) and _
				...
				Wscript.Echo "Error! Arguments wrong, " & propTemp & " is not supported.  Try -h or --help."
				Wscript.Quit(intError)
			end if
		Next
...
'-------------------------------------------------------------------------------------------------
	Function f_GetThresolds()
	    ...
	    temp = 1
	    temp1 = 0
	    thresholdArraySize = 0

	    position2 = InStr(temp, strThresholds, ":")
	    if(position2 = 0) then
			position2 = Len(strThresholds)+1
	    end if
	    ...
	    do while (position2 > position1)
	      thresholdPair = Mid(strThresholds, position1 + 1, (position2 - position1 -1))
	      temp1 = InStr(1, thresholdPair, ",")
	      if(temp1 > 0) then
		warningTemp = Mid(thresholdPair, 1, temp1 -1)
		criticalTemp = Mid(thresholdPair, temp1 +1, len(thresholdPair) - temp1)
		if(IsNumeric(warningTemp) or IsNumeric(criticalTemp)) then
			thresholdArraySize = thresholdArraySize + 1
			    ReDim Preserve warningArray(thresholdArraySize)
			    ReDim Preserve criticalArray(thresholdArraySize)
			    warningArray(thresholdArraySize -1) = Trim(warningTemp)
			    criticalArray(thresholdArraySize -1) = Trim(criticalTemp)
		else
...
'*************************************************************************************************
		...
		strDescription="Checks network interface parameters using WMI."
		strVersion="1.5"
	...
	if ((UCase(arg(0))="-H") Or (UCase(arg(0))="--HELP")) and (argcountcommand=1) then
		...
		if( ((argcountcommand Mod 2) = 0) and (2 < tempCount < 9)) then
			...
			End if
			strThresholds = f_GetOneArg("-t")

			strSortInstances = f_GetOneArg("-s")
			if(strSortInstances <> "") then
			...
			if((strComputer = "") or (strInst = "") or (strProp = "")) then
				Wscript.Echo "Error! Arguments wrong, require verify -h -prop -inst parameters.  Try -h or --help."
				Wscript.Quit(intError)
			else
				Select Case tempCount
					...
					Case 6:
						if ((strUser <> "") and (strPass <> "") and (strThresholds <> "")) then
							f_GetThresolds()
							f_RemotePerfValue()
						else
