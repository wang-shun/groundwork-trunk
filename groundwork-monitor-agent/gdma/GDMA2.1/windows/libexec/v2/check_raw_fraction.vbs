...
'-------------------------------------------------------------------------------------------------
	Function f_Help()
		...
		str=str&"cscript "&strCommandName&" -h hostname -class classname -prop property -inst instance [-w warning_level -c critical_level] [-user username -pass password [-domain domain]]"&vbCrlF
		str=str&vbCrlF
		...
		str=str&vbCrlF
		str=str&"Example: cscript "&strCommandName&" -h Ser1 -class Win32_PerfRawData_PerfDisk_LogicalDisk -prop PercentFreeSpace -inst ""*""  -w 30 -c 70 [-user Ser1\Administrator -pass password -domain ITSP] " &vbCrlF
		str=str&"Example: cscript "&strCommandName&" -h Ser1 -class Win32_PerfRawData_PerfDisk_LogicalDisk -prop PercentFreeSpace -inst ""name=C:""  -w 30 -c 70 [-user Ser1\Administrator -pass password -domain ITSP] "
		wscript.echo str
...
'-------------------------------------------------------------------------------------------------
	Function f_FormatData(colWMI)
		...
		For Each objWMI in colWMI
			...
			if(strDSName = "") then strDSName = strPropName
			if((warningValue <> "") and (criticalValue <> "")) then
				Dim strPerfValue
				Dim strWarnValue
				Dim strCritValue
...
'*************************************************************************************************
		...
		strCommandName="check_raw_fraction.vbs"
		strDescription="Check WMI counters of type PERF_RAW_FRACTION (537003008)."
		strVersion="1.4"
	...
	if ((UCase(arg(0))="-H") Or (UCase(arg(0))="--HELP")) and (argcountcommand=1) then
		...
		if( ((argcountcommand Mod 2) = 0) and (3 < tempCount < 10)) then
			...
			if((strComputer = "") or (strClass = "") or (strProp = "") or (strInst = "")) then
				Wscript.Echo "Error! Arguments wrong, require verify -h -class -prop -inst parameters.  Try using -h or --help."
				Wscript.Quit(intError)
			else
				Select Case tempCount
					...
					Case Else
						Wscript.Echo "Error! Arguments are wrong.  Try using -h or --help."
						Wscript.Quit(intError)
				End Select
