...
strPass = ""
strVersion = "1.4"
strCommandName = "get_counter_type.vbs"
...
if (i=3) then
  ...
  if (i=5) then
	...
	wscript.echo strCommandName&" version "&strVersion&vbCrlF
        wscript.echo "Usage: "&strCommandName&" <hostname> <class> [<property>] [<user> <password>]"
	wscript.echo ""
	wscript.echo "   Use this program to identify the WMI counter type for a particular"
	wscript.echo "   class-property.  If you don't know the name of the property then start"
	wscript.echo "   with just the class name, e.g. Win32_Process.  This will return a list"
	wscript.echo "   of available properties within the class.  When specifying the property,"
	wscript.echo "   the upper/lower case letters must match what is published or you will"
	wscript.echo "   receive an error that the property does not have a defined countertype."
	wscript.echo "   It is also possible to get this error if the counter does indeed not"
	wscript.echo "   have a defined countertype!"
        wscript.quit(3)
      end if
    end if
  end if
