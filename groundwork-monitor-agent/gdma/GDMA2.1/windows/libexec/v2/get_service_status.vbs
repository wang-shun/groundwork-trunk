...
'
' Modified by Mitul Ghodasara (eGain) on 8 Sep 2008
'		--> Changed the file name
'		--> Accordingly changed the HELP contents
'		--> Added data for Perf i.e. 100% when service ok, 0% when stopped or not installed
'		--> Added an extra argument -ignore which has values yes or no.  This parameter controls the behaviour of
'			 alert generation i.e. in case a service is not installed then based on this parameter alert will be generated
'		  	 if -ignore=yes alert will be generated for unavailalbe services and if -ignore=no alert will not be generated for
'			 unavailable services.  Alert mechanism for stopped services is not changed i.e. in any case if a service is
'			 available on a computer and stopped, alert will be generated.
'		--> Modified the names as passed for perf data i.e. although in the Status Information of a Windows service there
'		     might be special characters like $ or space or - which are acceptable but the same cannot work in RRD that is
'		     used to store data and generate perf graphs.  So these special characters ($ or space or -) will be replaced by
'		     double underscores (__).  Bascially this will be done only for perf data, no changes would be done for text shown
'		     shown in Status Information.
'		--> In addition to above data for Perf which is stored in RRD also has a limitation that a column name (which is
'		     called ds name in RRD) cannot be more than 19 characters.  So basically when data is stored in RRD the
'		     windows service name is used as the column name.  So further we have modified the script that whenever
'		     a windows service name is greater than 19 characters in length, only the first 19 characters will be displayed in
'		     the perf graph.  So while configuring need to make sure that whenever windows services are configured to
'		     be monitored in groups i.e. using a single groundwork service (monitor), their first 19 characters would be unique.
'		     or else rrd might get confused and if not rrd then definitely wrong data woudl be stored in the same which in turn
'		     would create wrong and confusing perf graphs.
'*************************************************************************************************
'                                        Public Variable
'*************************************************************************************************
Const intOK = 0
...
'-------------------------------------------------------------------------------------------------
	Function f_Help()
		...
		Dim str
		str="Check State of a Service. If your Local Machine has the same Administrator account and password as the Remote Machine then you don't have to use the two (or three) last parameters."&vbCrlF&vbCrlF
		str=str&"cscript get_service_status.vbs -h hostname -inst instancename [-user username -pass password [-domain domain]]"&vbCrlF
		str=str&vbCrlF
		...
		str=str&"Example: cscript get_service_status.vbs -h Ser1 -inst ""WINS,wmi"" [-user SER1\Administrator -pass password] " & vbCrlF
		wscript.echo str
			...
			for Each objWMI In colWMI
				if(Ucase(instance) = Ucase(objWMI.Name)) then
					if(objWMI.State = "Running") then
						Wscript.Echo "OK - " & objWMI.Name & ": " & objWMI.State & "; | '" & mid(replace(replace(replace(objWMI.Name,"$","__")," ","__"),"-","__"),1,19) & "'=100;;;; "
						Wscript.Quit(intOk)
						Exit Function
					else
						Wscript.Echo "Critical - " & objWMI.Name & ": " & objWMI.State & "; | '" & mid(replace(replace(replace(objWMI.Name,"$","__")," ","__"),"-","__"),1,19) & "'=0;;;; "
						Wscript.Quit(intCritical)
						Exit Function
					end if
				end if
			next
			Wscript.Echo "Unknown - " & instance & ": not installed; | '" & mid(replace(replace(replace(instance,"$","__")," ","__"),"-","__"),1,19) & "'=0;;;; "
			'Wscript.Quit(intUnknown)
			if UCase(strIgnore)=UCase("Yes") Then
				Wscript.Quit(intOk)
			else
				Wscript.Quit(intUnknown)
			end if
...
'-------------------------------------------------------------------------------------------------
	Function f_GetMultiInstance()
		...
		for Each objWMI In colWMI
			if(Ucase(instance) = Ucase(objWMI.Name)) then
				if(objWMI.State = "Running") then
					...
					strPerfTemp = "'" & mid(replace(replace(replace(objWMI.Name,"$","__")," ","__"),"-","__"),1,19) & "'=100;;;; "
					intReturnTemp = intOk
					Exit Function
				else
					strResultTemp = "Critical - " & objWMI.Name & ": " & objWMI.State
					strPerfTemp = "'" & mid(replace(replace(replace(objWMI.Name,"$","__")," ","__"),"-","__"),1,19) & "'=0;;;; "
		...
		strPerfTemp = "'" & mid(replace(replace(replace(instance,"$","__")," ","__"),"-","__"),1,19) & "'=0;;;; "
		if UCase(strIgnore)=UCase("Yes") Then
			intReturnTemp = intOk
		else
	...
	Function f_GetInformation()
		...
		if(instanceArraySize = 1) then
			...
			for i  = 0 to instanceArraySize -1
				...
				strPerf = strPerf & strPerfTemp
			next
			Wscript.Echo strResult2 & strResult1 & " | " & strPerf
