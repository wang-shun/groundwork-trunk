...
'-------------------------------------------------------------------------------------------------
	Function f_Help()
		...
		str=str&strDescription&"  If your Local Machine has the same Administrator account and password as the Remote Machine then you don't have to use the two (or three) last parameters."&vbCrlF&vbCrlF
  		str=str&"cscript "&strCommandName&" -h hostname [-user username -pass password [-domain domain]]"&vbCrlF
  		str=str&vbCrlF
  		...
  		str=str&vbCrlF
  		str=str&"Example: cscript "&strCommandName&" -h Ser1 [-user Ser1\Administrator -pass password -domain ITSP] " &vbCrlF
  		wscript.echo str
  	...
  	if ((UCase(arg(0))="-H") Or (UCase(arg(0))="--HELP")) and (argcountcommand=1) then
		f_help()
  		...
  		if( ((argcountcommand Mod 2) = 0) and (1 < tempCount < 5)) then
  			...
  			if((strComputer = "")) then
  				Wscript.Echo "Error! Arguments wrong, require verify -h parameter.  Try -h or --help."
  				Wscript.Quit(intError)
  			else
