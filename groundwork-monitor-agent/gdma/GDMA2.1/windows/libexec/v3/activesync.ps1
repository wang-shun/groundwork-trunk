...
Usage:
	...
	activesync.ps1 -warning <warning value> -critical <critical value> -waittime <wait time secs> -verbose -help
  
	This plugin checks the performance of the Active Sync services on MS Exchange. The metrics polled are:
	Ping commands pending, Sync commands Pending and Activesync Requests Queued. Thresholds are against 
	ActiveSync Requests Queued, and defaults are:
	Warning = $warning, Critical = $critical number of queued requests. 
...
#	Fork64IfWoW
	...
	$p = getCounter "MSExchange ActiveSync" "Ping Commands Pending"
	$s = getCounter "MSExchange ActiveSync" "Sync Commands Pending"
	$q = getCounter "MSExchange ActiveSync" "Requests Queued"
	
	$state = setState $t $warning $critical

	$status_msg = "There are $q ActiveSync requests queued, with $p ping commands and $s sync commands pending." 
	$perfdata_msg = "requests_queued=$q;$warning;$critical;; pings_pending=$p;;;; syncs_pending =$s;;;;"
