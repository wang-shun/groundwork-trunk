...
# finds the maximum value out of the parameter list
function getMaximum
{
	$lstate = 0
	
	foreach ($stateval in $args)
	{
		if ($stateval -gt $lstate)
		{
			$lstate = $stateval
		}
	}
...
Usage:
	...
	16controlpanel.ps1 -warning <warning value> -critical <critical value> -waittime <wait time secs> -verbose -help
  
	This plugin checks the performance and health of the MS Exchange Control Panel. This gives an overview 
	of MS Exchange CAS Server health. The metrics polled are:
	Outbound Proxy Request Average Response Time,	
	ASP.net Request Failures per second,
	Control Panel Requests Average Response time.
	Thresholds are applied to both the response time measures. State is changed if either is above threshold. 
	Defaults are warning=$warning, critical = $critical, in ms.
...
#	Fork64IfWoW
	...
	$p = getCounter "MSExchange Control Panel" "Outbound Proxy Requests - Average Response Time"
	$f = getCounter "MSExchange Control Panel" "ASP.Net Request Failures/sec"
	$r = getCounter "MSExchange Control Panel" "Requests - Average Response Time"
	
	$statep = setState $p $warning $critical
	$stater = setState $r $warning $critical
	# a very high critical value to trigger onyl warning (32-bit maxint)
	$statef = setState $f 0 2147483647 
	if ($verbose) { write-host "States: p - $statep, r - $stater, f - $statef" }	

	$state = getMaximum $stateb $stater $statef

	$status_msg = "The Exchange Control Panel is taking $p milliseconds to complete proxy requests, $r milliseconds to complete normal requests, and has $f failures per second." 
	$perfdata_msg = "proxy_req_time=$p;$warning;$critical;; req_time=$r;$warning;$critical;; failures_per_sec=$f;;;;"
