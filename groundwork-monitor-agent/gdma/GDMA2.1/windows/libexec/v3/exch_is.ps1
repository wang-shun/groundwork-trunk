...
Usage:
	...
	exch_is.ps1 -warning <warning value> -critical <critical value> -waittime <wait time secs> -verbose -help
  
	This plugin checks the ratio of memory used by the IS process to the IS database Cache Size.
...
{
	$r0 = Get-Content $tmpfile -totalcount 1
} else {
	$r0 = 0
}
	
$b = getCounter "Process" "Private Bytes" "store"
$c = getCounter "MSExchange Database" "Database Cache Size (MB)" "Information Store"
	
if ($c)
{
	# convert b from bytes to mbytes
	$r1 = ($b / 1048576) / $c
} else {
	$r1 = 0
}
...
# write r1 as r0 back to disk
$r1 | out-File $tmpfile

if ($verbose) { write-host "r0: $r0, r1: $r1" }
if ($r0)
{
	$d = "{0:n2}" -f (100 * ($r1 - $r0) / $r0)
	# treat negative delta as no issue
	if ($d -lt 0) { $d = 0 }
	...
	{ 
		"OK" 
			{ $status_msg = "There is no significant change in the Information Store memory to database cache ratio."  }
		"WARNING" 
			{ $status_msg = "There has been a significant ($d%) change in the Information Store memory to database cache ratio. This can indicate a nascent memory leak." }
		"CRITICAL" 
			{ $status_msg = "There has been a large change in the Information Store memory to database cache ratio. This can indicate a memory leak." }	
	}
} else {
	$d = 0
	$state = "UNKNOWN"
...
$perfdata_msg = "pct_change=$d%;$warning;$critical;0;100 is_bytes_used=$b;;;; is_db_cache=$c;;;;"
