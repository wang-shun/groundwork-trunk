...
Usage:
	...
	exch_rpc.ps1 -warning <warning value> -critical <critical value> -waittime <wait time secs> -verbose -help
  
	This plugin checks RPC Requests and RPC averaged Latency against independent thresholds.
...
$r = getCounter "MSExchangeIS" "RPC Requests"
$l = getCounter "MSExchangeIS" "RPC Averaged Latency"
		
($wrx, $wlx) = $warning.replace("'","").split(' ,')
($crx, $clx) = $critical.replace("'","").split(' ,')
if ($verbose) { write-host "Thresholds - warning: $wrx $wlx, critical: $crx $clx" }

$rstate = setState $r $wrx $crx
$lstate = setState $wlx $clx

if ($rstate -eq "CRITICAL" -or $lstate -eq "CRITICAL")
{
	$state = "CRITICAL"
} elseif ($rstate -eq "WARNING" -or $lstate -eq "WARNING") {
	$state = "WARNING"
} else {
	$state = "OK"
}
		
$status_msg = "The Information store is currently handling $r requests. Latency for all client types is running at $l ms."
$perfdata_msg = "requests=$r;$wr;$cr;; latency=$l;$wl;$cl;;"
