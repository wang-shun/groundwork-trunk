...
Usage:
	...
	exch_stalls -warning <warning value> -critical <critical value> -waittime <wait time secs> -verbose -help
  
	This plugin checks the Database Page Fault Stalls/sec and Log Record Stall/sec to which thresholds apply.
...
#	Fork64IfWoW
	...
	$p = getCounter "MSExchange Database" "Database Page Fault Stalls/sec" "Information Store"
	$l = getCounter "MSExchange Database" "Log Record Stalls/sec" "Information Store"
	
	# Check thresholds for both
	$state = setState $l $warning $critical
	
	# and the special condition
	if ($state -ne "OK" -and $p -ne 0)
	{
		$state = "WARNING"
	}
	...
	if ($p -eq 0)
	{
		$status_msg = "There are no Database page faults, and $l Log Record stalls per second."
	} elseif ($p -gt 0) {
		$status_msg = "There are $p Database page faults per second. This is an indication of overload. There are $l Log Record Stalls per second."
	}

	$perfdata_msg = "db_page_faults_sec=$p;;;; db_log_stalls_sec=$l;$warning;$critical;;"
