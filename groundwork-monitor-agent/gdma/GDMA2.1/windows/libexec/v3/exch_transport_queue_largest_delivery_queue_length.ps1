...
param
(
    [int]$warning   = 200, # neutralize warnings, same value as critical
    [int]$critical  = 200,
	[switch]$verbose,
    [switch]$help
)
...
$m = getCounter "MSExchangeTransport Queues" "Largest Delivery Queue Length" "_total"
	
# Check thresholds for user count
$state = setState $m $warning $critical

$status_msg = "Largest delivery queue length is $m"

$perfdata_msg = "Unreach_Que=$m;$warning;$critical;;"
