...
param
(
    [int]$warning   = 250, # neutralize warnings, same value as critical
    [int]$critical  = 250,
	[switch]$verbose,
    [switch]$help
)
...
$m = getCounter "MSExchangeTransport Queues" "Active Mailbox Delivery Queue Length" "_total"
	
# Check thresholds for user count
$state = setState $m $warning $critical
	
$status_msg = "Active mailbox delivery queue length is $m"

$perfdata_msg = "Active_Mail_Que=$m;$warning;$critical;;"
