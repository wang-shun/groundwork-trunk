...
param
(
    [int]$warning   = 1, # neutralize warnings, same value as critical
    [int]$critical  = 1,
	[switch]$verbose,
    [switch]$help
)
...
$m = getCounter "MSExchangeTransport Queues" "Poison Queue Length" "_total"
	
# Check thresholds for user count
$state = setState $m $warning $critical

$status_msg = "Poison queue length is $m"

$perfdata_msg = "Poison_Que=$m;$warning;$critical;;"
