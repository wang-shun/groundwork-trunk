...
param
(
    [int]$warning   = 100, # neutralize warnings, same value as critical
    [int]$critical  = 100,
	[switch]$verbose,
    [switch]$help
)
...
$m = getCounter "MSExchangeTransport Queues" "Retry Remote Delivery Queue Length" "_total"
	
# Check thresholds for user count
$state = setState $m $warning $critical

if ($state -eq "CRITICAL") {
    $status_msg = "Check next hop to look for queuing issue."
} else {
    $status_msg = "Retry remote delivery queue length is $m"
}

$perfdata_msg = "Retry_Rem_Que=$m;$warning;$critical;;"
