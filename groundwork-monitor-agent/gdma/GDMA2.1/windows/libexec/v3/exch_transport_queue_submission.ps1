...
$m = getCounter "MSExchangeTransport Queues" "Submission Queue Length" "_total"
	
# Check thresholds for user count
$state = setState $m $warning $critical

if ($state -eq "CRITICAL") {
    $status_msg = "Look for bottlenecks in AD and Mailbox Servers"
} else {
	$status_msg = "Submission queue length is $m"
}

$perfdata_msg = "Submission_Que=$m;$warning;$critical;;"
