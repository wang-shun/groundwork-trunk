...
Usage:
	...
	exchange_avail.ps1 -warning <warning value> -critical <critical value> -waittime <wait time secs> -verbose -help
  	
	This plugin checks the perforrmance of the Exchange 2010 availability service. Values returned are 
	Time to process and requests per second. Threshold is against Time to Process. 
	Default is warning = $warning seconds, critical = $critical seconds. 
...
#	Fork64IfWoW
	...
	$t = getCounter "MSExchange Availability Service" "Average Time to Process a Free Busy Request"
	$r = getCounter "MSExchange Availability Service" "Availability Requests (sec)"
	
	$state = setState $t $warning $critical

	$status_msg = "The Calendar free-busy average processing time is $t seconds. There are $r requests per second." 
	$perfdata_msg = "time_to_process=$t;$warning;$critical;; requests_per_sec=$r;;;;"
