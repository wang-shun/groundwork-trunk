...
Usage:
	...
	mem_page_file.ps1 -warning <warning value> -critical <critical value> -waittime <wait time secs> -verbose -help
  
	This plugin checks the useage percentage of the paging file. If the paging file is heavily used, it 
	indicates a performance problem that can be fixed with more physical memory or different allocation
	of services between systems. 
	The default thresholds are warning = $warning, critical = $critical in percent of paging file used. 

	Warning value: if the returned numeric value is above this, the state will be WARNING.
	
	Critical Value: if the returned value is above this, the state will be CRITICAL.
...
#	Fork64IfWoW
	...
	$p = getCounter "Memory" "% Committed Bytes In Use" 

	$state = setState $p $warning $critical
	
	# Formatting - two decimals
	$p = "{0:n2}" -f $p
	
	$status_msg = "The paging file is $p percent used."
	$perfdata_msg = "pct_paging_used=$p%;$warning;$critical;;"
