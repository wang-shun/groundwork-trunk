...
Usage:
	...
	rpc_abook.ps1 -warning <warning value> -critical <critical value> -waittime <wait time secs> -verbose -help
  
	This plugin checks the performance of the RPC Client interface to the address book. 
	The metrics polled are:
	Browse Request Latency, Request Latency, and Referral Requst Latency. The thresholds are
	against all metrics, and the worst case is reported (if any are above threshold, state is changed).
...
#	Fork64IfWoW
	...
	$b = getCounter "MSExchangeAB" "NSPI RPC Browse Requests Average Latency"
	$r = getCounter "MSExchangeAB" "NSPI RPC Requests Average Latency"
	$z = getCounter "MSExchangeAB" "Referral RPC Requests Average Latency"
	
	$stateb = setState $b $warning $critical
	$stater = setState $r $warning $critical	
	$statez = setState $z $warning $critical
	if ($verbose) { write-host "States: b - $stateb, r - $stater, z - $statez" }	

	$state = getMaximum $stateb $stater $statez

	$status_msg = "The address book NSPI interface has $r ms latency for requests, $b ms for browse requests, and $z ms for Referral Requests." 
	$perfdata_msg = "request_latency=$r;$warning;$critical;; browse_latency=$b;$warning;$critical; ref_latency=$z;$warning;$critical;;"
