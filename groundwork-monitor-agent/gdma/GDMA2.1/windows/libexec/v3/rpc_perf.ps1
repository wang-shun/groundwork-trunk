...
param
(
	# Two thresholds - rpc requests and average latency
    [string]$warning   = "40,200",
    [string]$critical  = "50,250",
    [int]$waittime  = 2,
	[switch]$verbose,
    [switch]$help
)
...
Usage:
	...
	rpc_perf.ps1 -warning <warning value> -critical <critical value> -waittime <wait time secs> --instance -verbose -help
  	
	This plugin checks the performance of the RPC Client interface. The metrics polled are:
	RPC Average Latency, RPC Requests, and RPC Client Backoffs per second. The thresholds are
	against both Requests and Latency (defaults are requests,latency warning levels = $warning, 
	critical levels = $critical)
	
	Note: specify custom thresholds in pairs of requests,latency in number(or requests) and 
	ms (of latency).
 
	Warning value: if the returned numeric value is above this, the state will be WARNING 
	(default $warning).
	...
	instance: a specific network interface if desired. Default will be "*" for all interfaces.
...
#	Fork64IfWoW
	...
	$r = getCounter "MSExchange RpcClientAccess" "RPC Requests"
	$l = getCounter "MSExchange RpcClientAccess" "RPC Averaged Latency"
	$b = getCounter "MSExchangeIS" "RPC Client Backoff/sec"	
	
	($wr, $wl) = $warning.replace("'","").split(' ,')
	($cr, $cl) = $critical.replace("'","").split(' ,')
	
	if ($verbose) { write-host "Thresholds - warning: $wr $wl, critical: $cr $cl" }

	$state = setState $r $wr $cr
	$state = setState $l $wl $cl	
	
	$status_msg = "There are $r active requests being processed, and the average latency is $l. There are $b Client Backoffs per second."
	$perfdata_msg = "rpc_requests=$r;$wr;$cr;; ave_latency=$l;$wl;$cl;; backoffs=$b;;;;"
