	...
	if (colOS.count = 0) then
		...
		for Each objOS In colOS
			...
			if (result < CDbl(arg(5))) then
	          ...
	          str1 = str1 & "'LogGrowth'="&result&";" & arg(5) & ";" & arg(7) & ";;    "
	          key = intOK          
	        else
...
Function f_help()
  ...
  str=str&"cscript mssql_log_growth.vbs -h hostname -d database -w warning_level -c critical_level [-user username -pass password [-domain domain]]"
  str=str&vbCrlF
  ...
  str=str&vbCrlF
  str=str&"Example: cscript mssql_log_growth.vbs -h Ser1 -d * -w 30 -c 70 [-user Ser1\Administrator -pass password -domain ITSP]"
  wscript.echo str
end function
'******************************
'** Main program
