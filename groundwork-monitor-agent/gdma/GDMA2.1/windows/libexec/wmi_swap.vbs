...
Function GetPagesPerSec(strComputer)		
	...
	if (colOS.count = 0) then
	    f_Error("Error! Can't get PerfOS_Memory instance!")
	end if	
...
Function GetPagesPerSecEx( strComputer,strUser,strPassword)
	...
	if (colOS.count = 0) then
	    f_Error("Error! Can't get computer name instance!")
	end if
	for Each objOS In colOS					
...
if (Err.Number>0) Then
	f_Error("Problem reading arguments")
else 
  ...
  ' Check for Case 1, local machine
    if (UCase(arg(0))="-H") And (UCase(arg(2))="-W") And (UCase(arg(4))="-C") And (Wscript.Arguments.Count=6) then
      ...
      if (Err.Number > 0) Then		
        f_Error("Couldn't determine value for Pages per sec")
      else 
