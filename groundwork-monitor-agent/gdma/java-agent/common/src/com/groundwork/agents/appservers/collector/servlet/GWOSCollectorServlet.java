...
public abstract class GWOSCollectorServlet extends HttpServlet {
	private static org.apache.log4j.Logger log = Logger
			.getLogger(GWOSCollectorServlet.class);
	...
	private String performPost(String userName, String password,
		...
		try {
			...
			URI uri = new URI(
					...
					"//"
							+ gwServerName
							+ "/foundation-webapp/restwebservices/uploadProfiles/upload",
					null, null);
			URL url = uri.toURL();
		...
		if (param != null) {
			...
			if (action != null && !action.equalsIgnoreCase("")
				...
				if (startDiscButton != null) {
					...
					boolean testConnSuccess = gwosService
							.testConnection(createPropFileFromParams(request));
					if (testConnSuccess)
						request
			...
			if (action != null && !action.equalsIgnoreCase("")
				...
				request.getSession().setAttribute("selectedComponents",
						services);
				log.info("# of selected components is : " + services.size());
				rd = getServletContext().getRequestDispatcher(
			...
			if (action != null && !action.equalsIgnoreCase("")
				...
				if (selectedCompObj != null && selectedAliases != null) {
					...
					if (code == 0) {
						...
						Object propObj = request.getSession().getAttribute("statProps");
						this.createPropertyFile(comps, aliases, warnings,
								criticals, appServerName, properties, propFile,(Properties)propObj);
						rd = getServletContext().getRequestDispatcher(
								"/confirm.jsp");
					} else
						rd = getServletContext().getRequestDispatcher(
	...
	private void createPropertyFile(List<String> comps, List<String> aliases,
			List<String> warnings, List<String> criticals,
			String appServerName, String properties, String propFile, Properties statProps) {
