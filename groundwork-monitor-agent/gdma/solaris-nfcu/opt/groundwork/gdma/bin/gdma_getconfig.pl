...
$scpstmt = "scp -B -q -p -i $id_dsa_file gdma\@$gwserver:$remote_dir/$configfile $gw_gdma_home/$configfile.new";
print "Transferring configuration [$configfile] from Groundwork Server [$gwserver]\n";
print "Command: $scpstmt"."\n" if $debug;
my $res = qx/$scpstmt 2>&1/;
if (!$res) {
    # The transfer itself went okay; now let's try to atomically move the complete new file into place.
    rename ("$gw_gdma_home/$configfile.new", "$gw_gdma_home/$configfile") or $res = "Could not rename the incoming config file: $!";
}
