...
#
# updated 12/02/08 : returns critical if regex matched , and -warning option to return warning
#                    and added message option so can more sensibly report on -p (a|b|c)
...
sub print_help ();
    ...
    if ($pattern_count) {
        if ( defined $message ) { print "$message\n"; } else { print "($pattern_count): $pattern_line\n";}
        #exit $ERRORS{'WARNING'};
        if ( defined $warning ) { exit $ERRORS{'WARNING'}; } else { exit $ERRORS{'CRITICAL'}; }
    } else {
...
sub print_help () {
    ...
    print "-w, --warning\n";
    print "    Exit with warning status if regex matched\n";
    print "-m, --message=<message>\n";
    print "    Exit with status message of <message> if regex matched\n";
