...
' Get Send Public Queue in Local Machine. Input: Computer Name
Function QueuePublicSend(strComputer)		
	...
	for Each objOS In colOS		
		QueuePublicSend = objOS.SendQueueSize
		exit function
	next
End Function

' Get Send Public Queue in Remote Machine. Input Computer Name + User Name + Password
Function QueuePublicSendEx( strComputer,strUser,strPassword, strDomain)
	Dim sql
	...
	for Each objOS In colOS					
		if strComp ( objOS.Name , strComputer , 1) = 0 then			
			QueuePublicSendEx = QueuePublicSend( ".")			
			exit Function
		end if
	next
	...
	for Each objOS In colOS				
		QueuePublicSendEx = objOS.SendQueueSize
		exit function
	next
End Function
...
Function f_help()
  Dim str
  str="Check Messages in Public Send Queue of MS Exchange. If your Local Machine has the same Administrator account and password as the Remote Machine then you don't have to use the two last parameters."&vbCrlF&vbCrlF
  str=str&"cscript exchange_msg_queue_public_sendq.vbs -h hostname -w warning_level -c critical_level [-user username -pass password [-domain domain]]"
  str=str&vbCrlF
  ...
  str=str&vbCrlF
  str=str&"Example: cscript exchange_msg_queue_public_sendq.vbs -h Ser1 -w 30 -c 70 [-user Ser1\Administrator -pass password -domain ITSP] "
  wscript.echo str
end function
...
if (Err.Number>0) Then
  ...
  ' Check for Case 1, local machine
    if (UCase(arg(0))="-H") And (UCase(arg(2))="-W") And (UCase(arg(4))="-C") And (Wscript.Arguments.Count=6) then
		sComputer = arg(1)		
		Queue = QueuePublicSend(sComputer)
      if (Err.Number <> 0) Then		
        f_Error("")
      else 
        if (Queue < CLng(arg(3))) then
          prefix = "OK - "
          Wscript.Echo prefix & " Send Queue Size = "& Queue & " | 'Queue Size' =" &Queue & ";" & CLng(arg(3)) & ";" & CLng(arg(5)) & ";;"
          WScript.Quit(intOK)
        else 
      ...
      if (UCase(arg(0))="-H") And (UCase(arg(2))="-W") And (UCase(arg(4))="-C") And (UCase(arg(6))="-USER") And (UCase(arg(8))="-PASS") And (Wscript.Arguments.Count=10) then
        sComputer = arg(1)
        Queue = QueuePublicSendEx( sComputer , arg(7) , arg(9),"")
        if (Err.Number<>0) Then			
			f_Error("")
        else
			if (Queue < CLng(arg(3))) then
        ...
        if (UCase(arg(0))="-H") And (UCase(arg(2))="-W") And (UCase(arg(4))="-C") And (UCase(arg(6))="-USER") And (UCase(arg(8))="-PASS") And (UCase(arg(10))="-DOMAIN") And (Wscript.Arguments.Count=12) then
       	 sComputer = arg(1)
        	Queue = QueuePublicSendEx( sComputer , arg(7) , arg(9),arg(11))
        	if (Err.Number<>0) Then			
				f_Error("")
	        else
				if (Queue < CLng(arg(3))) then
