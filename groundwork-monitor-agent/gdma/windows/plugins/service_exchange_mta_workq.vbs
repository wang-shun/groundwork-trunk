...
Function MTAWorkQueueEx( strComputer,strUser,strPassword, strDomain)
	Dim sql
...
Function f_help()
  ...
  str=str&"cscript service_exchange_mta_workq.vbs -h hostname -w warning_level -c critical_level [-user username -pass password [-domain domain]]"
  str=str&vbCrlF
  ...
  str=str&vbCrlF
  str=str&"Example: cscript service_exchange_mta_workq.vbs -h Ser1 -w 30 -c 70 [-user Ser1\Administrator -pass password -domain ITSP] "
  wscript.echo str
end function
    ...
    if (UCase(arg(0))="-H") And (UCase(arg(2))="-W") And (UCase(arg(4))="-C") And (Wscript.Arguments.Count=6) then
      ...
      if (Err.Number <> 0) Then		
        ...
        if (result < CLng(arg(3))) then
          prefix = "OK - "
          Wscript.Echo prefix & " MTA Work Queue Length = "& result & " | 'Queue Length' =" & result & ";" & CLng(arg(3)) & ";" & CLng(arg(5)) & ";;"
          WScript.Quit(intOK)
        else 
      ...
      if (UCase(arg(0))="-H") And (UCase(arg(2))="-W") And (UCase(arg(4))="-C") And (UCase(arg(6))="-USER") And (UCase(arg(8))="-PASS") And (Wscript.Arguments.Count=10) then
        ...
        sComputer = arg(1)        
        result =  MTAWorkQueueEx( sComputer , arg(7) , arg(9), arg(11))
        if (Err.Number<>0) Then			
			f_Error("")
        else
			if (result < CLng(arg(3))) then
