...
# Original version : DSN 9/2008
# DSN : 12/2008 Updated to fork dispatcher process using win32 process handling
# DSN : 1/2009 Updated to poll every 60 seconds - this is for per-service frequency mod
# DSN : 4/2009 Updated to be built using PDK8, same options as above , frequency default changed,  logging improved/reduced, dispatcher timeout increased
...
my $frequency     = 200 ; # default dispatcher invocation frequency in seconds
my $dispatcher_timeout =  190 ; # dispatcher process tree and itself will be all killed after this much time, in seconds
...
__USAGE__
    ...
    Log("\n$Config{DisplayName} service version $version starting at : ".localtime);
    Log("GDMA service log file is $logfile (only used in service mode)") if $debug;
    Log("GDMA dispatcher invocation frequency : $frequency seconds");
    Log("Dispatcher program : $dispatcher");
    Log("Dispatcher config file : $config");
    Log("Dispatcher process tree will be terminated after : $dispatcher_timeout seconds");
    ...
    while (ContinueRun($frequency))
    {
       ...
       Log(localtime()." : Running '$cmd $cmdargs' at frequency $frequency, dispatcher timeout of $dispatcher_timeout") if $debug;
       ...
       if (Win32::Process::Create($ProcessObj, "$cmd", "$exename $cmdargs", 0, NORMAL_PRIORITY_CLASS, ".")) # service working dir is $ENV{SystemRoot} eg c:\windows 
       {
	    ...
	    my $TimeStarted = time;
	    Log(">>> [ " . localtime() . "] Dispatcher process started, process ID $ProcessID") if $debug;
       } 
       else 
       {
	...
	my $dispatcherpid = Win32::Process::Info->new();
        ...
        if ( @info ) 
        {
            ...
            if ( $procname eq $exename )
            {
                ...
                Log("Killing dispatcher process $ProcessID - it ran too long ($duration seconds)" );
...
sub reset_gdma_pull_counter_file
{
   ...
   Log("Resetting counter file $counterfile");
   ...
   Log("Counter file $counterfile was reset");
