...
public class VemaBaseStorage extends GWOSEntity
{
    ...
    // constructors
    // -------------------------------------------------------------------
    protected     VemaBaseStorage()            { /* nothing to do! */      }
    protected     VemaBaseStorage(String name) { storageName = name;       }
	...
	public void setStorageName( String name  ) { storageName   = name;    } 
    ...
    public void mergeInNew(VemaBaseStorage update)
    {
        if (update == null) 
            return;

        if (this.storageName == null || update.storageName != null) 
            this.storageName           = update.storageName;
    ...
    public String formatSelf( VemaBaseStorage o )
    {
    ...
    public String getXML(String action)
    {
        ...
        addAttribute("Host",        this.storageName);
        addAttribute("Description", this.storageName);
        addAttribute("DisplayName", this.storageName);

        // addAttribute("LastStateChange", this.lastStateChange);

        // Add attribute for device
        if(this.macAddress != null) // and...
            if(     this.macAddress.length() > 0) addAttribute("Device", this.macAddress);
            else if(this.ipAddress.length()  > 0) addAttribute("Device", this.ipAddress);
            else                                  addAttribute("Device", this.storageName);
        else                                      addAttribute("Device", this.storageName);
