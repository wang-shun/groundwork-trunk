...
# various Clouds modules, partly because we don't yet know exactly
...
#     my $logger = Clouds::Logger->new ($logfile);
...
sub system_log {
    openlog('Clouds', 'nofatal,pid', 'user');
    syslog('crit', join('',@_));
    closelog();
}
...
sub log_message {
    if (!defined $logfile) {
	printstack(1,1);
	die "ERROR:  Clouds::Logger log file is not defined\n";
    }
    if (not log_is_open()) {
	printstack(1,1);
	die "ERROR:  Clouds::Logger log file is not open\n";
    }
    print LOG @_, "\n";
