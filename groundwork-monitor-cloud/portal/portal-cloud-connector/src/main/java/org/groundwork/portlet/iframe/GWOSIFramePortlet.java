...
public class GWOSIFramePortlet extends IFramePortlet
{
	...
	private static Log logger = LogFactory.getLog(GWOSIFramePortlet.class);
	  ...
	  /* By default remote url is not allowed due the javascript limitation for the jQuery to calculate the height*/
    ...
    public void doView(RenderRequest request, RenderResponse response) {
        try {
        	/*Jboss portal doesn't make it easy to get access to a HTTP session */
        	...
        	/* Validate URL to make sure that we are the correct URL when the IFrame was configured for localhost*/
        	this.urlToRender = (this.configuredURL.startsWith("/") ? this.configuredURL : validateURL(this.configuredURL, serverNameInURL));
    ...
    protected void setRenderAttributes(RenderRequest request) {
        ...
        /*Check if the IFRame URL needs to have the GroundWork UID attached to the URL */
    ...
    private String validateURL(String userURL, String serverName) throws PortletException {
        ...
        try {
            ...
            if ((URLHost.compareToIgnoreCase("localhost") == 0)
            	...
            	{
	                // get the local hostname just in case the URL is incomplete or
	                // points to localhost
	...
	public static Properties loadPropertiesFromFilePath(String filePath) {
		...
		try {
			...
			logger.info("Unable to find properties file [" + filePath + "]. Using default");
		} finally {
			try {
				if (defaultFS != null) {
					defaultFS.close();
				...
				logger
						.warn("Unable to close the input stream for properties file ["
								+ filePath
								+ "]. Exception is - "
								+ ioe.getMessage());
			}
