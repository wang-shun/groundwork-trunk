...
if [ "`fgrep -c monarch_clouds.cgi $HTTPD_CONF`" -eq 0 ]; then
    ...
    echo "         to allow access to Cloud page ..."
...
# Actually, we do want to test whether this is the last uninstall or an upgrade.
# The reason being, that if this is an upgrade, we shouldn't bother to recreate
# the jbossportal database now (while the old package is being uninstalled), as
# that will happen later on when the new package is installed.  There's no point
# in running this procedure twice.
#
...
# the point at which we want to reconstruct the jbossportal database.  In contrast,
# 'remove' is specified when Ubuntu dpkg wants to delete the rest of the package,
# and that's when we should be be reconstructing the jbossportal database.
# 
...
# %doc %attr(0444,nagios,nagios) %{cloudpath}/doc/%{name}-%{version}/groundwork-cloud-connector.0.1.pdf
