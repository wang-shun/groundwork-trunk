...
# Copyright 2010-2011 GroundWork Open Source, Inc. ("GroundWork").  All rights
...
if [ "`fgrep -c /cloud-connector/ $HTTPD_CONF`" -eq 0 ]; then
    ...
    # FIX MINOR:  Some of this editing is rather a hack.  Clean it up for the next GWMEE
    # release, when we move all the CSS and image files out of the monarch.war file into a
    # shared layout-related file tree, to be shared between Monarch, the Cloud Connector,
    # and any other applications that need access to such stuff.
	...
	-e 'print "ProxyPass /cloud-connector/  http://localhost:8080/cloud-connector/\nProxyPass /monarch/css/download/ http://localhost:8080/monarch/css/download/\nProxyPass /monarch/css/      !\n" if m{ProxyPass\s+/monarch/\s+};' \
	-e 'print "Alias /monarch/css \"/usr/local/groundwork/core/monarch/htdocs/monarch\"\n" if m{^\s*Alias\s+/monarch\s+};' \
	$HTTPD_CONF
    echo "NOTICE:  Restarting Apache to pick up configuration change ..."
    /usr/local/groundwork/ctlscript.sh restart apache
fi

# Later on, we don't bother to remove the /cloud-connector line from the
...
# restarted.  It will be stopped and perhaps started again inside the
# fix_DeactivationTime_type script invoked below.
JOSSO_FILE=/usr/local/groundwork/config/resources/josso-agent-config.xml
if [ -f $JOSSO_FILE ]; then
    if [ "`fgrep -c /cloud-connector $JOSSO_FILE`" -eq 0 ]; then
	echo "NOTICE:  Saving josso-agent-config.xml as josso-agent-config.xml.pre_clouds"
	echo "         and editing, to allow access to Cloud Connector screens ..."
	/usr/local/groundwork/perl/bin/perl -p -i.pre_clouds \
	    -e 'print "\t\t\t\t\t<agent:partner-app id=\"cloud-connector\" context=\"/cloud-connector\"/>\n" if m{</agent:partner-apps>};' \
	    $JOSSO_FILE
    fi
fi
...
%attr(0644,nagios,nagios) %{gwpath}/core/monarch/lib/MonarchClouds.pm
...
%changelog
* Tue Mar  1 2011 Glenn Herteg <support@groundworkopensource.com> 2.1.0
- changed the names of portal objects to better clarify their purpose

* Wed Feb 16 2011 Glenn Herteg <support@groundworkopensource.com> 2.1.0
- extend editing of httpd.conf to support access to Monarch CSS files
  even before the Monarch portal is accessed during a given session

* Tue Feb 15 2011 Glenn Herteg <support@groundworkopensource.com> 2.1.0
- encapsulation porting to work under the JOSSO framework
- edit josso-agent-config.xml so the Cloud Connector works under JOSSO
- fix permissions of MonarchClouds.pm
