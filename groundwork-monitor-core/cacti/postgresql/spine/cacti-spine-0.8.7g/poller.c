...
void poll_host(int host_id, int host_thread, int last_host_thread, int host_data_ids, char *host_time) {
	...
	char query4[BUFSIZE];
	char query5[BUFSIZE];
	char update6[BUFSIZE];
	char update7[BUFSIZE];
	char query8[BUFSIZE];
	char query9[BUFSIZE];
	...
	if (host_id) {
		...
		if ((result = db_query(&mysql, query2)) != 0) {
			...
			if (row) {
				...
				SPINE_LOG(("Host[%i] TH[%i] ERROR: Database Returned a Null Host Result", host->id, host_thread));
				num_rows = 0;
				host->ignore_host = TRUE;
			}
		}else{
					...
					if (perform_assert) {
						...
						if (!reindex_err) {
							...
							if ((IS_UNDEFINED(poll_result)) || (STRIMATCH(poll_result, "No Such Instance"))) {
								...
								if (host_thread == 1) {
#if USE_POSTGRESQL
									// FIX MAJOR:  we need to retry the transaction in the event of a serialization failure
									char delete[BUFSIZE];
									snprintf(delete, BUFSIZE, "DELETE FROM poller_command where poller_id = 0 and action = %i and command = '%i:%i'", POLLER_COMMAND_REINDEX, host->id, reindex->data_query_id);
									snprintf(query3, BUFSIZE, "INSERT INTO poller_command (poller_id, time, action,command) values (0, NOW(), %i, '%i:%i')", POLLER_COMMAND_REINDEX, host->id, reindex->data_query_id);
									if (db_insert(&mysql, "start transaction isolation level serializable") &&
										db_insert(&mysql, delete) &&
										db_insert(&mysql, query3)) {
										db_insert(&mysql, "commit");
									}else{
										db_insert(&mysql, "rollback");
									}
#else
	...
	if (host_thread == last_host_thread) {
		db_insert(&mysql, update6);
		db_insert(&mysql, update7);
	}
