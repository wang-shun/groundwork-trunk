	...
	while (1) {
		if (mysql_query(mysql, query)) {
			...
			if ((error == 1213) || (error == 1205)) {
				...
				die("FATAL: Database Error:'%i', Message:'%s'", error, mysql_error(mysql));
			}
		}else{
...
void db_connect(const char *database, MYSQL *mysql) {
	...
	mysql_init(mysql);
	if (mysql == NULL) {
		die("FATAL: Database unable to allocate memory and therefore can not connect");
	}

	options_error = mysql_options(mysql, MYSQL_OPT_CONNECT_TIMEOUT, (char *)&timeout);
	if (options_error < 0) {
		die("FATAL: Database options unable to set timeout value");
	}
	...
	if (options_error < 0) {
		die("FATAL: Database options unable to set reconnect option\n");
	}
	#endif
