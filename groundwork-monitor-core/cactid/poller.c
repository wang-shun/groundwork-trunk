						...
						if (num_oids > 0) {
							...
							for (j = 0; j < num_oids; j++) {
								if (host->ignore_host) {
									...
									snprintf(temp_result, BUFSIZE-1, "%s", strip_all_but_digits(snmp_oids[j].result));
									/*snprintf(snmp_oids[j].result, sizeof(snmp_oids[j].result)-1, "%s", strip_alpha(temp_result));*/
									snprintf(snmp_oids[j].result, sizeof(snmp_oids[j].result)-1, "%s", translate_updown(temp_result));
