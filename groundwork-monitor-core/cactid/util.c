...
char *translate_updown(char *string)
{

	CACTID_LOG_DEBUG(("DEBUG: translate_updown INPUT %s\n", string));
	
	if (!strncmp(string, "up", 2))
		strcpy(string, "1");
	else if (!strncmp(string, "down", 4))
		strcpy(string, "2");

	CACTID_LOG_DEBUG(("DEBUG: translate_updown OUTPUT %s\n", string));

	return(string);
}

/*
**	strip everything except for
**	digits.
*/

char *strip_all_but_digits(char *string)
{
	char temp[512], *t, *s;
