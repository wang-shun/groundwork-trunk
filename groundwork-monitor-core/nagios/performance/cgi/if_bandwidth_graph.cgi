...
my $action = "/graphs/cgi-bin/if_bandwidth_graph.cgi";
...
sub body() {
	...
	$line .= "\"DEF:ifspeed=$rrd:ifspeed:AVERAGE\", ";
	$line .= "\"CDEF:intmp=in,8,*,100,*\",";
	$line .= "\"CDEF:inutl=intmp,ifspeed,/\",";
	$line .= "\"AREA:inutl#00FF00:in\",";
	$line .= "\"GPRINT:inutl:MIN:(min=%.0lf\", ";
	$line .= "\"GPRINT:inutl:AVERAGE:ave=%.0lf\", ";
	$line .= "\"GPRINT:inutl:MAX:max=%.0lf)\", ";

	$def .= "\"DEF:out=$rrd:out:AVERAGE\", ";
	$line .= "\"CDEF:outtmp=out,8,*,100,*\",";
	$line .= "\"CDEF:oututl=outtmp,ifspeed,/\",";
	$line .= "\"LINE2:oututl#FF00FF:out\", ";
	...
	for ($i = 0; $i < 5; $i++) {
		if ($i == 0) {
			# 2 hours
			$title = "$host $service Bandwidth Utilization Last 2 Hours";
			$start = $end - 7200;
			...
			# 48 hours
			$title = "$host $service Bandwidth Utilization Last 48 Hours";
			$start = $end - 172800;
			...
			# 14 days
			$title = "$host $service Bandwidth Utilization Last 14 Days";
			$start = $end - 1209600;
			...
			# 60 days
			$title = "$host $service Bandwidth Utilization Last 60 Days";
			$start = $end - 5184000;
			...
			# 360 days
			$title = "$host $service Bandwidth Utilization Last 360 Days";
			$start = $end - 31104000;
		...
		my $evalstring = '($averages,$xsize,$ysize) = RRDs::graph("$graphfile", "-s", $start, "-e", $end, "--vertical-label", $vlabel, "-X", "0", "-t", $title, "-w", 750, "-h", 200, "-u","100","--rigid", '.$defstring.");";
		eval($evalstring);
		my $err = RRDs::error;
		if ($err) {
			$errstr .= "$err<br>";
...
<tr>
<td class=head align=center>$host $service Bandwidth Utilization</td>
</tr>
<tr>
<td align=center>
