...
my $action = '/graphs/cgi-bin/if_graph2.cgi';
...
sub body() {
	...
	$line .= "\"AREA:in#00FF00:in\",";
	$line .= "\"GPRINT:in:MIN:(min=%.0lf\", ";
	$line .= "\"GPRINT:in:AVERAGE:ave=%.0lf\", ";
	$line .= "\"GPRINT:in:MAX:max=%.0lf)\", ";

	$def .= "\"DEF:indiscards=$rrd:indis:AVERAGE\", ";
	$def .= "\"DEF:outdiscards=$rrd:outdis:AVERAGE\", ";
	$def .= "\"CDEF:discards=indiscards,outdiscards,+\", ";
	$line .= "\"STACK:discards#FFA500:discards\", ";
	$line .= "\"GPRINT:discards:MIN:(min=%.0lf\", ";
	$line .= "\"GPRINT:discards:AVERAGE:ave=%.0lf\", ";
	$line .= "\"GPRINT:discards:MAX:max=%.0lf)\", ";

	$def .= "\"DEF:inerrors=$rrd:inerr:AVERAGE\", ";
	$def .= "\"DEF:outerrors=$rrd:outerr:AVERAGE\", ";
	$def .= "\"CDEF:errors=inerrors,outerrors,+\", ";
	$line .= "\"STACK:errors#FF0000:errors\", ";
	$line .= "\"GPRINT:errors:MIN:(min=%.0lf\", ";
	$line .= "\"GPRINT:errors:AVERAGE:ave=%.0lf\", ";
	$line .= "\"GPRINT:errors:MAX:max=%.0lf)\", ";

	$def .= "\"DEF:out=$rrd:out:AVERAGE\", ";
	$line .= "\"LINE2:out#FF00FF:out\", ";
	$line .= "\"GPRINT:out:MIN:(min=%.0lf\", ";
	$line .= "\"GPRINT:out:AVERAGE:ave=%.0lf\", ";
	$line .= "\"GPRINT:out:MAX:max=%.0lf)\", ";
	...
	for ($i = 0; $i < 5; $i++) {
		if ($i == 0) {
			...
			$start = $end - 31104000;
			$graph4 = "$host\_if_4.png";
			$graph4 =~ s/\s/_/g;
			$graphfile = "$graphdir/$graph4";
		}

		my($averages,$xsize,$ysize);
		my $evalstring = '($averages,$xsize,$ysize) = RRDs::graph("$graphfile", "-s", $start, "-e", $end, "--vertical-label", $vlabel, "-t", $title, "-w", 750, "-h", 200, "--rigid", '.$defstring.");";
		eval($evalstring);
		my $err = RRDs::error;
		if ($err) {
			$errstr .= "$err<br>";
