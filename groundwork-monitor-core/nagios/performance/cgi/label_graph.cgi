...
use lib "/usr/local/groundwork/lib/perl5/site_perl/5.8.7";
...
#
my $action = "/graphs/cgi-bin/label_graph.cgi";
...
sub body() {
	...
	my %dsnames = ();
	my @colors = ('#0000FF','#00FF00','#FF0000','#C05599','#E092E3','#6F76C4','#9BAEFF','#818181','#C0C0C0','#8BA016','#D3DB00','#64A2B8','#8DD9E0','#7E87B7','#F3B50F','#EB6232');
#	my @colors = ('#C05599','#E092E3','#6F76C4','#9BAEFF','#818181','#C0C0C0','#8BA016','#D3DB00','#64A2B8','#8DD9E0','#7E87B7','#F3B50F','#EB6232');
	...
	my $i = 0;
	foreach my $key (sort keys %dsnames) {
		$dsnames{$key} = $colors[$i % ($#colors + 1)];
		$i++;
	}
	#
	# Generate the graphs
	#
	foreach $key (keys %dsnames) {
	 	$def .= "\"DEF:$key=$rrd:$key:AVERAGE\", ";
		$line .= "\"LINE2:$key$dsnames{$key}:$key\", ";
	...
	for ($i = 0; $i < 5; $i++) {
		if ($i == 0) {
            # 2 hours
            $title = "$host $service Count Last 2 Hours";
            $start = $end - 7200;
			...
			# 48 hours
			$title = "$host $service Count Last 48 Hours";
			$start = $end - 172800;
			...
			# 14 days
			$title = "$host $service Count Last 14 Days";
			$start = $end - 1209600;
			...
			# 60 days
			$title = "$host $service Count Last 60 Days";
			$start = $end - 5184000;
			...
			# 360 days
			$title = "$host $service Count Last 360 Days";
			$start = $end - 31104000;
...
# This one forces lower limit to zero on graph
#		my $evalstring = '($averages,$xsize,$ysize) = RRDs::graph("$graphfile", "-s", $start, "-e", $end, "--color", "CANVAS#$bcolor", "--vertical-label", $vlabel, "-t", $title, "--lower-limit", 0, "-w", 700, "-h", 150, '.$defstring.");";

# This one allows floating upper and lower limits on graph
...
<tr>
<td class=head align=center>$host $service Count</td>
</tr>
<tr>
<td align=center>
...
<TITLE>$host $service Measurement</TITLE>
