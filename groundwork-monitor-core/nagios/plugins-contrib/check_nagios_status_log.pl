...
#
# Copyright 2004 - 2008 GroundWork OpenSource Solutions
#
...
use strict; 
use Getopt::Long;
use vars qw($rval @hostlist @servicelist $statlog $opt_l $opt_p $opt_H $opt_s $opt_V $opt_h $PROGNAME);
use lib "/usr/local/nagios/libexec";
use utils qw(%ERRORS &print_revision &support &usage);

my $debug = 0;
my $StatusLog = "/var/log/nagios/status.log";
...
# Handle -V here as host or service name is not required for version.
if ($opt_V) {
...
sub print_usage () {
    print "Usage:
    $PROGNAME -H <host:state,...> | -s <host:service:state,...> | -l [-v] [-p <path/file>]
    $PROGNAME -h | --help
    $PROGNAME -V | --version   
";
}
...
sub print_help () {
    print_revision($PROGNAME,'$Id: check_nagios_status_log.pl,v 1.3 2005/03/03 23:03:25 hmann Exp $');
    print "\nCopyright (c) 2008 Groundwork Open Source Solutions, Inc
