...
print_usage() {
  echo "Usage:"
  echo "  $PROGNAME -o <OID> -H <hostname> -w <warning level> -c <critical level> [-p <community string>] [-v <SNMP version>]"
  echo "  $PROGNAME -h "
...
while getopts ":o:p:H:w:c:v:h" Option
do
...
# Replace the commas in OID with space
OID=`echo $OID | sed -e 's/,/ /g'`
# Remove extra spaces
OID=`echo $OID | sed -e 's/\s+/ /g'`

shift $((OPTIND - 1))

if [[ $SNMPVER =~ [^0-9a-zA-Z.] ]]; then
        echo ""
        echo "Error - Incorrect snmp version"
        echo ""
        print_help
fi
...
FIRST=`$SNMPWALK $SNMPHOST -c $COMMSTR -v $SNMPVER $OID`
...
SECOND=`$SNMPWALK $SNMPHOST -c $COMMSTR -v $SNMPVER $OID`
