...
my $SAR = "/usr/local/groundwork/common/bin/sar"; 

use Getopt::Long;
use vars qw($opt_V $opt_c $opt_s $opt_n $opt_u $opt_i $opt_D $opt_p $opt_h $opt_t);
...
# Set-up the Locale to suppress AM/PM indicator in "sar" output.
$ENV{"LC_ALL"} = "C";
# Debug switch
if ($opt_D) { $debug = 1 }
...
if (!defined($opt_t)) {
	...
	($opt_t =~ /^\d+$/) ||
		usage("Invalid value: -t <Time interval in seconds>\n");
}
...
my ($lbl, $user, $nice, $sys, $iowait, $steal, $idle) = undef;
if ($cpu eq "ALL" ) {
	(@res = qx/$SAR $opt_t/) || die "No output from sar: $!";
} else {
	(@res = qx/$SAR $opt_t -P $cpu/) || die "No output from sar: $!"; 
}
foreach (@res) {
	chomp;
	# Ignore the line containing "Average".
	if (/Average/) { last };
	($lbl,$cpu,$user,$nice,$sys,$iowait,$steal,$idle) = split(/\s+/);
...
sub print_usage () {
	...
	[-t], --time (time interval in seconds)
...
-t, --time
   Time interval in seconds for sar
-h, --help
   Print help
-V, --Version
