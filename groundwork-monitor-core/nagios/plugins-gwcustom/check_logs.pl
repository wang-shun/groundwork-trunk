...
foreach my $cnf ( @log_files ) {
    ...
    # Open log file
    open( LOG_FILE, '<', $cnf->{'file_name'} ) or die "Unable to open log file $cnf->{'file_name'}: $!\n";
    ...
    open(SEEK_FILE, '>', $seek_file) or die "Unable to open seek count file $seek_file: $!";
