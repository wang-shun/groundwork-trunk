#!/bin/sh
# This script runs as sh, not as bash, to provide portability to AIX,
# which does not necessarily have bash installed.
...
if [ ! -d $WORK_DIR ]; then
	echo "Plugin error:  working directory $WORK_DIR is missing";
	exit 1;
fi

cd $WORK_DIR
if [ $? -ne 0 ]; then
	echo "Plugin error:  cannot change working directory to $WORK_DIR";
	exit 1;
fi
...
if [ $OLD_LINE_COUNT -gt $NEW_LINE_COUNT ]; then
	...
	if [ $OLD_LINE_COUNT -eq $NEW_LINE_COUNT ]; then
		...
		#  Exiting without re-analyzing the logfile.	
