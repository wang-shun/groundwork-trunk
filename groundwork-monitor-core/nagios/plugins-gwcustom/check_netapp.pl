...
my %OIDLIST = (
		...
		miscGlobalStatusMessage		=>	'1.3.6.1.4.1.789.1.2.2.25.0'
	      );
...
foreach $element ('failedfan','failedpsu') {
	...
	if(($my_return =~ /no/) || ($my_return == 0)) {
		...
		$state = "OK" unless ($state eq "CRITICAL");
	}
	else {
		$status_string .= "$element = $my_return ";
		$state = "CRITICAL";
...
my $globalstatus = &SNMPGET($OIDLIST{'miscGlobalStatus'});
if ($globalstatus != 3) { # not "ok"
