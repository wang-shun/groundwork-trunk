...
# Weighting Constants - defaults to 50-50
my $lambda0 = 0.5;
my $lambda1 = 1;
...
# feed this assumed value back into the array so we can use it for graphing later...
...
# Account for the annoying fact that we can have a negative prediction value by setting it to 0 if so.
						if ($alpha < 0 && $positive) {
							$alpha = 0;
						}
...
#	print "$graph_rrdfile, $starttime, $step, $ref_ds,$rows\n";
	...
	while ($row < $rows) {
		$value = $values[$row];
		$time_variable = $times[$rows];
  		if (($row == $first) && (!(defined$value))) {
			$first++;
		} else {
			if ($row == $first) {
# We are at first valid point  - look for the second
				print "first point = looking for next valid data point: value = $value, row = $row, init count = $first\n" if $debug;
				$value1 = $values[$row];
			} else {
				if (($row == $first+1) && (!(defined $value))) {
# second point not there, give up on first point
					print "not a valid second point - moving on: row = $row, init count = $first\n" if $debug;

					$first++;
					$first++;
				} else {
					if ($row == $first+1) {
# we are at the second data point - store the values as the same at time zero, and the actual and linear interpolation at time 1
						print "initial valid points - inserting data: row = $row, init count = $first\n" if $debug;
						#RRDs::update ( "$graph_rrdfile", "$times[$row-1]:$value1:$value1");
						$linear_value = ($beta*($row-1)) + $value1; # y = mx + b
						RRDs::update ( "$graph_rrdfile", "$times[$row]:$value1:$linear_value");
						print "value = $value1, trend = $linear_value\n" if $debug;
					}
# Now the general case - use the linear and real values to populate the rrd
					$linear_value = ($beta*($row-1)) + $value1; # y = mx + b
					RRDs::update ( "$graph_rrdfile", "$times[$row]:$value:$linear_value");
					print "regular points - inserting data: row = $row\n" if $debug;
