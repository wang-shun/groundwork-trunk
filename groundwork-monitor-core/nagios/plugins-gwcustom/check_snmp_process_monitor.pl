...
sub init_cache {
    ...
    # in this form the cache file is always read if present
    #
    if ( !$opt_cache ) {
        %processes = ();
        return;
...
sub snmpget {
    ...
    my $arg = shift ;
    my $addr = join '.', @$arg ;
    verbose "SNMPGET: $addr\n" ;
    # my $tmpvar = SNMP::Varbind->new( shift );
    my $tmpvar = SNMP::Varbind->new( $arg );
...
sub update_cache {
    ...
    #
    # new
    # to avoid the merging of a outdated cache with a full process scan
    #
    %processes = () ;
