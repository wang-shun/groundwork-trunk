 ...
 * License: GPL
 *
 * $Id: check_snmp.c,v 1.1.1.3 2005/01/27 00:07:03 hmann Exp $
 *
 * License Information:
 *
...
#define REVISION "$Revision: 1.1.1.3 $"
...
-H <ip_address> -o <OID> [-w warn_range] [-c crit_range] \n\
          ...
          [-X privpasswd] [-e performance data]\n"

#define LONGOPTIONS "\
 -H, --hostname=HOST\n\
    Name or IP address of the device you wish to query\n\
 -e, --performance\n\
    Output performance data after the plugin message\n\
 -o, --oid=OID(s)\n\
...
/* 	else */
	if (performance == 1) 
		printf ("%s %s -%s | %s\n", 
			label, state_text (result), outbuff, outbuff);
	else
...
#ifdef HAVE_GETOPT_H
		c =
			getopt_long (argc, argv, "hevVt:c:w:H:C:o:E:d:D:s:R:r:l:u:p:m:P:L:U:a:A:X:",
									 long_options, &option_index);
#else
		c = getopt (argc, argv, "hevVt:c:w:H:C:o:E:d:D:s:R:r:l:u:p:m:P:L:U:a:A:X:");
#endif
