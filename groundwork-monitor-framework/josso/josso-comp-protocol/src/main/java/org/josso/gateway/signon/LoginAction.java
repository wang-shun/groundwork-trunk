...
/**
 * Base login action extended by concrete actions associated to specific
 * authentication schemes. This actions controls the auth. process and invokes
 * subclasses methods (template method).
 * 
...
public abstract class LoginAction extends SignonBaseAction {
		...
		if (backTo != null) {
			...
			if (cfg.getTrustedHosts().size() > 0) {
				...
				if (backTo.startsWith("http://")
						|| backTo.startsWith("https://")) {
					try {
						URL backToUrl = new URL(backTo);
						backToHost = backToUrl.getHost();
						backToHost = backToHost.substring(backToHost
								.lastIndexOf("@") + 1);
					} catch (MalformedURLException e) {
						if (logger.isDebugEnabled())
							logger.debug("BackTo URL is malformed : [backTo="
									+ backTo + "]");
					}
				}
				for (String trustedHost : cfg.getTrustedHosts()) {
					if (StringUtils.isNotBlank(trustedHost)
							&& trustedHost.equals(backToHost)) {
						trusted = true;
						break;
					}
				}
			...
			if (!trusted && cfg.getTrustedHosts().size() > 0) {
				...
				logger.warn("Attempt to use untrusted host in back_to URL "
						+ backTo);
	 ...
	 */
	protected ActionForward askForLogin(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) {

		try {

			// Ask user for login information.
			SSOWebConfiguration cfg = Lookup.getInstance()
					.lookupSSOWebConfiguration();
	...
	protected ActionForward login(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			try {
				// 1 - Handle Outbound relaying by generating an assertion for
				// the authentication request
				SSOContext ctx = SSOContext.getCurrent();
				AuthenticationAssertion authAssertion = gwy.assertIdentity(c,
						ctx.getScheme());
				...
				ActionForward af = roleValidation(roles, request, response, mapping, c);
				if (af!= null)
					return af;
				...
				String back_to = this
						.getBackTo(request, session, authAssertion);

				
				if (back_to == null) {

					// Return to controller, if we do not have a back-to url,
				...
				return null; // No forward is needed, we perfomed a
								// 'sendRedirect'.

			} catch (AuthenticationFailureException e) {

				if (logger.isDebugEnabled())
					logger.debug("[AuthenticationFailureException] "
							+ e.getMessage(), e);
				...
				boolean ok = this.onLoginAuthenticationException(e, request,
						response, c);
				if (ok) {
					return null; // No forward is needed, we perfomed a
									// 'sendRedirect'.
				...
				SSOWebConfiguration cfg = SSOContext.getCurrent()
						.getSecurityDomain().getSSOWebConfiguration();
	...
	/**
	 * @param e
	 *            is the <AuthenticationFailureException> Exception to br
	 *            handled
	 * @param request
	 *            is the <HttpServletRequest> context
	 * @param response
	 *            is the <HttpServletResponse> context
	 * @param credentials
	 *            contains the <Crdential> used to perform de authentication
	 * @return false will execute the default mapping.findForward otherwise no
	 *         action will be taken
	 */
	protected boolean onLoginAuthenticationException(
			AuthenticationFailureException e, HttpServletRequest request,
			HttpServletResponse response, Credential[] credentials)
			throws IOException {
		...
		if (cmd != null && cmd.equals("login_optional")) {
			...
			if (logger.isDebugEnabled())
				logger
						.debug("[login()], Login Optional failed, redirecting user to : "
								+ back_to);
		...
		String on_error = (String) request.getSession(true).getAttribute(
				KEY_JOSSO_ON_ERROR);
		...
		if (on_error != null) {
			...
			// TODO : Improve error information handling, this could be managed
			// with an outbound mechanism, like assertions.

			// Add error type and received username to ERROR URL.
			SSOGateway g = getSSOGateway();
			on_error += (on_error.indexOf("?") >= 0 ? "&" : "?")
					+ "josso_error_type=" + e.getErrorType();
			try {
				SSOContext ctx = SSOContext.getCurrent();
				on_error += "&josso_username="
						+ g.getPrincipalName(ctx.getScheme(), credentials);
			} catch (Exception ex) {
				if (logger.isDebugEnabled())
					logger
							.error("  [onLoginAuthenticationException()] cant find PrincipalName");
			}

			response.sendRedirect(response.encodeRedirectURL(on_error));
			if (logger.isDebugEnabled())
				logger
						.debug("[login()], authentication failure. Redirecting user to : "
								+ on_error);
	 ...
	 */
	protected ActionForward relay(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			AuthenticationAssertion authAssertion = g.assertIdentity(session
					.getId());
	...
	protected boolean onFatalError(Exception e, HttpServletRequest request,
			HttpServletResponse response) {
		// Fatal error ...
		logger.error(e.getMessage(), e);
		ActionErrors errors = new ActionErrors();
		errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("sso.error", e
	...
	/**
	 * Check if the request can be relayed to the requesting party without
	 * having to reauthenticate.
	 * 
	 * @param request
	 * @return true if a relay can be achieved or false in case a new
	 *         authentication assertion must be issued.
	 */
	protected boolean canRelay(HttpServletRequest request) {
		 ...
		 * 
		 * 
		 * String jossoSessionId = getJossoSessionId(request); if (
		 * jossoSessionId != null ) { SSOSessionManager ssoSessionManager =
		 * Lookup.getInstance().lookupSecurityDomain().getSessionManager();
		 * SSOSession s = ssoSessionManager.getSession( jossoSessionId); if (s
		 * != null) canRelay = true; }
		 * 
		 * } catch (NoSuchSessionException e) { // Ingore this error .... we
		 * probably got an old SESSION id ... if (logger.isDebugEnabled())
	...
	/**
	 * Helper to check if there are any valid roles for the user. A valid user should have Authenticated Role and 
	 * any other role assigned. Else authorization error will be displayed
	 * @param roles
	 * @param request
	 * @param response
	 * @param mapping
	 ...
	 * @throws IOException
	 */
	private ActionForward roleValidation(SSORole[] roles,
			HttpServletRequest request, HttpServletResponse response,
			ActionMapping mapping, Credential[] c) throws IOException {
		if (roles != null && roles.length >= 2 && authenticatedFound(roles)) {
			logger.debug("Valid roles found for the user");
			return null;
		}
		else {
			logger.debug("No valid roles found for the user. Authorization failed");
			// logs the error
			ActionErrors errors = new ActionErrors();
			errors.add(ActionErrors.GLOBAL_ERROR, new ActionError(
					"sso.login.failed.invalid.role"));
			saveErrors(request, errors);

			// Invalid login attempt, redirect to ON ERROR URL, if any.
			boolean ok = this
					.onLoginAuthenticationException(
							new AuthenticationFailureException(
									"No Roles exist for the user. Atleast one role should be associated for a user"),
							request, response, c);
			if (ok) {
				return null; // No forward is needed, we perfomed a
								// 'sendRedirect'.
	...
	/**
	 * Helper to check if the role array contains Authenticated.
	 * @param roles
	 * @return
	 */
	private boolean authenticatedFound(SSORole[] roles) {
		boolean found = false;
		for (SSORole role : roles) {
			if (role.getName()!= null && role.getName().equalsIgnoreCase("Authenticated")) {
				return true;
			}
		}
		return false;
