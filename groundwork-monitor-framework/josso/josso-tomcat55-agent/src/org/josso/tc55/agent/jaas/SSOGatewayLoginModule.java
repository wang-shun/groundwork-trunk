 ...
 * Gateway by getting the associated user and roles and filling it to the
 * provided Subject. This way clients can obtain the authenticated identity
 * associated with the session and use it to protect resources (ie: web, etc.).
 * 
 * @author <a href="mailto:gbrigand@josso.org">Gianluca Brigandi</a>
 * @version CVS $Id: SSOGatewayLoginModule.java 1597 2010-03-02 04:05:45Z
 *          gnastov $
 */
public class SSOGatewayLoginModule extends GWSynchronizeLDAPLoginModule
		implements LoginModule {

	private static final Log logger = LogFactory
			.getLog(SSOGatewayLoginModule.class);
	...
	/**
	 * Authenticate the user by prompting for the SSO Session Identifier
	 * assigned by the SSO Gateway on logon.
	 * 
	 * This method obtains from the gateway, using the provided session
	 * identifier, the user associated with such session identifier. Only the
	 * NameCallBack is used, since its not a user/password pair but only one
	 * value containing the session identifier. Any other callback type is
	 * ignored.
	 * 
	...
	public boolean login() throws LoginException {
		...
		try {
			...
			if (request != null)
				...
				logger
						.warn("No SSO Agent request found in thread local variable, can't identify requester");
		...
		logger.debug("Requested authentication to gateway by " + _requester
				+ " using sso session " + ssoSessionId + "/" + ssoSessionId2);
		...
		try {
			...
			SSOIdentityManagerService im = Lookup.getInstance()
					.lookupSSOAgent().getSSOIdentityManager();
			...
			_succeeded = true;
			// Since Login is succeeded here, check for LDAP synch
			if (isLDAP && (isSynchronizeIdentity() || isSynchronizeRoles())) {
				String userName = ssoUser.getName();
				if (!userExists(userName) || !isPreserveRoles()) {
					// Since no user is found perform synch
					// This is the dummy secret. Secret is always in the LDAP
					String secret = "groundwork";
					// Only Synch if user doesn't exist in the database
					this.performSynchronization(userName, secret);
				} // end if
			} // end if
			...
			throw new FailedLoginException(
					"Fatal error authenticating session : " + e);
		}
		return true;
	}

	/**
	 * Helper method to find if user already in the database
	 * 
	 * @param userName
	 * @return
	 * @throws Exception
	 */
	private boolean userExists(final String userName) throws Exception {
		Boolean result = false;
		try {
			TransactionManager tm = (TransactionManager) new InitialContext()
					.lookup("java:/TransactionManager");
			result = (Boolean) Transactions.requiresNew(tm,
					new Transactions.Runnable() {
						public Object run() throws Exception {
							User dbUser = getUserModule().findUserByUserName(
									userName);
							if (dbUser != null)
			...
			// OK to catch. Do nothing if user doesnot exist
		}
	 ...
	 * succeeded.
	 * 
	 * Using the SSO user name, saved by the previosuly executed login()
	 * operation, obtains from the gateway the roles associated with the user
	 * and fills the Subject with the user and role principals. If this
	...
	public boolean commit() throws LoginException {
		if (_succeeded == false) {
			...
			try {
				...
				logger.debug("Added SSOUser Principal to the Subject : "
						+ _ssoUserPrincipal);
				...
				for (int i = 0; i < _ssoRolePrincipals.length; i++) {
					if (_subject.getPrincipals()
							.contains(_ssoRolePrincipals[i]))
						continue;

					_subject.getPrincipals().add(_ssoRolePrincipals[i]);
					logger.debug("Added SSORole Principal to the Subject : "
							+ _ssoRolePrincipals[i]);
				}

				commitSucceeded = true;
				return true;
			} catch (Exception e) {
				logger.error("Session login failed for Principal : "
						+ _ssoUserPrincipal, e);
				throw new LoginException(
						"Session login failed for Principal : "
								+ _ssoUserPrincipal);
			} finally {
	 ...
	 * Logout the user.
	 * 
	 * This method removes the SSO User and Role Principals from the Subject
	...
	public boolean logout() throws LoginException {
		...
		logger.debug("Removed SSOUser Principal from Subject : "
				+ _ssoUserPrincipal);

		// Remove all the SSORole Principals from the Subject.
		for (int i = 0; i < _ssoRolePrincipals.length; i++) {
			_subject.getPrincipals().remove(_ssoRolePrincipals[i]);
			logger.debug("Removed SSORole Principal from Subject : "
					+ _ssoRolePrincipals[i]);
		}
	...
	protected SSORole[] getRoleSets() throws LoginException {
		try {
			...
			logger.error("Session login failed for Principal : "
					+ _ssoUserPrincipal, e);
			throw new LoginException("Session login failed for Principal : "
					+ _ssoUserPrincipal);
		}
