# This is the groundwork-jira-integration spec file needed to construct
# the Groundwork Monitor JIRA Integration RPM.
...
%define	name		groundwork-jira-integration
...
Summary: GroundWork Monitor JIRA Integration Software
...
# We require no RPM pre-requisites for the groundwork-jira-integration RPM
...
software to integrate with JIRA helpdesk ticketing.
...
if [ `env | egrep -c '^PG_PASS=' || true` -ne 1 ]; then
    ...
    echo "        See the JIRA_INTEGRATION_INSTALL_NOTES file."
    exit 1
fi
...
if [ "$1" = 1 ]; then
    ...
    # echo "       no attempt will be made to kill a previous JIRA Integration."
...
if [ `expr match "$1" '[0-9]*'` -gt 0 ] && [ "$1" -gt 1 ]; then
    ...
    # FIX LATER:  We ought to shut down the JIRA Integration Software if
...
if [ "$1" = 1 -o "$1" = "configure" -o "$1" = "abort-remove" ]; then
    ...
    # FIX LATER:  Shut down any old processes associated with the JIRA Integration software.
...
if false; then
    ...
    sav_file=%{gwpath}/config/console-admin-config.xml.$timestamp.pre_jira

    if [ -f $old_file ]; then
	cp -p $old_file $sav_file
	if [ $? -ne 0 ]; then
	    echo "FATAL:  JIRA Integration RPM could not back up the console-admin-config.xml file." 1>&2
	    exit 1
	fi
    fi
    ...
    rpm_file=%{jirapath}/config/console-admin-config.xml

    if [ -f $rpm_file ]; then
	cp -p $rpm_file $old_file
	if [ $? -ne 0 ]; then
	    echo "FATAL:  JIRA Integration RPM could not install the console-admin-config.xml file." 1>&2
	    exit 1
	fi
    else
	echo "FATAL:  JIRA Integration RPM could not find the new console-admin-config.xml file." 1>&2
	exit 1
    fi
fi

# Make sure the JIRA Integration nagios cron job is in place.  We carry out
...
if [ $? -ne 0 ]; then
    echo "FATAL:  JIRA Integration RPM could not fetch the old nagios crontab" 1>&2
    exit 1
fi
if [ "`echo "$old_crontab" | sed -e 's/#.*//' | fgrep -c '/twoway_helpdesk.pl '`" -eq 0 ]; then
    ...
    new_cron_job='*/5 * * * * /usr/local/groundwork/jira/bin/twoway_helpdesk.pl > /dev/null 2>&1'
    echo "$old_crontab$newline$new_cron_job" | crontab -u nagios -
    if [ $? -ne 0 ]; then
	echo "FATAL:  JIRA Integration RPM could not install its nagios cron job" 1>&2
	exit 1
    fi
else
    echo "NOTICE:  The JIRA Integration nagios cron job looks like it was already in place."
fi
...
sav_file=/usr/local/groundwork/config/foundation.properties.$timestamp.pre_jira

old_properties=`cat $old_file`
if [ $? -ne 0 ]; then
    echo "FATAL:  JIRA Integration RPM could not read the old foundation.properties" 1>&2
    exit 1
fi
if [ "`echo \"$old_properties\" | sed -e 's/#.*//' | fgrep -c 'fas.executor.interrupt'`" -eq 0 ]; then
    ...
    if [ $? -ne 0 ]; then
	echo "FATAL:  JIRA Integration RPM could not copy the foundation.properties file." 1>&2
	exit 1
    fi
    ...
    if [ $? -ne 0 ]; then
	echo "FATAL:  JIRA Integration RPM could not create a revised foundation.properties file." 1>&2
	exit 1
    fi
    cp -p $old_file $sav_file
    if [ $? -ne 0 ]; then
	echo "FATAL:  JIRA Integration RPM could not back up the foundation.properties file." 1>&2
	exit 1
    fi
    mv $new_file $old_file
    if [ $? -ne 0 ]; then
	echo "FATAL:  JIRA Integration RPM could not install a new foundation.properties file." 1>&2
	exit 1
    fi
else
    echo "NOTICE:  The JIRA Integration special Foundation property"
...
# upgrade.  (The prepare_databases_for_jira script is idempotent, so that's
...
# the prepare_databases_for_jira script, it will default to localhost), and
...
# empty string.
%{jirapath}/db/prepare_databases_for_jira -e < /dev/tty
if [ $? -ne 0 ]; then
    # Although correct operation of the JIRA Integration depends on the
    ...
    echo "ERROR:  JIRA Integration RPM could not make required database changes." 1>&2
fi
...
if [ "$1" = 0 -o "$1" = "remove" ]; then
    ...
    # Make sure the JIRA Integration nagios cron job is removed.  We carry out
    # this action in small steps to minimize the likelihood of leaving the system in a mangled
    # state, having corrupted the existing content and not having installed the new content.
	...
	echo "NOTICE:  The JIRA Integration nagios cron job looks like it was already commented out or removed."
    else
	# new_cron_job='*/5 * * * * /usr/local/groundwork/jira/bin/twoway_helpdesk.pl > /dev/null 2>&1'
	new_crontab=`echo "$old_crontab" | sed -e '/\/twoway_helpdesk.pl /d'`
	if [ $? -ne 0 ]; then
	    echo "FATAL:  JIRA Integration RPM could not deal with its nagios cron job" 1>&2
	    exit 1
	fi
	echo "$new_crontab" | crontab -u nagios -
	if [ $? -ne 0 ]; then
	    echo "FATAL:  JIRA Integration RPM could not remove its nagios cron job" 1>&2
	    exit 1
	fi
    fi
    ...
    %{jirapath}/db/scrub_databases_for_jira
    if [ $? -ne 0 ]; then
	...
	echo "FATAL:  JIRA Integration RPM could not make required database changes." 1>&2
    fi
...
# way, we would want to kill all outstanding copies of the JIRA Integration software,
...
%attr(0644,root,root) %{logrotatepath}/groundwork-jira

# The tree of directories containing this software.
%dir %attr(0755,nagios,nagios) %{jirapath}
%dir %attr(0755,nagios,nagios) %{jirapath}/bin
%dir %attr(0755,nagios,nagios) %{jirapath}/config
%dir %attr(0755,nagios,nagios) %{jirapath}/db
%dir %attr(0755,nagios,nagios) %{jirapath}/doc
%dir %attr(0755,nagios,nagios) %{jirapath}/info
%dir %attr(0755,nagios,nagios) %{jirapath}/logs
%dir %attr(0755,nagios,nagios) %{jirapath}/perl

%attr(0444,nagios,nagios) %{jirapath}/db/delete_jira_actions.sql
%attr(0444,nagios,nagios) %{jirapath}/db/helpdesk_actions_seed_file.sql
%attr(0444,nagios,nagios) %{jirapath}/db/helpdesk_dynamic_property_types.sql
%attr(0444,nagios,nagios) %{jirapath}/db/helpdesk_initialize_bridge_db.sql
%attr(0754,nagios,nagios) %{jirapath}/db/prepare_databases_for_jira
%attr(0754,nagios,nagios) %{jirapath}/db/scrub_databases_for_jira

%attr(0444,nagios,nagios) %{jirapath}/info/build_info
...
%config(noreplace) %attr(0644,nagios,nagios) %{jirapath}/config/authorized_users.conf
# %config(noreplace) %attr(0600,nagios,nagios) %{jirapath}/config/bridge_db.conf
%config(noreplace) %attr(0644,nagios,nagios) %{jirapath}/config/oneway_helpdesk.conf
%config(noreplace) %attr(0600,nagios,nagios) %{jirapath}/config/jira_module.conf
%config(noreplace) %attr(0644,nagios,nagios) %{jirapath}/config/twoway_helpdesk.conf
# FIX MINOR
# %config(noreplace) %attr(0644,nagios,nagios) %{jirapath}/config/console-admin-config.xml
# FIX MAJOR:  drop these files if they don't make sense in this integration
# %config(noreplace) %attr(0644,nagios,nagios) %{jirapath}/config/hostgroup_and_service_to_assignment_group_mapping.conf
# %config(noreplace) %attr(0644,nagios,nagios) %{jirapath}/config/hostgroup_to_category_and_subcategory_mapping.conf
# %config(noreplace) %attr(0644,nagios,nagios) %{jirapath}/config/monarch_group_to_location_mapping.conf

%dir %attr(0755,nagios,nagios) %{jirapath}/doc/%{name}-%{version}
%doc %attr(0444,nagios,nagios) %{jirapath}/doc/%{name}-%{version}/JIRA_INTEGRATION_INSTALL_NOTES
%doc %attr(0444,nagios,nagios) %{jirapath}/doc/%{name}-%{version}/JIRA_INTEGRATION_RELEASE_NOTES
# FIX LATER:  create these files and enable their inclusion here
# %doc %attr(0444,nagios,nagios) %{jirapath}/doc/%{name}-%{version}/groundwork-jira-integration.1.0.pdf

%attr(0754,nagios,nagios) %{jirapath}/bin/oneway_helpdesk.pl
%attr(0754,nagios,nagios) %{jirapath}/bin/twoway_helpdesk.pl
...
# %attr(0644,root,root) /etc/logrotate.d/groundwork-jira-integration
...
# %dir %attr(0755,nagios,nagios) %{jirapath}/perl/bin
# %dir %attr(0755,nagios,nagios) %{jirapath}/perl/lib
# %dir %attr(0755,nagios,nagios) %{jirapath}/perl/man
# %dir %attr(0755,nagios,nagios) %{jirapath}/perl/share
...
%changelog
* Tue Apr 30 2013 Glenn Herteg <support@groundworkopensource.com> 1.0.0
