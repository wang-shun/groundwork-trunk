...
sub createTicket {
	...
	eval {
		...
		$OTRSBody .=	"MonitorStatus:   " . $ticketProps{'MonitorStatus'} . "\n"
			.	"ReportDate:      " . $ticketProps{'ReportDate'} . "\n"
			.	"MsgCount:        " . $ticketProps{'MsgCount'} . "\n"
			.	"TextMessage:     " . $ticketProps{'TextMessage'} . "\n\n";
		...
		my $ArticleID = $RPC->Dispatch( $OTRSUser, $OTRSPass, 'TicketObject', 'ArticleCreate',
		        TicketID         => $TicketID,
		...
		if ($ticketProps{'HostName'}) {
			my $returnCode = $RPC->Dispatch( $OTRSUser, $OTRSPass, 'TicketObject', 'TicketFreeTextSet',
				Counter		=> 1,
				Key		=> 'Host',
...
sub updateStatusForTickets {
	...
	eval {
		foreach my $event (@$ticketedEvents) {
			my %TicketData = $RPC->Dispatch( $OTRSUser, $OTRSPass, 'TicketObject', 'TicketGet',
		        	TicketID        => $event->{'ClientData'},
				UserID		=> 3,
			);
