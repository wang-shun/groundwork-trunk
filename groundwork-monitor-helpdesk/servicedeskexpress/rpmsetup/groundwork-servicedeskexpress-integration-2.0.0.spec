# This is the groundwork-servicedeskexpress-integration spec file needed to construct
# the Groundwork Monitor Service Desk Express Integration RPM.
...
%define	name		groundwork-servicedeskexpress-integration
...
%define	servicedeskexpresspath	%{gwpath}/servicedeskexpress
...
Summary: GroundWork Monitor Service Desk Express Integration Software
...
# We require no RPM pre-requisites for the groundwork-servicedeskexpress-integration RPM
...
software to integrate with Service Desk Express helpdesk ticketing.
...
# make servicedeskexpress_integration_install
...
if [ `env | egrep -c '^PG_PASS=' || true` -ne 1 ]; then
    ...
    echo "        See the SERVICEDESKEXPRESS_INTEGRATION_INSTALL_NOTES file."
    exit 1
fi
...
if [ "$1" = 1 ]; then
    ...
    # echo "       no attempt will be made to kill a previous Service Desk Express Integration."
...
if [ `expr match "$1" '[0-9]*'` -gt 0 ] && [ "$1" -gt 1 ]; then
    ...
    # FIX LATER:  We ought to shut down the Service Desk Express Integration Software if
...
if [ "$1" = 1 -o "$1" = "configure" -o "$1" = "abort-remove" ]; then
    ...
    # FIX LATER:  Shut down any old processes associated with the Service Desk Express Integration software.
...
if false; then
    ...
    sav_file=%{gwpath}/config/console-admin-config.xml.$timestamp.pre_servicedeskexpress

    if [ -f $old_file ]; then
	cp -p $old_file $sav_file
	if [ $? -ne 0 ]; then
	    echo "FATAL:  Service Desk Express Integration RPM could not back up the console-admin-config.xml file." 1>&2
	    exit 1
	fi
    fi
    ...
    rpm_file=%{servicedeskexpresspath}/config/console-admin-config.xml

    if [ -f $rpm_file ]; then
	cp -p $rpm_file $old_file
	if [ $? -ne 0 ]; then
	    echo "FATAL:  Service Desk Express Integration RPM could not install the console-admin-config.xml file." 1>&2
	    exit 1
	fi
    else
	echo "FATAL:  Service Desk Express Integration RPM could not find the new console-admin-config.xml file." 1>&2
	exit 1
    fi
fi

# Make sure the Service Desk Express Integration nagios cron job is in place.  We carry out
...
if [ $? -ne 0 ]; then
    echo "FATAL:  Service Desk Express Integration RPM could not fetch the old nagios crontab" 1>&2
    exit 1
fi
if [ "`echo "$old_crontab" | sed -e 's/#.*//' | fgrep -c '/twoway_helpdesk.pl '`" -eq 0 ]; then
    ...
    new_cron_job='*/5 * * * * /usr/local/groundwork/servicedeskexpress/bin/twoway_helpdesk.pl > /dev/null 2>&1'
    echo "$old_crontab$newline$new_cron_job" | crontab -u nagios -
    if [ $? -ne 0 ]; then
	echo "FATAL:  Service Desk Express Integration RPM could not install its nagios cron job" 1>&2
	exit 1
    fi
else
    echo "NOTICE:  The Service Desk Express Integration nagios cron job looks like it was already in place."
fi
...
sav_file=/usr/local/groundwork/config/foundation.properties.$timestamp.pre_servicedeskexpress

old_properties=`cat $old_file`
if [ $? -ne 0 ]; then
    echo "FATAL:  Service Desk Express Integration RPM could not read the old foundation.properties" 1>&2
    exit 1
fi
if [ "`echo \"$old_properties\" | sed -e 's/#.*//' | fgrep -c 'fas.executor.interrupt'`" -eq 0 ]; then
    ...
    if [ $? -ne 0 ]; then
	echo "FATAL:  Service Desk Express Integration RPM could not copy the foundation.properties file." 1>&2
	exit 1
    fi
    ...
    if [ $? -ne 0 ]; then
	echo "FATAL:  Service Desk Express Integration RPM could not create a revised foundation.properties file." 1>&2
	exit 1
    fi
    cp -p $old_file $sav_file
    if [ $? -ne 0 ]; then
	echo "FATAL:  Service Desk Express Integration RPM could not back up the foundation.properties file." 1>&2
	exit 1
    fi
    mv $new_file $old_file
    if [ $? -ne 0 ]; then
	echo "FATAL:  Service Desk Express Integration RPM could not install a new foundation.properties file." 1>&2
	exit 1
    fi
else
    echo "NOTICE:  The Service Desk Express Integration special Foundation property"
...
# upgrade.  (The prepare_databases_for_servicedeskexpress script is idempotent, so that's
...
# the prepare_databases_for_servicedeskexpress script, it will default to localhost), and
...
# empty string.
%{servicedeskexpresspath}/db/prepare_databases_for_servicedeskexpress -e < /dev/tty
if [ $? -ne 0 ]; then
    # Although correct operation of the Service Desk Express Integration depends on the
    ...
    echo "ERROR:  Service Desk Express Integration RPM could not make required database changes." 1>&2
fi
...
# Stop the Service Desk Express Integration.
#
...
if [ "$1" = 0 -o "$1" = "remove" ]; then
    ...
    # Make sure the Service Desk Express Integration nagios cron job is removed.  We carry out
    # this action in small steps to minimize the likelihood of leaving the system in a mangled
    # state, having corrupted the existing content and not having installed the new content.
	...
	echo "NOTICE:  The Service Desk Express Integration nagios cron job looks like it was already commented out or removed."
    else
	# new_cron_job='*/5 * * * * /usr/local/groundwork/servicedeskexpress/bin/twoway_helpdesk.pl > /dev/null 2>&1'
	new_crontab=`echo "$old_crontab" | sed -e '/\/twoway_helpdesk.pl /d'`
	if [ $? -ne 0 ]; then
	    echo "FATAL:  Service Desk Express Integration RPM could not deal with its nagios cron job" 1>&2
	    exit 1
	fi
	echo "$new_crontab" | crontab -u nagios -
	if [ $? -ne 0 ]; then
	    echo "FATAL:  Service Desk Express Integration RPM could not remove its nagios cron job" 1>&2
	    exit 1
	fi
    fi
    ...
    %{servicedeskexpresspath}/db/scrub_databases_for_servicedeskexpress
    if [ $? -ne 0 ]; then
	...
	echo "FATAL:  Service Desk Express Integration RPM could not make required database changes." 1>&2
    fi
...
# way, we would want to kill all outstanding copies of the Service Desk Express
...
%attr(0644,root,root) %{logrotatepath}/groundwork-servicedeskexpress

# The tree of directories containing this software.
%dir %attr(0755,nagios,nagios) %{servicedeskexpresspath}
%dir %attr(0755,nagios,nagios) %{servicedeskexpresspath}/bin
%dir %attr(0755,nagios,nagios) %{servicedeskexpresspath}/config
%dir %attr(0755,nagios,nagios) %{servicedeskexpresspath}/db
%dir %attr(0755,nagios,nagios) %{servicedeskexpresspath}/doc
%dir %attr(0755,nagios,nagios) %{servicedeskexpresspath}/info
%dir %attr(0755,nagios,nagios) %{servicedeskexpresspath}/logs
%dir %attr(0755,nagios,nagios) %{servicedeskexpresspath}/perl

%attr(0444,nagios,nagios) %{servicedeskexpresspath}/db/delete_servicedeskexpress_actions.sql
%attr(0444,nagios,nagios) %{servicedeskexpresspath}/db/helpdesk_actions_seed_file.sql
%attr(0444,nagios,nagios) %{servicedeskexpresspath}/db/helpdesk_dynamic_property_types.sql
%attr(0444,nagios,nagios) %{servicedeskexpresspath}/db/helpdesk_initialize_bridge_db.sql
%attr(0754,nagios,nagios) %{servicedeskexpresspath}/db/prepare_databases_for_servicedeskexpress
%attr(0754,nagios,nagios) %{servicedeskexpresspath}/db/scrub_databases_for_servicedeskexpress

%attr(0444,nagios,nagios) %{servicedeskexpresspath}/info/build_info
...
%config(noreplace) %attr(0644,nagios,nagios) %{servicedeskexpresspath}/config/authorized_users.conf
%config(noreplace) %attr(0600,nagios,nagios) %{servicedeskexpresspath}/config/bridge_db.conf
%config(noreplace) %attr(0644,nagios,nagios) %{servicedeskexpresspath}/config/oneway_helpdesk.conf
%config(noreplace) %attr(0600,nagios,nagios) %{servicedeskexpresspath}/config/servicedeskexpress_module.conf
%config(noreplace) %attr(0644,nagios,nagios) %{servicedeskexpresspath}/config/twoway_helpdesk.conf
# FIX MINOR
# %config(noreplace) %attr(0644,nagios,nagios) %{servicedeskexpresspath}/config/console-admin-config.xml
# %config(noreplace) %attr(0644,nagios,nagios) %{servicedeskexpresspath}/config/hostgroup_and_service_to_assignment_group_mapping.conf
# %config(noreplace) %attr(0644,nagios,nagios) %{servicedeskexpresspath}/config/hostgroup_to_category_and_subcategory_mapping.conf
# %config(noreplace) %attr(0644,nagios,nagios) %{servicedeskexpresspath}/config/monarch_group_to_location_mapping.conf

%dir %attr(0755,nagios,nagios) %{servicedeskexpresspath}/doc/%{name}-%{version}
%doc %attr(0444,nagios,nagios) %{servicedeskexpresspath}/doc/%{name}-%{version}/SERVICEDESKEXPRESS_INTEGRATION_INSTALL_NOTES
%doc %attr(0444,nagios,nagios) %{servicedeskexpresspath}/doc/%{name}-%{version}/SERVICEDESKEXPRESS_INTEGRATION_RELEASE_NOTES
# FIX LATER:  create these files and enable their inclusion here
# %doc %attr(0444,nagios,nagios) %{servicedeskexpresspath}/doc/%{name}-%{version}/groundwork-servicedeskexpress-integration.2.0.pdf

%attr(0754,nagios,nagios) %{servicedeskexpresspath}/bin/oneway_helpdesk.pl
%attr(0754,nagios,nagios) %{servicedeskexpresspath}/bin/twoway_helpdesk.pl
...
# %attr(0644,root,root) /etc/logrotate.d/groundwork-servicedeskexpress-integration
...
# %dir %attr(0755,nagios,nagios) %{servicedeskexpresspath}/perl/bin
# %dir %attr(0755,nagios,nagios) %{servicedeskexpresspath}/perl/lib
# %dir %attr(0755,nagios,nagios) %{servicedeskexpresspath}/perl/man
# %dir %attr(0755,nagios,nagios) %{servicedeskexpresspath}/perl/share
