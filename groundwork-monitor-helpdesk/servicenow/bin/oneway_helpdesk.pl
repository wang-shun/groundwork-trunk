...
# Copyright (c) 2014-2015 GroundWork Open Source (www.groundworkopensource.com).
...
use lib '/usr/local/groundwork/servicenow/perl/lib';
...
my $confFile = "/usr/local/groundwork/servicenow/config/oneway_helpdesk.conf";
...
sub main {
	...
	if ($numEvents) {
	    ...
	    ## It's possible that this failed because none of the the selected events matched one of
		...
		"NOTICE:  Log messages '%s' yielded no corresponding events."
		  . "  Did you select only events with Status other than the"
		  . " values configured in oneway_helpdesk.conf as being acceptable"
		  . " for incident generation?  Those values are currently:  %s",
...
sub processInputArguments {
	...
	# calling context.
	#
	# In the JIRA integration, since we are going to use this field as the JIRA
	# Summary, that timestamp is of little use to us.  The JIRA itself will
	# automatically contain a "Created:" field, which represents equivalent
	# information.
	#
	# In the ServiceNow integration, we already have an "Opened" field added
	# automatically by ServiceNow that contains an equivalent timestamp, so there is
...
sub createTicket {
	...
	if ( isTicketAlreadyFiled() ) {
		...
		if ($@) {
		    ...
		    ## We have sometimes seen exception messages contain full HTML documents and be tens of thousands of bytes long.
		    ## When that happens, it creates a huge mess in the Event Console window when we attempt to display that message,
		    ## as it shows up as a huge body of raw text rather than as a simple message.
...
sub updateTicketDetails {
	...
	if ( ( $ticketNo ne "" ) && ( $ticketStatus ne "" ) ) {
	    ...
	    ## FIX MAJOR:  shouldn't the succes or failure of these routines affect $outcome ?
