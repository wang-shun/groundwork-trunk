...
# Copyright 2014-2015 GroundWork Open Source, Inc. ("GroundWork").  All rights
...
#     PG_PASS (required; the PostgreSQL root account password on PG_HOST)
...
# We only execute this part of the post-install script if we actually
...
%{servicenowpath}/db/prepare_databases_for_helpdesk -e < /dev/tty
if [ $? -ne 0 ]; then
...
%config(noreplace) %attr(0644,nagios,nagios) %{servicenowpath}/config/monarch_group_to_location_mapping.conf
%config(noreplace) %attr(0644,nagios,nagios) %{servicenowpath}/config/oneway_helpdesk.conf
%config(noreplace) %attr(0644,nagios,nagios) %{servicenowpath}/config/servicegroup_to_assignment_group_mapping.conf
...
%changelog
* Tue Jan 13 2015 Glenn Herteg <support@groundworkopensource.com> 2.0.0
- added servicegroup_to_assignment_group_mapping.conf

* Mon Dec 15 2014 Glenn Herteg <support@groundworkopensource.com> 2.0.0
- ported to use PostgreSQL instead of MySQL

* Wed Jun 22 2011 Glenn Herteg <support@groundworkopensource.com> 1.0.1
- fixed trap processing in a database-initialization script

* Tue Jun 21 2011 Glenn Herteg <support@groundworkopensource.com> 1.0.1
- suppressed certain Perl dependences that escaped earlier testing
- upgraded the GW::Nagios module to a newer version, to fix simple bugs
  (that were not affecting operation of this integration, but we don't
  want to ship known-broken code)
