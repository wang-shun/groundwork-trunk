...
\set ticketing_provider LOWERCASE_TICKETING_PROVIDER
...
BEGIN
	...
	INSERT INTO ActionProperty (ActionID, Name, Value) VALUES(
	    (SELECT ActionID FROM Action WHERE Name = ActionMenuString),
	    'Script', '/usr/local/groundwork/ticketing/bin/oneway_helpdesk.pl'
	);
    END IF;
	...
	INSERT INTO ActionParameter (ActionID, Name, Value) VALUES (
	    (SELECT ActionID FROM Action WHERE Name = ActionMenuString),'Comment','Comment'
	);
    END IF;
...
SELECT install_helpdesk_actions(
%if "TICKETING_PROVIDER" eq "ServiceNow"
