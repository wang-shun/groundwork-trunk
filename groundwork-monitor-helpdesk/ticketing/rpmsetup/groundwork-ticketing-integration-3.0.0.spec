# This is the groundwork-ticketing-integration spec file needed to construct
# the Groundwork Monitor Ticketing Integration RPM.
...
%define	name		groundwork-%{lowercase_ticketing_provider}-integration
...
%define	ticketingpath	%{gwpath}/ticketing
...
Summary: GroundWork Monitor Ticketing Integration Software
...
# FIX MINOR:  We should put the BuildPrereq back, if we ever disable the
# "rpmbuild --dbpath" option in the makefile.
# BuildPreReq: patch

# We require no RPM pre-requisites for the groundwork-ticketing-integration RPM
...
supplying software to integrate with helpdesk ticketing.
...
# The list of Perl capabilities here reflects what we already provide
# with the Perl 5.24.0 release in GWMEE 7.1.1 and later.
...
%{__find_requires} $* |\
	...
	-e '/^perl(IO::Socket)/d'				\
	-e '/^perl(IO::Socket::SSL)/d'				\
	-e '/^perl(JSON)/d'					\
	-e '/^perl(LWP::UserAgent)/d'				\
	-e '/^perl(MIME::Base64)/d'				\
	-e '/^perl(MIME::Type)/d'				\
	-e '/^perl(MIME::Types)/d'				\
	-e '/^perl(POSIX)/d'					\
	-e '/^perl(POSIX::RT::Timer)/d'				\
	-e '/^perl(SOAP::Lite)/d'				\
	-e '/^perl(URI::URL)/d'					\
	-e '/^perl(lib)/d'					\
	-e '/^perl(strict)/d'					\
...
# make ticketing_integration_install
...
if [ `env | egrep -c '^PG_PASS=' || true` -ne 1 ]; then
    ...
    echo "        See the %{uppercase_ticketing_provider}_INTEGRATION_INSTALL_NOTES file."
    exit 1
fi
...
if [ "$1" = 1 ]; then
    ...
    # echo "       no attempt will be made to kill a previous ticketing Integration."
...
if [ `expr match "$1" '[0-9]*'` -gt 0 ] && [ "$1" -gt 1 ]; then
    ...
    # FIX LATER:  We ought to shut down the Ticketing Integration Software if it's running,
...
if [ "$1" = 1 -o "$1" = "configure" -o "$1" = "abort-remove" ]; then
    ...
    # FIX LATER:  How do we make a portable test here, to support Unbuntu as well?
    # if [ `expr match "$1" '[0-9]*'` -gt 0 ] && [ "$1" -gt 1 ]; then

    # Upgrading.

    # FIX LATER:  Shut down any old processes associated with the Ticketing Integration software.
...
# FIX MAJOR:
# If we execute the following code, we are replacing a GWMEE-supplied file that might have
# been further modified at the customer site by the addition of local filters.  We really
# don't want to go taking any of that stuff out of play simply because we want to add some
# filters of our own.  There should be some way for us to drop in a sibling file that does
# not clobber any existing file.  See GWMON-12889 for more information on this issue.
#
# FIX MAJOR:  We should only execute this part of the post-install script if we actually
# supply a modified copy of the console-admin-config.xml file in this RPM.  Figure out how
# to make this connection automatically, depending on what's in the file-list.
#
if false; then
    ...
    sav_file=%{gwpath}/config/console-admin-config.xml.$timestamp.pre_ticketing

    if [ -f $old_file ]; then
	cp -p $old_file $sav_file
	if [ $? -ne 0 ]; then
	    echo "FATAL:  Ticketing Integration RPM could not back up the console-admin-config.xml file." 1>&2
	    exit 1
	fi
    fi
    ...
    rpm_file=%{ticketingpath}/config/console-admin-config.xml

    if [ -f $rpm_file ]; then
	cp -p $rpm_file $old_file
	if [ $? -ne 0 ]; then
	    echo "FATAL:  Ticketing Integration RPM could not install the console-admin-config.xml file." 1>&2
	    exit 1
	fi
    else
	echo "FATAL:  Ticketing Integration RPM could not find the new console-admin-config.xml file." 1>&2
	exit 1
    fi
fi

# Make sure the Ticketing Integration nagios cron job is in place.  We carry out this
# action in small steps to minimize the likelihood of leaving the system in a mangled
# state, having corrupted the existing content and not having installed the new content.
old_crontab=`crontab -l -u nagios`
if [ $? -ne 0 ]; then
    echo "FATAL:  Ticketing Integration RPM could not fetch the old nagios crontab" 1>&2
    exit 1
fi
if [ "`echo "$old_crontab" | sed -e 's/#.*//' | fgrep -c '/twoway_helpdesk.pl '`" -eq 0 ]; then
    ...
    new_cron_job='*/5 * * * * /usr/local/groundwork/ticketing/bin/twoway_helpdesk.pl > /dev/null 2>&1'
    echo "$old_crontab$newline$new_cron_job" | crontab -u nagios -
    if [ $? -ne 0 ]; then
	echo "FATAL:  Ticketing Integration RPM could not install its nagios cron job" 1>&2
	exit 1
    fi
else
    echo "NOTICE:  The Ticketing Integration nagios cron job looks like it was already in place."
fi
...
timestamp=`date +"%%Y-%%m-%%d.%%H_%%M_%%S"`
old_file=%{gwpath}/config/foundation.properties
# FIX MINOR:  There are improved ways to name a temporary file that provide even better security.
new_file=%{gwpath}/config/foundation.properties.new.$$
sav_file=%{gwpath}/config/foundation.properties.$timestamp.pre_ticketing

old_properties=`cat $old_file`
if [ $? -ne 0 ]; then
    echo "FATAL:  Ticketing Integration RPM could not read the old foundation.properties" 1>&2
    exit 1
fi
if [ "`echo \"$old_properties\" | sed -e 's/#.*//' | fgrep -c 'fas.executor.interrupt'`" -eq 0 ]; then
    ...
    if [ $? -ne 0 ]; then
	echo "FATAL:  Ticketing Integration RPM could not copy the foundation.properties file." 1>&2
	exit 1
    fi
    ...
    if [ $? -ne 0 ]; then
	echo "FATAL:  Ticketing Integration RPM could not create a revised foundation.properties file." 1>&2
	exit 1
    fi
    cp -p $old_file $sav_file
    if [ $? -ne 0 ]; then
	echo "FATAL:  Ticketing Integration RPM could not back up the foundation.properties file." 1>&2
	exit 1
    fi
    mv $new_file $old_file
    if [ $? -ne 0 ]; then
	echo "FATAL:  Ticketing Integration RPM could not install a new foundation.properties file." 1>&2
	exit 1
    fi
else
    echo "NOTICE:  The Ticketing Integration special Foundation property"
...
#
# This script depends on possibly having the PG_HOST environment variable defined
# (inside the prepare_databases_for_helpdesk script, it will default to localhost),
# and definitely having the PG_PASS environment variable defined (representing the
# PostgreSQL postgres-account password on the PG_HOST).  In theory, we might also want
# the PG_PORT environment variable defined (it will default to port 5432), but in
...
#
# The -e option says the script should insist on drawing PG_HOST and PG_PASS from
# environment variables and failing if the script doesn't get what it needs via
# that route, rather than prompting the user for them if they are not so provided.
# Currently, the script ignores that flag, and it's no longer clear whether we really
# want to enforce it anyway.  We redirect the standard input stream to allow psql to
# prompt for a password, if PG_PASS is defined as an empty string.
%{ticketingpath}/db/prepare_databases_for_helpdesk -e < /dev/tty
if [ $? -ne 0 ]; then
    # Although correct operation of the Ticketing Integration depends on the
    ...
    echo "ERROR:  Ticketing Integration RPM could not make required database changes." 1>&2
fi
...
# Stop the Ticketing Integration.
#
...
if [ "$1" = 0 -o "$1" = "remove" ]; then
    ...
    # Make sure the Ticketing Integration nagios cron job is removed.  We carry out this
    # action in small steps to minimize the likelihood of leaving the system in a mangled
    # state, having corrupted the existing content and not having installed the new content.
	...
	echo "NOTICE:  The Ticketing Integration nagios cron job looks like it was already commented out or removed."
    else
	# new_cron_job='*/5 * * * * /usr/local/groundwork/ticketing/bin/twoway_helpdesk.pl > /dev/null 2>&1'
	new_crontab=`echo "$old_crontab" | sed -e '/\/twoway_helpdesk.pl /d'`
	if [ $? -ne 0 ]; then
	    echo "FATAL:  Ticketing Integration RPM could not deal with its nagios cron job" 1>&2
	    exit 1
	fi
	echo "$new_crontab" | crontab -u nagios -
	if [ $? -ne 0 ]; then
	    echo "FATAL:  Ticketing Integration RPM could not remove its nagios cron job" 1>&2
	    exit 1
	fi
    fi
    ...
    %{ticketingpath}/db/scrub_databases_for_helpdesk
    if [ $? -ne 0 ]; then
	...
	echo "FATAL:  Ticketing Integration RPM could not make required database changes." 1>&2
    fi
...
# Either way, we would want to kill all outstanding copies of the Ticketing
...
# FIX MAJOR:  Decide which way we want to fall on this decision.
...
%attr(0644,root,root) %{logrotatepath}/groundwork-ticketing

# The tree of directories containing this software.
%dir %attr(0755,nagios,nagios) %{ticketingpath}
%dir %attr(0755,nagios,nagios) %{ticketingpath}/bin
%dir %attr(0755,nagios,nagios) %{ticketingpath}/config
%dir %attr(0755,nagios,nagios) %{ticketingpath}/db
%dir %attr(0755,nagios,nagios) %{ticketingpath}/doc
%dir %attr(0755,nagios,nagios) %{ticketingpath}/info
%dir %attr(0755,nagios,nagios) %{ticketingpath}/logs
%dir %attr(0755,nagios,nagios) %{ticketingpath}/perl

%attr(0444,nagios,nagios) %{ticketingpath}/db/delete_helpdesk_actions.sql
%attr(0444,nagios,nagios) %{ticketingpath}/db/helpdesk_actions_seed_file.sql
%attr(0444,nagios,nagios) %{ticketingpath}/db/helpdesk_dynamic_property_types.sql
%attr(0444,nagios,nagios) %{ticketingpath}/db/helpdesk_initialize_bridge_db.sql
%attr(0754,nagios,nagios) %{ticketingpath}/db/prepare_databases_for_helpdesk
%attr(0754,nagios,nagios) %{ticketingpath}/db/scrub_databases_for_helpdesk

%attr(0444,nagios,nagios) %{ticketingpath}/info/build_info
...
%config(noreplace) %attr(0644,nagios,nagios) %{ticketingpath}/config/authorized_users.conf
%config(noreplace) %attr(0644,nagios,nagios) %{ticketingpath}/config/oneway_helpdesk.conf
%config(noreplace) %attr(0644,nagios,nagios) %{ticketingpath}/config/twoway_helpdesk.conf
%config(noreplace) %attr(0600,nagios,nagios) %{ticketingpath}/config/%{lowercase_ticketing_provider}_module.conf

# These files:
#
#     console-admin-config.xml
...
#
# currently make no sense in the context of a JIRA or OTRS integration.

%if "%{lowercase_ticketing_provider}" == "servicenow"

%config(noreplace) %attr(0644,nagios,nagios) %{ticketingpath}/config/console-admin-config.xml
%config(noreplace) %attr(0644,nagios,nagios) %{ticketingpath}/config/hostgroup_and_service_to_assignment_group_mapping.conf
%config(noreplace) %attr(0644,nagios,nagios) %{ticketingpath}/config/hostgroup_to_category_and_subcategory_mapping.conf
%config(noreplace) %attr(0644,nagios,nagios) %{ticketingpath}/config/monarch_group_to_location_mapping.conf
%config(noreplace) %attr(0644,nagios,nagios) %{ticketingpath}/config/servicegroup_to_assignment_group_mapping.conf

%endif

%dir %attr(0755,nagios,nagios) %{ticketingpath}/doc/%{name}-%{version}
%doc %attr(0444,nagios,nagios) %{ticketingpath}/doc/%{name}-%{version}/%{uppercase_ticketing_provider}_INTEGRATION_INSTALL_NOTES
%doc %attr(0444,nagios,nagios) %{ticketingpath}/doc/%{name}-%{version}/%{uppercase_ticketing_provider}_INTEGRATION_RELEASE_NOTES
# FIX LATER:  create these files and enable their inclusion here
# %doc %attr(0444,nagios,nagios) %{ticketingpath}/doc/%{name}-%{version}/groundwork-%{lowercase_ticketing_provider}-integration.1.0.pdf

%attr(0754,nagios,nagios) %{ticketingpath}/bin/oneway_helpdesk.pl
%attr(0754,nagios,nagios) %{ticketingpath}/bin/twoway_helpdesk.pl
...
# %dir %attr(0755,nagios,nagios) %{ticketingpath}/perl/bin
# %dir %attr(0755,nagios,nagios) %{ticketingpath}/perl/lib
# %dir %attr(0755,nagios,nagios) %{ticketingpath}/perl/man
# %dir %attr(0755,nagios,nagios) %{ticketingpath}/perl/share
...
%changelog
* Sun Feb 19 2017 Glenn Herteg <support@groundworkopensource.com> 1.0.0
- generalize RPM building to support multiple related integrations

* Sat Feb 11 2017 Glenn Herteg <support@groundworkopensource.com> 1.0.0
