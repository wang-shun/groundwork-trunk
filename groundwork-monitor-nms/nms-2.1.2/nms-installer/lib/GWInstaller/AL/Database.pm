...
sub can_connect{
	...
	if ($fqdn_status eq "fqdn") {
 	...
 	if($passwd){
 		$mysql_cmd = "/usr/local/groundwork/mysql/bin/mysql -p$passwd $host_segment -e 'select User from mysql.user limit 1' 2>&1";
		$mysql_login = `$mysql_cmd`; #mysql -p$passwd -e 'select User from mysql.user limit 1' 2>&1`;
	}
	else{
		
		$mysql_cmd = "/usr/local/groundwork/mysql/bin/mysql $host_segment -e 'select User from mysql.user limit 1' 2>&1";
