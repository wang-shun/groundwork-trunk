...
sub deploy_nms_cacti()
{
    ...
    if (!$there_is_a_previous_cacti_database)
    {
        ...
        if ($deploy_profile eq 'local')
        {
            `/usr/local/groundwork/mysql/bin/mysqladmin --user=$db_root_user --password=$db_root_password create $database_name`;
            `/usr/local/groundwork/mysql/bin/mysql --user=$db_root_user --password=$db_root_password $database_name < $cacti_home/cacti.sql`;
            `/usr/local/groundwork/mysql/bin/mysql --user=$db_root_user --password=$db_root_password $database_name < $patch/cacti.sql`;
        }
        else
        {
            `/usr/local/groundwork/mysql/bin/mysqladmin --user=$db_root_user --password=$db_root_password --host=$db_host create $database_name  >> nms.log 2>&1`;
            `/usr/local/groundwork/mysql/bin/mysql --user=$db_root_user --password=$db_root_password $database_name < $cacti_home/cacti.sql  >> nms.log 2>&1`;
            `/usr/local/groundwork/mysql/bin/mysql --user=$db_root_user --password=$db_root_password $database_name < $patch/cacti.sql  >> nms.log 2>&1`;
        }
    }
    else
    {
		...
		`cp -rf /usr/local/groundwork/cacti /usr/local/groundwork/nms/backups >/dev/null`;
            ...
            if ($deploy_profile eq 'local')
            {
                ...
                `/usr/local/groundwork/mysql/bin/mysqldump --user=$db_root_user --password=$db_root_password $database_name >/usr/local/groundwork/nms/backups/cacti/cacti.sql 2>&1`;
            }
            else
            {
                $dsn = "dbi:mysql:$database_name:$db_host:$db_port";
                `/usr/local/groundwork/mysql/bin/mysqldump --user=$db_root_user --password=$db_root_password --host=$db_host $database_name >/usr/local/groundwork/nms/backups/cacti/cacti.sql 2>&1`;
            }
    ...
    if ($deploy_profile eq 'local')
    {
            `/usr/local/groundwork/mysql/bin/mysql --user=$db_root_user --password=$db_root_password $database_name < /tmp/build_$instance.tmp >> nms.log 2>&1`;
    }
    else
    {
            `/usr/local/groundwork/mysql/bin/mysql --user=$db_root_user --password=$db_root_password --host=$db_host $database_name < /tmp/build_$instance.tmp  >> nms.log 2>&1`;
    }
    ...
    if ($deploy_profile eq 'local')
    {
        `/usr/local/groundwork/mysql/bin/mysql --user=$database_user --password=$database_password $database_name < $patch/pa.sql 2>>nms.log`;
    }
    else
    {
        `/usr/local/groundwork/mysql/bin/mysql --user=$database_user --password=$database_password --host=$db_host $database_name < $patch/pa.sql 2>>nms.log`;
    }
...
#1		#cacti application
#1    	$cmd = "/usr/local/groundwork/php/bin/php ./bin/install_application.php Cacti install $shortname  >/dev/null 2>&1";
#1    	print `$cmd`;
    	
    	#cacti sv graphs
#1    	$cmd = "/usr/local/groundwork/php/bin/php ./bin/install_application.php svcactigraph install $shortname  >/dev/null 2>&1";
#1    	print `$cmd`;
#1    }
#1    else{
...
#1    	GWInstaller::AL::GWLogger::log("WARNING: Installer will not be able to install Cacti GW Monitor Application.");
...
#1    	$cmd = "/usr/local/groundwork/php/bin/php ./bin/install_application.php cacti remove $shortname >/dev/null 2>&1";
#1    	print `$cmd`;

    	#cacti sv graphs
#1    	$cmd = "/usr/local/groundwork/php/bin/php ./bin/install_application.php svcactigraph remove $shortname  >/dev/null 2>&1";
#1    	print `$cmd`;
    	
#1    	#cacti sv graphs
#1    	$cmd = "rm -rf /usr/local/groundwork/core/guava/htdocs/guava/packages/svcactigraph  >/dev/null 2>&1";
#1    	print `$cmd`;
...
#1    	GWInstaller::AL::GWLogger::log("WARNING: There may be something wrong with your GW Monitor installation. Unable to find php binary.");
#1    	GWInstaller::AL::GWLogger::log("WARNING: Installer will not be able to remove Cacti GW Monitor Application.");
#1    }	
...
#1	my $guavapackagedir="$gw_home/core/guava/htdocs/guava/packages";
...
#1	my $patchfile="$guavapackagedir/$dirname/views/cacti.inc.php";

#1        if (! -d $guavapackagedir)
#1       {
#1                GWInstaller::AL::GWLogger::log("WARNING:Can only install a Cacti package if Groundwork is installed.");
#1        }

	# Delete Old Version
	GWInstaller::AL::GWLogger::log("\tCleaning previous Cacti Package.");
#1	`rm -rf $guavapackagedir/$dirname`;
#1	`rm -rf $guavapackagedir/svcactigraph`;
...
#1	`mkdir $guavapackagedir/svcactigraph`;
#1	`cp -rf $gw_home/nms/tools/installer/guava-packages/svcactigraph/* $guavapackagedir/svcactigraph`;

	# Now Patch the svcactigraph package.
	$cacti_url = "http:\\/\\/$cacti_host:$cacti_port\\/cacti\\/";
#1	$cmd = 'sed "s/this->cactiurl =.*/this->cactiurl = \"' . $cacti_url . '\";/g" -i ' . $guavapackagedir . '/svcactigraph/support/svcactigraphs.inc.php';
#1	`$cmd`;

#1	$cmd = 'sed "s/dbHost =.*/dbHost = ' . "'" . $cacti_db_host . "'" . ';/g" -i ' . $guavapackagedir . '/svcactigraph/sysmodules/svcactigraphs.inc.php';
	`$cmd`;
#1	$cmd = 'sed "s/dbUsername =.*/dbUsername = ' . "'" . $cacti_db_user . "'" . ';/g" -i ' . $guavapackagedir . '/svcactigraph/sysmodules/svcactigraphs.inc.php';
#1	`$cmd`;
#1	$cmd = 'sed "s/dbPassword =.*/dbPassword = ' . "'" . $cacti_db_pass . "'" . ';/g" -i ' . $guavapackagedir . '/svcactigraph/sysmodules/svcactigraphs.inc.php';
	`$cmd`;
#1	$cmd = 'sed "s/dbDatabase =.*/dbDatabase = ' . "'" . $cacti_db_name . "'" . ';/g" -i ' . $guavapackagedir . '/svcactigraph/sysmodules/svcactigraphs.inc.php';
	`$cmd`;
...
#1	$command = 'sed ' . "'" . 's/\\$this->baseURL =.*/\\$this->baseURL = "http:\\/\\/' . $cacti_host . ':' . $cacti_port . '";/g' . "'" . " $patchfile >$patchfile.new";
#1	`$command`;
#1	`rm -f $patchfile`;
...
#1	`$command`;
#1	$command = "sed 's/shortname = .*/shortname = cacti_$cacti_short_hostname_as_var/g' -i $patchfile";
#1	`$command`;
#1	$command = "sed 's/classname = .*/classname = cactiView$cacti_short_hostname_as_var/g' -i $patchfile";
#1	`$command`;
#1	$patchfile = "$guavapackagedir/$dirname/views/cacti.inc.php";
#1	$command = "sed 's/class cactiView .*/class cactiView$cacti_short_hostname_as_var extends GuavaApplication/g' -i $patchfile";
#1	`$command`;
