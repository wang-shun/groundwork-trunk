...
#1    	$cmd = "/usr/local/groundwork/php/bin/php ./bin/install_application.php ntop install $shortname  >> /dev/null 2>&1";
#1    	print `$cmd`;
#1    }
#1    else{
...
#1    	GWInstaller::AL::GWLogger::log("WARNING: Installer will not be able to install ntop GW Monitor Application.");
#1
#1    }
#1    return($result);
}
...
#1    my @properties = ();
#1    my $result = 0;
#1    $propfile = $installer->get_properties();
#1    my $host_name = $propfile->get_fqdn();
#1      my ($shortname,undef) = split('\.',$host_name);
#1
#1    ### Do Database removal
#1    GWInstaller::AL::GWLogger::log("Removing Guava Database items...");
#1    if(-e '/usr/local/groundwork/php/bin/php'){
#1    	$cmd = "/usr/local/groundwork/php/bin/php ./bin/install_application.php ntop remove $shortname >> /dev/null 2>&1";
#1    	print `$cmd`;
#1    }
#1    else{
#1
#1    	GWInstaller::AL::GWLogger::log("WARNING: There may be something wrong with your GW Monitor installation. Unable to find php binary.");
#1    	GWInstaller::AL::GWLogger::log("WARNING: Installer will not be able to remove ntop GW Monitor Application.");
#1    }	
    
    #Remove the files from the Guava Packages Directory
    
#1     $rmCMD = "rm -rf /usr/local/groundwork/core/guava/htdocs/guava/packages/ntop_" . $host_name . " >> /dev/null 2>&1";
#1     print `$rmCMD`;
#1}
...
#1        my $sourcedir = "/usr/local/groundwork/nms/tools/installer/guava-packages/ntop";
...
#1        {
#1                GWInstaller::AL::GWLogger::log("WARNING:Can only install a Ntop package if Groundwork is installed.");
#1        }
#1
#1	# 1. Delete Old Version
#1	GWInstaller::AL::GWLogger::log("\tCleaning previous Ntop Package.");
#1	`rm -rf $guavapackagedir/$dirname`;
#1
#1	# 2. Installing New version.
#1	GWInstaller::AL::GWLogger::log("\tInstalling new Ntop Package.");
#1	`mkdir $guavapackagedir/$dirname`;
#1	`cp -rf $sourcedir/* $guavapackagedir/$dirname`;
#1
#1	# 3. Patch with correct location of server.
