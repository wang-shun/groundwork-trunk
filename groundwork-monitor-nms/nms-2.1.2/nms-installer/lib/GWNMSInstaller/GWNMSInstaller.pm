...
# 6.0
        my $GInstaller = `grep "Installer name: " /usr/local/groundwork/Info.txt | sed 's/Installer name: //'`;
        my $CEInstaller = `grep "Installer name: groundwork-" /usr/local/groundwork/Info.txt`;
        my $EEInstaller = `grep "Pro" /usr/local/groundwork/Info.txt | grep " 6."`;
        if ( $CEInstaller ne "" ){
                $msg = "Found GroundWork Installer: $GInstaller  GroundWork NMS may not be installed with GroundWork Monitor Community Edition. Please upgrade to GroundWork Monitor Enterprise before continuing.";
                $nmscui->error_dialog($msg);
                $self->exit();
        }
        if ( $EEInstaller eq "" ){
                $msg = "Found GroundWork Installer: $GInstaller  This GroundWork NMS is not supporting your GroundWork Monitor Enterprise.";
                $nmscui->error_dialog($msg);
                $self->exit();
        }
        $msg = "Found GroundWork Installer $GInstaller installed on your system.";
