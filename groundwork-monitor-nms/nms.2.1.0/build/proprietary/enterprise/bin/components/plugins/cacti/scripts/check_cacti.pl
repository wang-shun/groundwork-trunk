#!/usr/local/groundwork/nms/tools/perl/bin/perl -w --
...
# (*) Someday, perhaps drop the "{HOST}" tier in the $alert_ref hash hierarchy, as it
#     is currently serving no purpose except to document the purpose of the next tier.
...
my $print_help = 0;
my $print_version = 0;
my $config_file = "/usr/local/groundwork/etc/check_cacti.conf";
my $debug_config = 0;
...
if ($print_version) {
    ...
    print "Copyright 2007 GroundWork Open Source, Inc. (\"GroundWork\").  All rights\n";
...
my $cacti_db_host = $config->get_scalar ('cacti_db_host');
my $cacti_db_name = $config->get_scalar ('cacti_db_name');
my $cacti_db_user = $config->get_scalar ('cacti_db_user');
my $cacti_db_pass = $config->get_scalar ('cacti_db_pass');
...
my           $send_nsca_command = "/usr/local/groundwork/bin/send_nsca -H $nsca_host -p $nsca_port -to $nsca_timeout -c /usr/local/groundwork/etc/send_nsca.cfg";
my $secondary_send_nsca_command = "/usr/local/groundwork/bin/send_nsca -H $secondary_nsca_host -p $secondary_nsca_port -to $secondary_nsca_timeout -c /usr/local/groundwork/etc/send_nsca.cfg";
...
for (;;) {
    ...
    #   Connect to Cacti DB
    #
    my $dbh = DBI->connect("DBI:mysql:$cacti_db_name:$cacti_db_host", $cacti_db_user, $cacti_db_pass);
    if (!$dbh) {
	print "Can't connect to database $cacti_db_name. Error:".$DBI::errstr;
	sleep (5) if (!$plugin_mode);	# Don't have gwservices restart us immediately, in a tight loop.
	exit $ERRORS{'CRITICAL'};
    }
	...
	$alert_ref->{HOST}->{$row->{description}}->{TOTAL_METRICS}++;
    $alert_ref->{HOST}->{$row->{description}}->{status} = $row->{status};
	my $host_hashref = $alert_ref->{HOST}->{$row->{description}};
	...
	if ($thold_alarm || $bl_alarm) {
	    ...
	    $host_hashref->{THOLD_ALARMS}++ if $thold_alarm;		# Keep track of all threshold alarms for this host
	    $host_hashref->{   BL_ALARMS}++ if    $bl_alarm;		# Keep track of all  baseline alarms for this host
	    ...
	    # autovivify the alarm counts so we don't risk referencing an undef later on, without damaging any existing value
    ...
    foreach my $host (sort keys %{$alert_ref->{HOST}}) {
	my $alarmcount = 0;
	my $sev = 0;
	my $host_hashref  = $alert_ref->{HOST}->{$host};
    ...
    my $host_status = sprintf "%0d",$alert_ref->{HOST}->{$host}->{status};
    my $serviceoutput = '';
    if ($host_status == 3 || $host_status == 2) { # if host is Up in cacti return threshold status.
	...
	my $id_hashref = $host_hashref->{ID}->{$id};
	    $serviceoutput .= '<br>'.(defined($id_hashref->{SERVICE}) ? $id_hashref->{SERVICE} : "UNKNOWN SERVICE").
		" last_value=".$id_hashref->{LASTREAD}." is out of bounds;";
	...
	if ($use_nsca) {
	    push @aggregated_commands,
		"$check_cacti_host\t$PROGNAME\t$service_result".
		"\tProcessed thresholds for $total_hosts in $elapsed_time ".($elapsed_time eq '1.000' ? 'second' : 'seconds').
		"|alarms=$alarm_count;;;0; metrics=$metric_count;;;0; hosts=$total_hosts;;;0; ExecutionTime=${elapsed_time}s;;".
		($elapsed_time_threshold > 0 ? $elapsed_time_threshold : '').";0;\n";
	} else {
	    push @aggregated_commands,
