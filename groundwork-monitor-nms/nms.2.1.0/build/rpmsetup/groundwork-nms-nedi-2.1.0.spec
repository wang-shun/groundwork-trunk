# This is the groundwork-nms-nedi spec file needed to construct the
# Groundwork Monitor NMS NeDi RPM.
...
%define	name		groundwork-nms-nedi
...
Summary: GroundWork Monitor NMS NeDi Module
License: All rights reserved.  Use is subject to GroundWork commercial license terms.
Group: Applications/Monitoring
Name: %{name}
Prefix: %{gwpath}
Release: %(cat %(echo $PWD)/rpmsetup/project.properties | fgrep 'org.groundwork.rpm.release.nedi.number' | awk '{ print $3; }')
...
# groundwork-nms-nedi RPM because we're adding certain new files and
...
a version of NeDi which is integrated into the monitoring system.
...
if [ "$1" = 0 ]; then
    ...
    # Last uninstall.
	# Remove cron entry if there is one.

	crontab -u nagios -l | grep -v "nedi.pl" >/tmp/clean_nedi.tmp 2>/dev/null;
	crontab -u nagios /tmp/clean_nedi.tmp;
...
# The groundwork-nms-nedi RPM should depend on the groundwork-nms-core RPM and include this file tree:
# %{gwpath}/nms/applications/nedi/...
