# This is the groundwork-nms-ntop spec file needed to construct the
# Groundwork Monitor NMS ntop RPM.
...
%define	name		groundwork-nms-ntop
...
Summary: GroundWork Monitor NMS ntop Module
License: All rights reserved.  Use is subject to GroundWork commercial license terms.
Group: Applications/Monitoring
Name: %{name}
Prefix: %{gwpath}
Release: %(cat %(echo $PWD)/rpmsetup/project.properties | fgrep 'org.groundwork.rpm.release.ntop.number' | awk '{ print $3; }')
...
# groundwork-nms-ntop RPM because we're adding certain new files and
...
a version of ntop which is integrated into the monitoring system.
...
if [ "$1" = 0 ]; then
    	# Last uninstall.
	# If NTOP has been deployed, then make sure that it
	# is dead before we uninstall.

	if [ -e /etc/init.d/nms-ntop ]; then
...
# The groundwork-nms-ntop RPM should depend on the groundwork-nms-core RPM and include this file tree:
# %{gwpath}/nms/applications/ntop/...
