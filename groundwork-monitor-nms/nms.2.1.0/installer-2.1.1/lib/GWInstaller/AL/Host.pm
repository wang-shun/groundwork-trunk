...
sub mysql_passwd_set{
	$retval = 0;
	$mysql_login = `mysql -e  'select User from mysql.user limit 1' 2>&1`;
	chomp($mysql_login);
...
sub verify_MySQL_config{
	...
	$retval = 1;
 	...
 	if($passwd){
 		$mysql_cmd = "mysql -p$passwd -e 'select User from mysql.user limit 1' 2>&1";
		$mysql_login = `mysql -p$passwd -e 'select User from mysql.user limit 1' 2>&1`;
	}
	else{
