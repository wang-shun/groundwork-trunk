...
sub deploy_nms_nedi
{
    ...
    if ($deploy_profile eq 'local')
    {
        `mysqldump --user=$db_root_user --password=$db_root_password $database_name --no-data=true >/tmp/nedi_dump.tmp 2>&1`;
    }
    else
    {
        `mysqldump --user=$db_root_user --password=$db_root_password --host=$db_host $database_name --no-data=true >/tmp/nedi_dump.tmp 2>&1`;
    }
    ...
    if ($there_is_a_previous_nedi_database && !$previous_database_is_up_to_date)
    {
        ...
        if ($deploy_profile eq 'local')
        {
            `mysqldump --skip-opt --no-create-db --no-create-info --user=$db_root_user --password=$db_root_password $database_name user configs devices monitoring links >/usr/local/groundwork/nms/backups/nedi/nedi.sql 2>&1`;
        }
        else
        {
            `mysqldump --skip-opt --no-create-db --no-create-info --user=$db_root_user --password=$db_root_password --host=$db_host $database_name user devices configs monitoring links >/usr/local/groundwork/nms/backups/nedi/nedi.sql 2>&1`;
        }
...
sub deploy_package
{
    ...
    if(-e '/usr/local/groundwork/php/bin/php'){
    	$cmd = "/usr/local/groundwork/php/bin/php ./bin/install_application.php NeDi install $shortname >/dev/null 2>&1";
    	print `$cmd`;
    }
    else{
    	...
    	GWInstaller::AL::GWLogger::log("WARNING: Installer will not be able to install NeDi GW Monitor Application.");
    }
...
sub remove_gwm_application{
    ...
    ### Do Database install
    if(-e '/usr/local/groundwork/php/bin/php'){
    	$cmd = "/usr/local/groundwork/php/bin/php ./bin/install_application.php nedi remove $shortname >/dev/null 2>&1";
    	print `$cmd`;
    }
    else{

    	GWInstaller::AL::GWLogger::log("WARNING: There may be something wrong with your GW Monitor installation. Unable to find php binary.");
    	GWInstaller::AL::GWLogger::log("WARNING: Installer will not be able to remove NeDi GW Monitor Application.");
    }
    
    
    #Remove the files from the Guava Packages Directory
    
     $rmCMD = "rm -rf /usr/local/groundwork/core/guava/htdocs/guava/packages/nedi_" . $host_name . " >/dev/null 2>&1";
     print `$rmCMD`;	
}
...
sub deploy_application_nedi_pkg
{
	...
	$gw_httpd_conf_dir = "/usr/local/groundwork/apache2/conf";
        ...
        my $sourcedir = "/usr/local/groundwork/nms/tools/installer/guava-packages/nedi";
	...
	my $patchfile="$guavapackagedir/$dirname/views/nedi.inc.php";
        ...
        {
                GWInstaller::AL::GWLogger::log("Can only install a Nedi package if Groundwork is installed.");
        }
