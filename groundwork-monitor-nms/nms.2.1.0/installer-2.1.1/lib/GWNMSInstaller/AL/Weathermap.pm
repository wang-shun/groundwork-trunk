...
sub remove_gwm_application{
    ...
    if(-e '/usr/local/groundwork/php/bin/php'){
    	$cmd = "/usr/local/groundwork/php/bin/php ./bin/install_application.php weathermap remove $shortname >> nms.log 2>&1";
    	print `$cmd`;
    }
    else{
    	...
    	GWInstaller::AL::GWLogger::log("WARNING: Installer will not be able to remove Weathermap GW Monitor Application.");
    }	
...
sub deploy_package()
{
    ...
    ### Do Database install
    if(-e '/usr/local/groundwork/php/bin/php'){
    	$cmd = "/usr/local/groundwork/php/bin/php ./bin/install_application.php Weathermap install $shortname  >> /dev/null 2>&1";
		print `$cmd`;
    }
    else{
    	GWInstaller::AL::GWLogger::log("WARNING: There may be something wrong with your GW Monitor installation. Unable to find php binary.");
    	GWInstaller::AL::GWLogger::log("WARNING: Installer will not be able to install Weathermap GW Monitor Application.");
