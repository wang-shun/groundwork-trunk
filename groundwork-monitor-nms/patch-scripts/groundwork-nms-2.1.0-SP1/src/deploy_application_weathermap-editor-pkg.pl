...
deploy_application_weathermap_editor_pkg()
{
	...
	$weathermap_editor_port = find_property($type, $subtype, $instance, "weathermap-editor:httpd:port");
        ...
        my $sourcedir = "$enterprise_home/bin/components/guava-packages/weathermap-editor";
my $guavapackagedir="$gw_home/core/guava/htdocs/guava/packages";
	...
	my $patchfile="$guavapackagedir/$dirname/views/weathermapeditor.inc.php";
        ...
        {
                leave_with_error("Can only install a Weathermap Editor package if Groundwork is installed.");
        }
