...
int main(int argc, char *argv[]) {
	...
	if (mysql_thread_safe()) {
		if (set.log_level == POLLER_VERBOSITY_DEBUG) {
			SPINE_LOG(("DEBUG: Database is Thread Safe!"));
		}
	}else{
		SPINE_LOG(("WARNING: Database is NOT Thread Safe!"));
	}
	...
	result = db_query(&mysql,
		...
		" and table_name='host' and column_name like 'poller_id'");
#else
	...
	result = db_query(&mysql,
		"select column_name from information_schema.columns"
		" where table_catalog = current_catalog and table_schema = current_schema"
		" and table_name='host' and column_name like 'device_threads'");
#else
...
#if USE_POSTGRESQL
		snprintf(querybuf, BIG_BUFSIZE, "INSERT INTO poller_time (poller_id, pid, start_time, end_time) VALUES (%i, %i, NOW(), 'epoch')", set.poller_id, getpid());
#else
	...
	if (db_insert(&mysql, "start transaction isolation level serializable") &&
		db_insert(&mysql, "delete from settings where name = 'date'") &&
		db_insert(&mysql, "insert into settings (name,value) values ('date',NOW())")) {
