...
unless ($is_portal) {
	...
	my @files = ('monarch.js','nicetitle.js','groundwork.css','monarch.css','dtree.js','dtree.css');
	...
	@files = ('MonarchFile.pm','MonarchStorProc.pm','MonarchExternals.pm','MonarchDoc.pm','MonarchLoad.pm','MonarchProfileExport.pm',
		'MonarchProfileImport.pm','nagios2collage.pm');
	foreach my $file (@files) {
...
my ($nagios_version) = $dbh->selectrow_array($sqlstmt);
...
$sth = $dbh->prepare ($sqlstmt);
unless ($sth->execute) { print "Error: $sqlstmt $@" }
$sqlstmt = "insert into setup values('enable_externals','config','0')"; 
$sth = $dbh->prepare ($sqlstmt);
unless ($sth->execute) { print "Error: $sqlstmt $@" }
...
$dbh->do("delete from file_host where file_id not in (select file_id from files)");
$dbh->do("delete from file_host where host_id not in (select host_id from hosts)");
$dbh->do("delete from file_service where file_id not in (select file_id from files)");
$dbh->do("delete from file_service where service_id not in (select service_id from services)");
$dbh->do("update files set type = 'hosts' where type = 'host'");
$dbh->do("update files set type = 'services' where type = 'service'");
...
#
# Add service groups to asset list
#


$sqlstmt = "select * from access_list where object = 'servicegroups'";
$sth = $dbh->prepare($sqlstmt);
$sth->execute();
my $check = undef;
while(my @values = $sth->fetchrow_array()) {
...
unless ($check) {
	$dbh->do("insert into access_list values('servicegroups','design_manage','1','add,modify,delete')");
...
$sqlstmt = "select * from access_list where object = 'externals'";
$sth = $dbh->prepare($sqlstmt);
$sth->execute();
my $check = undef;
while(my @values = $sth->fetchrow_array()) {
...
unless ($check) {
	$dbh->do("insert into access_list values('externals','design_manage','1','add,modify,delete')");
...
# Resource macro 
#
$dbh->do("update access_list set object = 'nagios_resource_macros' where object like '%resource%' and type = 'control'");
...
if (defined $fields{'servicename_id'}) {
	...
	while(my @temp = $sth->fetchrow_array()) { 
		if ($temp[1]) {
			unless ($splat_id eq $temp[1]) {
				$sqlstmt = "select tree_id from escalation_tree_template where tempalte_id = '$temp[0]'";
...
my @types = ('commands','misccommands','time_periods','contact_templates','contactgroups','contacts',
'extended_host_info_templates','extended_host_info','extended_service_info_templates','extended_service_info',
'host_dependencies','host_templates','hostgroups','hosts','service_dependency',
'service_dependency_templates','service_templates','services','escalation_templates','escalations');


$sqlstmt = "select * from files";
$sth = $dbh->prepare($sqlstmt);
$sth->execute();
my %files = ();
my %file_name = ();
my %type_file = ();
while(my @values = $sth->fetchrow_array()) {
	$files{$values[3]} = $values[0];
	$file_name{$values[1]} = $values[0];
	$type_file{$values[0]} = $values[3];
}
$sth->finish;

$sqlstmt = "select * from file_host";
$sth = $dbh->prepare($sqlstmt);
$sth->execute();
my %file_host = ();
while(my @values = $sth->fetchrow_array()) {
	$file_host{$values[0]} = $values[1];
}
$sth->finish;

$sqlstmt = "select * from file_service";
$sth = $dbh->prepare($sqlstmt);
$sth->execute();
my %file_service = ();
while(my @values = $sth->fetchrow_array()) {
	$file_service{$values[0]} = $values[1];
}
$sth->finish;

$sqlstmt = "select name, value from setup where type = 'file'";
$sth = $dbh->prepare($sqlstmt);
$sth->execute();
my %file_setup = ();
while(my @values = $sth->fetchrow_array()) {
	$file_setup{$values[0]} = $values[1];
}
$sth->finish;

$dbh->do("delete from setup where type = 'file'");

if (exists $file_name{'services.cfg'}) {
	$files{'services'} = $file_name{'services.cfg'};
} else {
	$dbh->do("insert into files values('','services.cfg','file','services')");
	$files{'services'} = $dbh->selectrow_array("select max(file_id) as lastrow from files");
}

if (exists $file_name{'hosts.cfg'}) {
	$files{'hosts'} = $file_name{'hosts.cfg'};
} else {
	$dbh->do("insert into files values('','hosts.cfg','file','hosts')");
	$files{'hosts'} = $dbh->selectrow_array("select max(file_id) as lastrow from files");
}

foreach my $type (@types) {
	unless ($files{$type}) {
		my $name = "$type\.cfg";
		$dbh->do("insert into files values('','$name','file','$type')");
		$files{$type} = $dbh->selectrow_array("select max(file_id) as lastrow from files");
	}
	$dbh->do("insert into setup values('$type','file',$files{$type})");
}

foreach my $file (keys %file_host) {
	unless ($type_file{$file} eq 'hosts') {
		$dbh->do("delete from file_host where file_id = '$file'");
	}
}

foreach my $file (keys %file_service) {
	unless ($type_file{$file} eq 'services') {
		$dbh->do("delete from file_service where file_id = '$file'");
	}
}
...
my @drop_list = ('stage_escalations','match_strings','import_schemas','stage_status');
