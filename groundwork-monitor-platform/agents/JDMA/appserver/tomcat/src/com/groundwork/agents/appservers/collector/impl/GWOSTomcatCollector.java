...
public class GWOSTomcatCollector extends GWOSCollectorImpl {
	...
	private static org.apache.log4j.Logger log = Logger.getLogger(GWOSTomcatCollector.class);

	private MBeanServerConnection adminclient = null;
	private TomcatAdminClient tam = null;

	public GWOSTomcatCollector() throws Exception {
		tam = new TomcatAdminClient();
		adminclient = tam.createMBeanServerConnection();
		properties = StatUtils.readProperties(TomcatAdminClient.TOMCAT_PROPERTIES);
	}

	public GWOSTomcatCollector(Properties prop) throws Exception {
		tam = new TomcatAdminClient();
		adminclient = tam.createMBeanServerConnection(prop);
		properties = prop;
	}
			...
			if (!mBeans.isEmpty()) {
				for (Object obj : mBeans) {
					...
					for (MBeanAttributeInfo attInfo : attInfoArray) {
						...
						if (attInfo.getType() != null
								&& (attInfo.getType().equals("long") || attInfo.getType().equals("java.lang.Long"))) {
							if (attInfo.getName() != null
									&& !attInfo.getName().equals("debug")
									&& attInfo.isReadable()) {
								long value = -1;
