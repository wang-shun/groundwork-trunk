...
<head>
    ...
    <script type="text/javascript" charset="utf-8">
        function validateOnSubmit() {
            if (document.assignForm.alias.length === undefined) {
                if (document.assignForm.alias.value == '' || document.assignForm.critThreshold.value == '' || document.assignForm.warnThreshold.value == '') {
                    alert('Please enter a valid alias, critical threshold and warning threshold for ' + document.assignForm.alias.value);
                    return false;
                }
                else {
                    var str = document.assignForm.alias.value;
                    if (!isSpclChar(str, document.assignForm.alias.value)) {
                        return false;
                    } // end if
                    if (!isNumeric(document.assignForm.critThreshold.value, document.assignForm.alias.value) || !isNumeric(document.assignForm.warnThreshold.value, document.assignForm.alias.value)) {
                        return false;
                    } // end if
                }  // end if
                ...
                <table cellpadding="0" cellspacing="0" border="0" class="display" id="example" style="width:auto;">
                    ...
                    <%
                        ...
                        if (objServices != null) {
                            ...
                            String mbeanPrefix = prop.getProperty("mbean.prefix");
                            for (String service : services) {
                                String alias = null;
                                if (mbeanPrefix != null && !mbeanPrefix.equalsIgnoreCase(""))
                                    alias = mbeanPrefix + "." + service;
                                else
                                    alias = service;
                    ...
                    <tr>
                        ...
                        <td><input type="text" name="alias" id="alias" size="50"
                                   maxlength="255" value="<%=alias%>" class="text"/></td>
                        <td><input type="text" name="warnThreshold" id="warnThreshold"
                                   size="10" maxlength="19" value="5" class="text"/></td>
                        <td><input type="text" name="critThreshold" id="critThreshold"
                                   size="10" maxlength="19" value="10" class="text"/></td>
                    </tr>
                    <%
