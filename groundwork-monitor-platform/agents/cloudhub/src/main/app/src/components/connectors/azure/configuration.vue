<template>
    <div class="container-fluid">
        ...
        <div class="row margin-top90">
            <div class="col-md-4">
                ...
                <form class="form-horizontal">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            <input type="text" :class="{'form-control': true, 'invalid': errors.has('config-hostname') }" id="config-hostname" name="config-hostname" placeholder="Hostname" v-model="gwos.gwosServer" v-validate="{ rules: { required: true, regex: /^[a-zA-Z0-9_\.\-\:]*$/ } }" data-vv-delay="500" />
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            <input autocomplete="new-password" type="text" :class="{'form-control': true, 'invalid': errors.has('config-username') }" id="config-username" name="config-username" placeholder="Username" v-model="gwos.wsUsername" v-validate="{ rules: { required: true } }" data-vv-delay="500" />
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            <input autocomplete="new-password" type="password" :class="{'form-control': true, 'invalid': errors.has('config-token') }" id="config-token" name="config-token" placeholder="Token" v-model="gwos.wsPassword" v-validate="{ rules: { required: true } }" data-vv-delay="500" />
                        </div>
                    </div>
            ...
            <div class="col-md-4">
                <p><strong>Azure Connector</strong></p>
                <hr />
                <form class="form-horizontal">
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label for="file" class="col-sm-4 control-label">Configuration File</label>
                        <div class="col-sm-8">
                            <form><input type="file" class="form-control" id="config-configFile" name="file" v-on:change="upload" /></form>
                            <input type="text" id="config-configFile-label" class="form-control" v-model="connection.credentialsFile" readonly />
                        </div>
                    </div>
                    ...
                    <div class="form-group form-vcentered">
                        ...
                        <div class="col-sm-5 col-xs-6">
                          <div class="checkbox">
                              <span class="pull-left hidden-xs">Interval (min):</span>
                              <span class="col-xs-12 visible-xs">Interval (min):</span>
                        ...
                        <div class="col-sm-7 col-xs-6">
                          <div class="checkbox">
                              <span class="pull-left col-sm-6 hidden-xs">Timeout (ms):</span>
                              <span class="col-xs-12 visible-xs">Timeout (ms):</span>
                              <span class="col-sm-6 col-xs-12 no-padding-right short-padding-left">
                                <input id="config-check-timeout" name="config-check-timeout" type="number" :class="{'form-control': true, 'invalid': errors.has('config-check-timeout') }" min="1" max="9999" v-model="connection.timeoutMs" v-validate="{ rules: { required: true } }" />
                              </span>
                          </div>
                        </div>
                    ...
                    <div class="form-group form-vcentered">
                        <div class="col-sm-4 col-xs-12">
                          <div class="checkbox">
                            <label class="hidden-xs">
                              <input id="config-resource-groups-1" name="config-resource-groups-1" type="checkbox" v-model="connection.enableResourceGroups" v-validate> Enable Resource Groups
                            </label>
                            <span class="col-xs-12 visible-xs">Enable Resource Groups:</span>
                            <label class="col-xs-12 visible-xs">
                              <input id="config-resource-groups-2" name="config-resource-groups-2" type="checkbox" v-model="connection.enableResourceGroups" class="checkbox-margin" v-validate> Yes
                            </label>
                          </div>
                        </div>
                    </div>
            ...
            <div class="col-md-4">
                <p><strong>Azure Service Resources</strong></p>
                <hr />
                <form class="form-horizontal">
                    <div class="form-group">
                        <div class="col-sm-offset-3 col-sm-3">
                            <button type="button" :disabled="!configName.length" v-on:click="discoverShow">Discover...</button>
                        </div>
                    </div>

                    <div class="form-group" v-for="view in views" v-show="view.enabled">
                        <div class="col-sm-offset-3 col-sm-3">
                            <div class="checkbox">
                                <label>
                                    <input type="checkbox" v-model="view.enabled" :disabled="view.name == 'CLUSTER'"> {{view.name}}
                                </label>
                            </div>
                        </div>
                    </div>
        ...
        <hsy-dialog class="confirm" id="azure-service-discovery" v-model="discover">
            <div slot="title">Azure Service Discovery</div>
            <div slot="body">
                <div class="row" v-for="view in discoveryServices">
                    <div class="col-sm-5">
                        <div class="checkbox">
                    ...
                    <div class="form-group">
                        <div class="col-sm-3">
                            <div class="checkbox no-margin-top">
                                <label>
                                    <input type="checkbox" v-model="view.enabled"> {{view.name}}
                                </label>
                            </div>
                        </div>
                    </div>
                ...
                <br />
                <div>
                    <button @click="discoverCancel">Cancel</button>&nbsp;&nbsp;
                    <button @click="discoverSave">Save</button>
                </div>
            </div>
        </hsy-dialog>
        ...
        <hsy-dialog class="confirm" v-model="prefix">
            ...
            <div slot="body">
                <div>Cluster Prefix has changed. Changing the prefix will rename all of your Hostnames for Azure Services.<br /><br />Are you sure you want to proceed?</div><br />
                <div>
        ...
        <hsy-dialog class="confirm" id="azure-confirmation-dialog" v-model="visible">
...
export default {
  ...
  data () {
    return {
      ...
      enableResourceGroupsInitial: false,
  ...
  methods: {
    retrieve: function(configName) {
      ...
      this.service.retrieve(configName, function(configurationData)
      {
        ...
        self.enableResourceGroupsInitial = configurationData.connection.enableResourceGroups;
        ...
        self.metricsLink = '/metrics/azure?name=' + self.common.configurationFile + '&profile=' + self.common.agentId;
      });
    },

    upload: function(event) {
        var form = event.target.parentNode, self = this;

        this.service.upload(form, function(response)
        {
            self.connection.credentialsFile = response;
        });
    },
    ...
    getState: function() {
        ...
        return {
            ...
            "resourceGroupsChanged": (this.enableResourceGroupsInitial !== this.connection.enableResourceGroups),
    ...
    save: function(event, skipCheck) {
        ...
        function(result) {
            self.configName = result.configuration.common.configurationFile;
            ...
            self.prefixServiceNamesInitial = self.common.prefixServiceNames;
            self.enableResourceGroupsInitial = self.connection.enableResourceGroups;

            self.monitorInitial = self.gwos.monitor;
            self.gwosServerInitial = self.gwos.gwosServer;
            self.displayNameInitial = self.common.displayName;

            self.$router.push({ path: '/configuration/azure', query: { name: self.common.configurationFile }})
        });
    },
    ...
    discoverShow: function() {
        ...
        this.service.getDiscoveryServices({name: self.common.configurationFile, profile: self.common.agentId},
            function(serviceList) {
                self.discoveryServices = serviceList;
                self.discover = true;
            },
            function(error) {
            });
    ...
    discoverCancel: function() {
        this.discover = false;
    },

    discoverSave: function() {
        this.views = this.discoveryServices.slice();
        this.discover = false;
    },
