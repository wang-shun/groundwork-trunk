<template>
    <div class="container-fluid">
        ...
        <div class="row margin-top90">
            ...
            <div class="col-md-4">
                <p><strong>Docker Connector</strong></p>
                <hr />
                <form class="form-horizontal">
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label for="config-server" class="col-sm-4 control-label">Docker Host:Port</label>
                        <div class="col-sm-8">
                            <input type="text" :class="{'form-control': true, 'invalid': errors.has('config-server') }" id="config-server" name="config-server" placeholder="Docker Host:Port" v-model="connection.server" v-validate.initial="{ rules: { required: true, regex: /^[a-zA-Z0-9_\.\-\:]*$/ } }" />
                        </div>
                    </div>

                    <div class="form-group">
                        <label for="config-prefix" class="col-sm-4 control-label">Host Prefix</label>
                        <div class="col-sm-8">
                            <input type="text" :class="{'form-control': true, 'invalid': errors.has('config-prefix') }" id="config-prefix" name="config-prefix" placeholder="Display Name" v-model="connection.prefix" v-validate.initial="{ rules: { required: true, regex: /^[a-zA-Z0-9_\.\-\:]*$/ } }" />
                        </div>
                    </div>
        ...
        <hsy-dialog class="confirm" v-model="prefix">
            ...
            <div slot="body">
                <div>Cluster Prefix has changed. Changing the prefix will rename all of your Hostnames for Docker Services.<br /><br />Are you sure you want to proceed?</div><br />
                <div>
  ...
  methods: {
    retrieve: function(configName) {
      ...
      this.service.retrieve(configName, function(configurationData)
      {
        ...
        self.hostPrefixInitial = self.connection.prefix;
        self.displayNameInitial = self.common.displayName;

        self.metricsLink = '/metrics/docker?name=' + self.common.configurationFile + '&profile=' + self.common.agentId;
      });
    },
    ...
    getState: function() {
        return {
            ...
            "hostPrefixChanged": (this.hostPrefixInitial !== this.connection.prefix),
    ...
    save: function(event, skipCheck) {
        ...
        function(result) {
            ...
            self.$router.push({ path: '/configuration/docker', query: { name: self.common.configurationFile }})
        });
    },
