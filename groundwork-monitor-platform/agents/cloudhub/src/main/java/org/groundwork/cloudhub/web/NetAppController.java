...
import static org.groundwork.cloudhub.web.CloudHubUI.NETAPP_ERROR;

@Controller
@RequestMapping("/netapp")
public class NetAppController extends HostController {

    protected static final String PROFILE_STATE = "profile-state-netapp";
    protected static final String CONFIG_STATE = "config-state-netapp";
    private static Logger log = Logger.getLogger(NetAppController.class);
    ...
    public ModelAndView navigateCreateConnection(HttpServletRequest request, HttpSession session) {
        ...
        NetAppConfiguration configuration = null;

        try {
            configuration = (NetAppConfiguration) configurationService.createConfiguration(VirtualSystem.NETAPP);
            ...
            log.error("Exception occurred navigating create netapp connection", che);
        } catch (Exception ex) {
            log.error("Exception occurred while navigating create netapp connection", ex);
        }
        ...
        return new ModelAndView("netapp/create-connection", "configBean", configuration);
    }

    @RequestMapping(value = "saveConnectionConfiguration", method = RequestMethod.POST)
    public ModelAndView saveConnectionConfiguration(@Valid @ModelAttribute(value = "configBean") NetAppConfiguration configBean, BindingResult bindingResult, HttpServletRequest request, HttpSession session) {
        if (bindingResult.hasErrors()) {
            return new ModelAndView("netapp/create-connection", "configBean", configBean);
        }
        ...
        if (views.size() > 0) {
            deleteViewMetrics(VirtualSystem.NETAPP, views, groupViews, sourceTypes, configBean, getCurrentUser(request));
        }
    ...
    @ResponseBody
    protected String testConnection(@Valid @ModelAttribute(value = "configBean") NetAppConfiguration configuration, HttpServletRequest request,  BindingResult bindingResult, HttpSession session) {
        ...
        try {
            ...
            log.error("Exception occurred while testing netapp connection", vex);
            message = vex.getMessage();
            result = NETAPP_ERROR;
        } catch (CloudHubException che) {
            log.error("Exception occurred while testing netapp connection", che);
            message = che.getMessage();
            result = GWOS_ERROR;
        } catch (Exception ex) {
            log.error("Exception occurred while testing netapp connection", ex);
    ...
    public ModelAndView navigateToProfile(@ModelAttribute(value = "configBean") NetAppConfiguration configBean, HttpServletRequest request) {
        ...
        try {
            ...
            localCloudHubProfile = profileService.readCloudProfile(VirtualSystem.NETAPP, configBean.getCommon().getAgentId());
            remoteCloudHubProfile = profileService.readRemoteCloudProfile(VirtualSystem.NETAPP, configBean.getGwos());
            if (localCloudHubProfile == null && remoteCloudHubProfile == null) {
                cloudHubProfile = profileService.createCloudProfile(VirtualSystem.NETAPP, configBean.getCommon().getAgentId());
                ...
                request.setAttribute(RESULT, result);
                cloudHubProfile = profileService.mergeCloudProfiles(VirtualSystem.NETAPP, remoteCloudHubProfile, localCloudHubProfile);
            }
            ...
            log.error("Exception occurred while navigating netapp profile", e);
        ...
        profileBean.setEnableStorage(configBean.getCommon().isStorageView());
        return new ModelAndView("netapp/assign-thresholds", "profileBean", profileBean);
    }
    ...
    public ModelAndView saveConnectionProfile(@Valid @ModelAttribute(value = "profileBean") DataCenterProfileWrapper profile, BindingResult bindingResult, HttpServletRequest request) {
        ...
        try {
            ...
            if (bindingResult.hasErrors()) {
                ...
                return new ModelAndView("netapp/assign-thresholds", "profileBean", profile);
            }

            CloudHubProfile cloudHubProfile = profileService.readCloudProfile(VirtualSystem.NETAPP, profile.getAgent());
            if (cloudHubProfile == null) {
                cloudHubProfile = profileService.createCloudProfile(VirtualSystem.NETAPP, profile.getAgent());
            }
            cloudHubProfile = profile.mergeToProfile(cloudHubProfile);
            profileService.saveProfile(cloudHubProfile);
            collectorService.setConfigurationUpdated(VirtualSystem.NETAPP);
            ConnectionConfiguration configuration = configurationService.readConfiguration(profile.getConfigFilePath() + "/" + profile.getConfigFileName());
            ...
            log.error("Exception occurred while saving netapp profile", che);
            result = SAVE_FAILURE;
        } catch (Exception ex) {
            log.error("Exception occurred while saving netapp profile", ex);
            result = SAVE_FAILURE;
        }
