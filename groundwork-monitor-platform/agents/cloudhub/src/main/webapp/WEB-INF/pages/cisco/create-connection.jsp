...
<title>Groundwork Net Hub for Cisco ACI</title>
...
<script>
	...
	function testForm(form, action) {
		postTestConnection(form, action, "Cisco");
	}
...
<body id="dt_example">
	<form:form id="createCiscoConnectionForm" commandName="configBean" method="POST" action="/cloudhub/mvc/cisco/saveConnectionConfiguration">
		<div id="container">
			<!-- <p align="center">
				<img align="top" src="images/logo.png">
			</p> -->
			<p align="center" class="agent_title">Net Hub Configuration Wizard for Cisco ACI</p>

            <div id="testConWaitMsg" class='message' style="display:none">
                <p><img src="/cloudhub/resources/images/loader.gif"/>Testing connection to Groundwork and CISCO ACI servers!</p>
            </div>
            <div id='testConResultMsg'>
                <c:if test="${result == 'ciscoerror'}">
                    <div id="messages" class='redMessage'>
                        <p>CISCO ACI server connection failed!</p>
                        <p>${errorMessage}</p>
                    </div>
                </c:if>
                ...
                <c:if test="${result == 'savesuccess'}">
                    <div id="messages" class='message'>
                        <p class='message'>CISCO ACI server connection saved successfully. (Remember to test connection to ensure connection validity)</p>
                    </div>
                </c:if>
                <c:if test="${result == 'savefailure'}">
                    <div id="messages" class='redMessage'>
                        <p class='message'>Sorry some problem occured in saving CISCO ACI server connection.!</p>
                        <p>${errorMessage}</p>
                    </div>
                </c:if>
			...
			<div id="controlbg">
				...
				<div class="controlheader">Cisco ACI / Groundwork Connection.
				...
				<div class="controlcontent">
					...
					<table cellpadding="0" cellspacing="0" border="0" class="display"
						...
						<tr>
							<td class="group2">Is SSL enabled on Cisco ACI Server?</td>
							<td class="group2">
						...
						<tr>
							<td class="group2">Cisco ACI Server URL:</td>
							...
							<td class="group2">http://cisco-host.yourdomain.com</td>
						</tr>

                        <tr>
							<td class="group2">Cisco ACI Server Username:</td>
							<td class="group2"><form:input type="text" class="text required"
						...
						<tr>
							<td class="group2">Cisco ACI Server Password:</td>
					...
					<p align="center">
						...
						<c:if test="${configBean.common.testConnectionDisabled == false}">
							<input id="testConnectionBtn" value="Test Connection" class="button-l" type="button" onclick="testForm('createCiscoConnectionForm', '/cloudhub/mvc/cisco/testConnection')"/>
						</c:if>
						...
						<input name="save" id='saveButton' value="Save" class="button-l" type="button" onclick="saveForm('createCiscoConnectionForm', '/cloudhub/mvc/cisco/saveConnectionConfiguration')"/>
