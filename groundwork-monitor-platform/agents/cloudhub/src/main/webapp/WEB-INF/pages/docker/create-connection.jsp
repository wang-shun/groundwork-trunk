...
<title>Groundwork Cloud Hub for Docker</title>
...
<script>
	...
	function testForm(form, action) {
		postTestConnection(form, action, "Docker");
	}
...
<body id="dt_example">
	<form:form id="createDockerConnectionForm" commandName="configBean" method="POST" action="/cloudhub/mvc/docker2/saveConnectionConfiguration">
		<div id="container">
			<!-- <p align="center">
            ...
            <div id="testConWaitMsg" class='message' style="display:none">
                <p><img src="/cloudhub/resources/images/loader.gif"/>Testing connection to Groundwork and Docker servers!</p>
            </div>
            <div id='testConResultMsg'>
                <c:if test="${result == 'dockererror'}">
                    <div id="messages" class='redMessage'>
                        <p>Docker server connection failed!</p>
                        <p>${errorMessage}</p>
                    </div>
                </c:if>
                ...
                <c:if test="${result == 'savesuccess'}">
                    <div id="messages" class='message'>
                        <p class='message'>Docker server connection saved successfully. (Remember to test connection to ensure connection validity)</p>
                    </div>
                </c:if>
                <c:if test="${result == 'savefailure'}">
                    <div id="messages" class='redMessage'>
                        <p class='message'>Sorry some problem occurred in saving Docker server connection.!</p>
                        <p>${errorMessage}</p>
                    </div>
                </c:if>
			...
			<div id="controlbg">
				...
				<div class="controlheader">Docker / Groundwork Connection.
				...
				<div class="controlcontent">
					...
					<table cellpadding="0" cellspacing="0" border="0" class="display"
						...
						<tr>
							<td class="group2">Docker Server URL:</td>
							...
							<td class="group2">docker-host.yourdomain.com:port</td>
						</tr>

                        <tr>
                            <td class="group2">Host Prefix:</td>
                            <td class="group2"><form:input type="text" class="text"
                                                           id="virtualEnv.prefix" name="virtualEnv.prefix"
                                                           maxlength="16" path="connection.prefix" /></td>
                            <td class="group2"><font color='red'><form:errors path='connection.prefix' /></font></td>
                            <td class="group2">docker1-</td>
                        </tr>
					...
					<p align="center">
						...
						<c:if test="${configBean.common.testConnectionDisabled == false}">
							<input id="testConnectionBtn" value="Test Connection" class="button-l" type="button" onclick="testForm('createDockerConnectionForm', '/cloudhub/mvc/docker2/testConnection')"/>
						</c:if>
						...
						<input name="save" id='saveButton' value="Save" class="button-l" type="button" onclick="saveForm('createDockerConnectionForm', '/cloudhub/mvc/docker2/saveConnectionConfiguration')"/>

                        <c:if test="${configBean.common.createProfileDisabled == false}">
                            <input id="next" value="Next" class="button-l" type="button" onclick="profileForm('createDockerConnectionForm', '/cloudhub/mvc/docker2/navigateToProfile')"/>
                        </c:if>
                        <c:if test="${configBean.common.createProfileDisabled == true}">
                            <input id="next" alt="You need to have a valid connection to proceed to the next page." name="next" value="Next" class="button-l" type="button" disabled="disabled"
								   onclick="profileForm('createDockerConnectionForm', '/cloudhub/mvc/docker2/navigateToProfile')" />
                        </c:if>
