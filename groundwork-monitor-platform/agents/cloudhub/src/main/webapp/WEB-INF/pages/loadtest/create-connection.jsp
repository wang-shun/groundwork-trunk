...
<title>Groundwork Cloud Hub for LoadTest</title>
...
<script>
	...
	function testForm(form, action) {
		postTestConnection(form, action, "LoadTest");
	}
...
<body id="dt_example">
	<form:form id="createLoadTestConnectionForm" commandName="configBean" method="POST" action="/cloudhub/mvc/loadtest/saveConnectionConfiguration">
		<div id="container">
			<!-- <p align="center">
				<img align="top" src="images/logo.png">
			</p> -->
			<p align="center" class="agent_title">Cloud Hub Configuration Wizard for LoadTest</p>

            <div id="testConWaitMsg" class='message' style="display:none">
                <p><img src="/cloudhub/resources/images/loader.gif"/>Testing connection to Groundwork and LoadTest servers!</p>
            </div>
            <div id='testConResultMsg'>
                <c:if test="${result == 'loadtesterror'}">
                    <div id="messages" class='redMessage'>
                        <p>LoadTest server connection failed!</p>
                        <p>${errorMessage}</p>
                    </div>
                </c:if>
                ...
                <c:if test="${result == 'savesuccess'}">
                    <div id="messages" class='message'>
                        <p class='message'>LoadTest server connection saved successfully. (Remember to test connection to ensure connection validity)</p>
                    </div>
                </c:if>
                <c:if test="${result == 'savefailure'}">
                    <div id="messages" class='redMessage'>
                        <p class='message'>Sorry some problem occurred in saving LoadTest server connection.!</p>
                        <p>${errorMessage}</p>
                    </div>
                </c:if>
			...
			<div id="controlbg">
				...
				<div class="controlheader">LoadTest / Groundwork Connection
				...
				<div class="controlcontent">
					...
					<table cellpadding="0" cellspacing="0" border="0" class="display"
						...
						<tr>
							<td class="group2">Number of Hosts, (virtual machines):</td>
							<td class="group2">
                                <form:input type="text" class="text required number"
                                            id="virtualEnv.hosts" name="virtualEnv.hosts" maxlength="5"
                                            path="connection.hosts"/></td>
							</td>
							<td class="group2"><font color='red'><form:errors path='connection.hosts' /></font></td>
							<td class="group2"></td>
						</tr>
						<tr>
							<td class="group2">Number of Host Groups, (hypervisors):</td>
							<td class="group2">
                                <form:input type="text" class="text required number"
                                            id="virtualEnv.hostGroups" name="virtualEnv.hostGroups" maxlength="5"
                                            path="connection.hostGroups"/></td>
							</td>
							<td class="group2"><font color='red'><form:errors path='connection.hostGroups' /></font></td>
							<td class="group2"></td>
						</tr>
                        <tr>
                            <td class="group2">Number of Services, (attributes):</td>
                            <td class="group2">
                                <form:input type="text" class="text required number"
                                            id="virtualEnv.services" name="virtualEnv.services" maxlength="5"
                                            path="connection.services"/></td>
                            </td>
                            <td class="group2"><font color='red'><form:errors path='connection.services' /></font></td>
                            <td class="group2"></td>
                        </tr>
						<tr>
							<td class="group2">Hosts Down, (percent):</td>
							<td class="group2">
                                <form:input type="text" class="text required number"
                                            id="virtualEnv.hostsDownPercent" name="virtualEnv.hostsDownPercent" maxlength="3"
                                            path="connection.hostsDownPercent"/></td>
							</td>
							<td class="group2"><font color='red'><form:errors path='connection.hostsDownPercent' /></font></td>
							<td class="group2"></td>
						</tr>
						<tr>
							<td class="group2">Services Critical, (percent):</td>
							<td class="group2">
                                <form:input type="text" class="text required number"
                                            id="virtualEnv.servicesCriticalPercent" name="virtualEnv.servicesCriticalPercent" maxlength="3"
                                            path="connection.servicesCriticalPercent"/></td>
							</td>
							<td class="group2"><font color='red'><form:errors path='connection.servicesCriticalPercent' /></font></td>
							<td class="group2"></td>
						</tr>
                        ...
                        <tr>
                            <td class="group2">Application Type:</td>
                            <td class="group2"><form:input type="text" class="text required"
                                                           maxlength="10" path="common.applicationType"
                                                           style="text-transform: uppercase" /> </td>
                            <td class="group2"><font color='red'><form:errors path='common.applicationType' /></font></td>
                            <td class="group2">${defaultApplicationType}</td>
                        </tr>
					</table>
                    <br />
					<p align="center">
						...
						<c:if test="${configBean.common.testConnectionDisabled == false}">
							<input id="testConnectionBtn" value="Test Connection" class="button-l" type="button" onclick="testForm('createLoadTestConnectionForm', '/cloudhub/mvc/loadtest/testConnection')"/>
						</c:if>
						...
						<input name="save" id='saveButton' value="Save" class="button-l" type="button" onclick="saveForm('createLoadTestConnectionForm', '/cloudhub/mvc/loadtest/saveConnectionConfiguration')"/>


						<c:if test="${configBean.common.createProfileDisabled == false}">
							<input id="next" value="Next" class="button-l" type="button" onclick="profileForm('createLoadTestConnectionForm', '/cloudhub/mvc/loadtest/navigateToProfile')"/>
						</c:if>
						<c:if test="${configBean.common.createProfileDisabled == true}">
							<input id="next" alt="You need to have a valid connection to proceed to the next page." name="next" value="Next" class="button-l" type="submit" disabled="disabled"
								   onclick="profileForm('createLoadTestConnectionForm', '/cloudhub/mvc/loadtest/navigateToProfile')" />
						</c:if>
					</p>
