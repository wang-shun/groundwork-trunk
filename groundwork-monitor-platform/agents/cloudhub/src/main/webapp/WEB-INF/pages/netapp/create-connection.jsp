...
<title>Groundwork Cloud Hub for NetApp</title>
...
<script>
	...
	function testForm(form, action) {
		postTestConnection(form, action, "NetApp");
	}
...
<body id="dt_example">
	<form:form id="createNetAppConnectionForm" commandName="configBean" method="POST" action="/cloudhub/mvc/netapp/saveConnectionConfiguration">
		<div id="container">
			<!-- <p align="center">
				<img align="top" src="images/logo.png">
			</p> -->
			<p align="center" class="agent_title">Cloud Hub Configuration Wizard for NetApp</p>

            <div id="testConWaitMsg" class='message' style="display:none">
                <p><img src="/cloudhub/resources/images/loader.gif"/>Testing connection to Groundwork and NetApp servers!</p>
            </div>
            <div id='testConResultMsg'>
                <c:if test="${result == 'netapperror'}">
                    <div id="messages" class='redMessage'>
                        <p>NetApp server connection failed!</p>
                        <p>${errorMessage}</p>
                    </div>
                </c:if>
                ...
                <c:if test="${result == 'savesuccess'}">
                    <div id="messages" class='message'>
                        <p class='message'>NetApp server connection saved successfully. (Remember to test connection to ensure connection validity)</p>
                    </div>
                </c:if>
                <c:if test="${result == 'savefailure'}">
                    <div id="messages" class='redMessage'>
                        <p class='message'>Sorry some problem occurred in saving NetApp server connection.!</p>
                        <p>${errorMessage}</p>
                    </div>
                </c:if>
			...
			<div id="controlbg">
				...
				<div class="controlheader">NetApp / Groundwork Connection
				...
				<div class="controlcontent">
					...
					<table cellpadding="0" cellspacing="0" border="0" class="display"
						...
						<tr>
							<td class="group2">Is SSL enabled on NetApp Server?</td>
							<td class="group2">
						...
						<tr>
							<td class="group2">NetApp Server Name:</td>
							...
							<td class="group2">NetApp-server.yourdomain.com</td>
						</tr>

						<tr>
							<td class="group2">NetApp Server Username:</td>
							<td class="group2"><form:input type="text" class="text required"
						...
						<tr>
							<td class="group2">NetApp Server Password:</td>
					...
					<p align="center">
						...
						<c:if test="${configBean.common.testConnectionDisabled == false}">
							<input id="testConnectionBtn" value="Test Connection" class="button-l" type="button" onclick="testForm('createNetAppConnectionForm', '/cloudhub/mvc/netapp/testConnection')"/>
						</c:if>
						...
						<input name="save" id='saveButton' value="Save" class="button-l" type="button" onclick="saveForm('createNetAppConnectionForm', '/cloudhub/mvc/netapp/saveConnectionConfiguration')"/>


						<c:if test="${configBean.common.createProfileDisabled == false}">
							<input id="next" value="Next" class="button-l" type="button" onclick="profileForm('createNetAppConnectionForm', '/cloudhub/mvc/netapp/navigateToProfile')"/>
						</c:if>
						<c:if test="${configBean.common.createProfileDisabled == true}">
							<input id="next" alt="You need to have a valid connection to proceed to the next page." name="next" value="Next" class="button-l" type="submit" disabled="disabled"
								   onclick="profileForm('createNetAppConnectionForm', '/cloudhub/mvc/netapp/navigateToProfile')" />
						</c:if>
					</p>
