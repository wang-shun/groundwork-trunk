...
<title>Groundwork Net Hub for Open Daylight</title>
...
<script>
	...
	function testForm(form, action) {
		postTestConnection(form, action, "OpenDayLight");
	}
...
<body id="dt_example">
	<form:form id="createOpenDaylightConnectionForm" commandName="configBean" method="POST" action="/cloudhub/mvc/opendaylight/saveConnectionConfiguration">
		<div id="container">
			<!-- <p align="center">
				<img align="top" src="images/logo.png">
			</p> -->
			<p align="center" class="agent_title">Net Hub Configuration Wizard for Open Daylight</p>

            <div id="testConWaitMsg" class='message' style="display:none">
                <p><img src="/cloudhub/resources/images/loader.gif"/>Testing connection to Groundwork and Open Daylight servers!</p>
            </div>
            <div id='testConResultMsg'>
                <c:if test="${result == 'opendaylighterror'}">
                    <div id="messages" class='redMessage'>
                        <p>Open Daylight server connection failed!</p>
                        <p>${errorMessage}</p>
                    </div>
                </c:if>
                ...
                <c:if test="${result == 'savesuccess'}">
                    <div id="messages" class='message'>
                        <p class='message'>Open Daylight server connection saved successfully. (Remember to test connection to ensure connection validity)</p>
                    </div>
                </c:if>
                <c:if test="${result == 'savefailure'}">
                    <div id="messages" class='redMessage'>
                        <p class='message'>Sorry some problem occurred in saving Open Daylight server connection.!</p>
                        <p>${errorMessage}</p>
                    </div>
                </c:if>
			...
			<div id="controlbg">
				...
				<div class="controlheader">Open Daylight / Groundwork Connection.
				...
				<div class="controlcontent">
					...
					<table cellpadding="0" cellspacing="0" border="0" class="display"
						...
						<tr>
							...
							<td class="group1">Configuration Display Name</td>
						</tr>
						<tr>
						...
						<tr>
							<td class="group2">Is SSL enabled on Open Daylight Server?</td>
							<td class="group2">
						...
						<tr>
							<td class="group2">Open Daylight Server URL:</td>
							...
							<td class="group2">opendaylight-host.yourdomain.com [:optional port]</td>
						</tr>

                        <tr>
							<td class="group2">Open Daylight Server Username:</td>
							<td class="group2"><form:input type="text" class="text required"
						...
						<tr>
							<td class="group2">Open Daylight Server Password:</td>
					...
					<p align="center">
						...
						<c:if test="${configBean.common.testConnectionDisabled == false}">
							<input id="testConnectionBtn" value="Test Connection" class="button-l" type="button" onclick="testForm('createOpenDaylightConnectionForm', '/cloudhub/mvc/opendaylight/testConnection')"/>
						</c:if>
						...
						<input name="save" id='saveButton' value="Save" class="button-l" type="button" onclick="saveForm('createOpenDaylightConnectionForm', '/cloudhub/mvc/opendaylight/saveConnectionConfiguration')"/>

                        <c:if test="${configBean.common.createProfileDisabled == false}">
                            <input id="next" value="Next" class="button-l" type="button" onclick="profileForm('createOpenDaylightConnectionForm', '/cloudhub/mvc/opendaylight/navigateToProfile')"/>
                        </c:if>
                        <c:if test="${configBean.common.createProfileDisabled == true}">
                            <input id="next" alt="You need to have a valid connection to proceed to the next page." name="next" value="Next" class="button-l" type="button" disabled="disabled"
								   onclick="profileForm('createOpenDaylightConnectionForm', '/cloudhub/mvc/opendaylight/navigateToProfile')"/>
                        </c:if>
