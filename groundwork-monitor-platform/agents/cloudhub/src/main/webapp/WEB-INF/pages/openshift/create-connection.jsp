...
<title>Groundwork Cloud Hub for :OpenShift</title>
...
<script>
	...
	function testForm(form, action) {
		postTestConnection(form, action, "OpenShift");
	}
...
<body id="dt_example">
	<form:form id="createOpenShiftConnectionForm" commandName="configBean" method="POST" action="/cloudhub/mvc/openshift/saveConnectionConfiguration">
		<div id="container">
			<!-- <p align="center">
				<img align="top" src="images/logo.png">
			</p> -->
			<p align="center" class="agent_title">Cloud Hub Configuration Wizard for OpenShift</p>

            <div id="testConWaitMsg" class='message' style="display:none">
                <p><img src="/cloudhub/resources/images/loader.gif"/>Testing connection to Groundwork and OpenSHIFT servers!</p>
            </div>
            <div id='testConResultMsg'>
                <c:if test="${result == 'openshifterror'}">
                    <div id="messages" class='redMessage'>
                        <p>OpenSHIFT server connection failed!</p>
                        <p>${errorMessage}</p>
                    </div>
                </c:if>
                ...
                <c:if test="${result == 'savesuccess'}">
                    <div id="messages" class='message'>
                        <p class='message'>OpenSHIFT server connection saved successfully. (Remember to test connection to ensure connection validity)</p>
                    </div>
                </c:if>
                <c:if test="${result == 'savefailure'}">
                    <div id="messages" class='redMessage'>
                        <p class='message'>Sorry some problem occurred in saving OpenSHIFT server connection.!</p>
                        <p>${errorMessage}</p>
                    </div>
                </c:if>
			...
			<div id="controlbg">
				...
				<div class="controlheader">OpenShift / Groundwork Connection
				...
				<div class="controlcontent">
					...
					<table cellpadding="0" cellspacing="0" border="0" class="display"
						...
						<tr>
							<td class="group2">Is SSL enabled on OpenShift Server?</td>
							<td class="group2">
						...
						<tr>
							<td class="group2">:OpenShift Server URL:</td>
							...
							<td class="group2">http://openshift.yourdomain.com</td>
						</tr>

						<tr>
							<td class="group2">:OpenShift Tenant ID:</td>
							<td class="group2"><form:input type="text" class="text required"
                            ...
                            <td class="group2">OpenShift Tenant Resource ID</td>
						</tr>

                        <tr>
                            <td class="group2">:OpenShift Tenant Name:</td>
                            <td class="group2"><form:input type="text" class="text required"
                                                           id="virtualEnv.url" name="virtualEnv.url" maxlength="256"
                                                           path="connection.url" /></td>
                            <td class="group2"><font color='red'><form:errors path='connection.url' /></font></td>
                            <td class="group2">OpenShift Tenant Name</td>
                        </tr>

                        <tr>
							<td class="group2">:OpenShift Server Username:</td>
							<td class="group2"><form:input type="text" class="text required"
						...
						<tr>
							<td class="group2">:OpenShift Server Password:</td>
					...
					<p align="center">
						...
						<c:if test="${configBean.common.testConnectionDisabled == false}">
							<input id="testConnectionBtn" value="Test Connection" class="button-l" type="button" onclick="testForm('createOpenShiftConnectionForm', '/cloudhub/mvc/openshift/testConnection')"/>
						</c:if>
						...
						<input name="save" id='saveButton' value="Save" class="button-l" type="button" onclick="saveForm('createOpenShiftConnectionForm', '/cloudhub/mvc/openshift/saveConnectionConfiguration')"/>
