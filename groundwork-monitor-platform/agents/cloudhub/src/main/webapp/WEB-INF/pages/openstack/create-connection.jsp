...
<title>Groundwork Cloud Hub for OpenStack</title>
...
<script>
	...
	function testForm(form, action) {
		postTestConnection(form, action, "OpenStack");
	}
...
<body id="dt_example">
	<form:form id="createOpenStackConnectionForm" commandName="configBean" method="POST" action="/cloudhub/mvc/openstack/saveConnectionConfiguration">
		<div id="container">
			<!-- <p align="center">
				<img align="top" src="images/logo.png">
			</p> -->
			<p align="center" class="agent_title">Cloud Hub Configuration Wizard for OpenStack</p>

            <div id="testConWaitMsg" class='message' style="display:none">
                <p><img src="/cloudhub/resources/images/loader.gif"/>Testing connection to Groundwork and OpenStack servers!</p>
            </div>
            <div id='testConResultMsg'>
                <c:if test="${result == 'openstackerror'}">
                    <div id="messages" class='redMessage'>
                        <p>OpenStack server connection failed!</p>
                        <p>${errorMessage}</p>
                    </div>
                </c:if>
                ...
                <c:if test="${result == 'savesuccess'}">
                    <div id="messages" class='message'>
                        <p class='message'>OpenStack server connection saved successfully. (Remember to test connection to ensure connection validity)</p>
                    </div>
                </c:if>
                <c:if test="${result == 'savefailure'}">
                    <div id="messages" class='redMessage'>
                        <p class='message'>Sorry some problem occurred in saving OpenStack server connection.!</p>
                        <p>${errorMessage}</p>
                    </div>
                </c:if>
			...
			<div id="controlbg">
				...
				<div class="controlheader">OpenStack / Groundwork Connection
					You need to have a valid connection to proceed to the next page.</div>
				<div class="controlcontent">
					...
					<table cellpadding="0" cellspacing="0" border="0" class="display"
						...
						<tr>
							<td class="group2">Is SSL enabled on OpenStack Server ?:</td>
							<td class="group2">
						...
						<tr>
							<td class="group2">OpenStack Server URL:</td>
							...
							<td class="group2">openstack.yourdomain.com</td>
						</tr>

						<tr>
							<td class="group2">OpenStack Tenant ID:</td>
							<td class="group2"><form:input type="text" class="text required"
								id="virtualEnv.tenantId" name="virtualEnv.tenantId" maxlength="256"
								path="connection.tenantId" /></td>
							<td class="group2"><font color='red'><form:errors path='connection.tenantId' /></font></td>
                            <td class="group2">an OpenStack Tenant Resource ID</td>
						</tr>

                        <tr>
                            <td class="group2">OpenStack Tenant Name:</td>
                            <td class="group2"><form:input type="text" class="text required"
                                                           id="virtualEnv.tenantName" name="virtualEnv.tenantName" maxlength="256"
                                                           path="connection.tenantName" /></td>
                            <td class="group2"><font color='red'><form:errors path='connection.tenantName' /></font></td>
                            <td class="group2">an OpenStack Tenant Name</td>
                        </tr>

                        <tr>
							<td class="group2">OpenStack Server Username:</td>
							<td class="group2"><form:input type="text" class="text required"
						...
						<tr>
							<td class="group2">OpenStack Server Password:</td>
                        ...
                        <tr>
                            <td class="group2">Nova Port Number:</td>
                            <td class="group2"><form:input type="text" class="text required number"
                                                           id="virtualEnv.novaPort" name="virtualEnv.novaPort" maxlength="9"
                                                           path="connection.novaPort" /></td>
                            <td class="group2"><font color='red'><form:errors path='connection.novaPort' /></font></td>
                            <td class="group2"></td>
                        </tr>

                        <tr>
                            <td class="group2">Keystone Port Number:</td>
                            <td class="group2"><form:input type="text" class="text required number"
                                                           id="virtualEnv.keystonePort" name="virtualEnv.keystonePort" maxlength="9"
                                                           path="connection.keystonePort" /></td>
                            <td class="group2"><font color='red'><form:errors path='connection.keystonePort' /></font></td>
                            <td class="group2"></td>
                        </tr>

                        <tr>
                            <td class="group2">Ceilometer Port Number:</td>
                            <td class="group2"><form:input type="text" class="text required number"
                                                           id="virtualEnv.ceilometerPort" name="virtualEnv.ceilometerPort" maxlength="9"
                                                           path="connection.ceilometerPort" /></td>
                            <td class="group2"><font color='red'><form:errors path='connection.ceilometerPort' /></font></td>
                            <td class="group2"></td>
                        </tr>
					...
					<p align="center">
						...
						<c:if test="${configBean.common.testConnectionDisabled == false}">
							<input id="testConnectionBtn" value="Test Connection" class="button-l" type="button" onclick="testForm('createOpenStackConnectionForm', '/cloudhub/mvc/openstack/testConnection')"/>
						</c:if>
						...
						<input name="save" id='saveButton' value="Save" class="button-l" type="button" onclick="saveForm('createOpenStackConnectionForm', '/cloudhub/mvc/openstack/saveConnectionConfiguration')"/>

                        <c:if test="${configBean.common.createProfileDisabled == false}">
                            <input id="next" value="Next" class="button-l" type="button" onclick="profileForm('createOpenStackConnectionForm', '/cloudhub/mvc/openstack/navigateToProfile')"/>
                        </c:if>
                        <c:if test="${configBean.common.createProfileDisabled == true}">
                            <input id="next" alt="You need to have a valid connection to proceed to the next page." name="next" value="Next" class="button-l" type="button" disabled="disabled"
								   onclick="profileForm('createOpenStackConnectionForm', '/cloudhub/mvc/openstack/navigateToProfile')" />
                        </c:if>
