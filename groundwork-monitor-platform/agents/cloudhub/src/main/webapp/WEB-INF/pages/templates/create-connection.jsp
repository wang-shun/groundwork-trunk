...
<title>Groundwork Cloud Hub for $ConnectorName</title>
...
<script>
	...
	function testForm(form, action) {
		postTestConnection(form, action, "$ConnectorName");
	}
...
<body id="dt_example">
	<form:form id="create$ConnectorNameConnectionForm" commandName="configBean" method="POST" action="/cloudhub/mvc/$connector/saveConnectionConfiguration">
		<div id="container">
			<!-- <p align="center">
				<img align="top" src="images/logo.png">
			</p> -->
			<p align="center" class="agent_title">Cloud Hub Configuration Wizard for $ConnectorName</p>

            <div id="testConWaitMsg" class='message' style="display:none">
                <p><img src="/cloudhub/resources/images/loader.gif"/>Testing connection to Groundwork and $ConnectorName servers!</p>
            </div>
            <div id='testConResultMsg'>
                <c:if test="${result == '$connectorerror'}">
                    <div id="messages" class='redMessage'>
                        <p>$ConnectorName server connection failed!</p>
                        <p>${errorMessage}</p>
                    </div>
                </c:if>
                ...
                <c:if test="${result == 'savesuccess'}">
                    <div id="messages" class='message'>
                        <p class='message'>$ConnectorName server connection saved successfully. (Remember to test connection to ensure connection validity)</p>
                    </div>
                </c:if>
                <c:if test="${result == 'savefailure'}">
                    <div id="messages" class='redMessage'>
                        <p class='message'>Sorry some problem occurred in saving $ConnectorName server connection.!</p>
                        <p>${errorMessage}</p>
                    </div>
                </c:if>
			...
			<div id="controlbg">
				...
				<div class="controlheader">$ConnectorName / Groundwork Connection
				...
				<div class="controlcontent">
					...
					<table cellpadding="0" cellspacing="0" border="0" class="display"
						...
						<tr>
							<td class="group2">Is SSL enabled on $ConnectorName Server?</td>
							<td class="group2">
						...
						<tr>
							<td class="group2">$ConnectorName Server Name:</td>
							...
							<td class="group2">$ConnectorName-server.yourdomain.com</td>
						</tr>

						<tr>
							<td class="group2">$ConnectorName Server URI:</td>
							<td class="group2"><form:input type="text" class="text required"
								id="virtualEnv.uri" name="virtualEnv.uri" maxlength="256"
								path="connection.uri" /></td>
							<td class="group2"><font color='red'><form:errors path='connection.uri' /></font></td>
							<td class="group2"> sdk </td>
						</tr>

						<tr>
							<td class="group2">$ConnectorName Server Username:</td>
							<td class="group2"><form:input type="text" class="text required"
						...
						<tr>
							<td class="group2">$ConnectorName Server Password:</td>
					...
					<p align="center">
						...
						<c:if test="${configBean.common.testConnectionDisabled == false}">
							<input id="testConnectionBtn" value="Test Connection" class="button-l" type="button" onclick="testForm('create$ConnectorNameConnectionForm', '/cloudhub/mvc/$connector/testConnection')"/>
						</c:if>
						...
						<input name="save" id='saveButton' value="Save" class="button-l" type="button" onclick="saveForm('create$ConnectorNameConnectionForm', '/cloudhub/mvc/$connector/saveConnectionConfiguration')"/>


						<c:if test="${configBean.common.createProfileDisabled == false}">
							<input id="next" value="Next" class="button-l" type="button" onclick="profileForm('create$ConnectorNameConnectionForm', '/cloudhub/mvc/$connector/navigateToProfile')"/>
						</c:if>
						<c:if test="${configBean.common.createProfileDisabled == true}">
							<input id="next" alt="You need to have a valid connection to proceed to the next page." name="next" value="Next" class="button-l" type="submit" disabled="disabled"
								   onclick="profileForm('create$ConnectorNameConnectionForm', '/cloudhub/mvc/$connector/navigateToProfile')" />
						</c:if>
					</p>
