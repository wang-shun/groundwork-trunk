...
<title>CloudHub for VMware</title>
...
<body id="dt_example">
	<form:form name="assignForm" id="assignForm" modelAttribute="profileBean" method="POST" action="/cloudhub/mvc/vmware2/saveConnectionProfile">
			...
			<div id="messages" class="message">
				<div id="testConResultMsg">
					...
					<c:if test="${result == 'success'}">
						<p><font size="3">VMware profile updated successfully.</font></p>
					</c:if>
	
					<c:if test="${result == 'savefailure'}">
						<p><font size="3" color="red">Sorry some problem occurred in saving VMware profile!</font></p>
					</c:if>

                    <c:if test="${result == 'readfailure'}">
                        <p><font size="3" color="red">Sorry some problem occurred in reading VMware profile</font></p>
                    </c:if>
							...
							<c:forEach items="${profileBean.hostMetrics}" var="metric"
								...
								<tr>
                                    ...
                                    <td class="tableRow6">
											...
											<form:hidden path="hostMetrics[${rowCounter.index}].sourceType" value="${metric.sourceType}"/>
											<form:hidden path="hostMetrics[${rowCounter.index}].computeType" value="${metric.computeType}"/>
											<form:hidden path="hostMetrics[${rowCounter.index}].expression" value="${metric.expression}"/>
											<form:hidden path="hostMetrics[${rowCounter.index}].format" value="${metric.format}"/>
                                    </td>
								</tr>
							</c:forEach>
						</tbody>
							...
							<c:forEach items="${profileBean.instanceMetrics}" var="metric" varStatus="rowCounter">
								<tr>
                                    ...
                                    <td class="tableRow6">
                                        ...
                                        <form:hidden path="instanceMetrics[${rowCounter.index}].expression" value="${metric.expression}"/>
                                        <form:hidden path="instanceMetrics[${rowCounter.index}].format" value="${metric.format}"/>
                                    </td>
								</tr>
							</c:forEach>
						</tbody>
                    ...
                    <table cellpadding="0" cellspacing="0" border="0" class="display" style="width: auto;">
                        ...
                        <c:forEach items="${profileBean.storageMetrics}" var="metric" varStatus="rowCounter">
                            <tr>
                                ...
                                <td class="tableRow2 center">
                                    <c:choose>
                                    <c:when test="${profileBean.enableStorage == false}">
                                        <form:checkbox disabled="true" id="storage_monitored_${rowCounter.count}" name="storage_monitored" path="storageMetrics[${rowCounter.index}].monitored" onchange="vmMonitorOnChangeCheckbox(this, 'storage')"/>
                                        <form:checkbox hidden="true" id="storage_monitored_${rowCounter.count}" name="storage_monitored" path="storageMetrics[${rowCounter.index}].monitored" onchange="vmMonitorOnChangeCheckbox(this, 'storage')"/>
                                    </c:when>
                                    <c:otherwise>
                                        <form:checkbox id="storage_monitored_${rowCounter.count}" name="storage_monitored" path="storageMetrics[${rowCounter.index}].monitored" onchange="vmMonitorOnChangeCheckbox(this, 'storage')"/>
                                    </c:otherwise>
                                    </c:choose>
                                </td>
                                ...
                                <td class="tableRow3 center">
                                    <c:choose>
                                    <c:when test="${profileBean.enableStorage == false}">
                                        <form:checkbox disabled="true" id="storage_graphed_${rowCounter.count}" name="storage_graphed" path="storageMetrics[${rowCounter.index}].graphed" onchange="vmGraphOnChangeCheckbox(this, 'storage')"/>
                                        <form:checkbox hidden="true" id="storage_graphed_${rowCounter.count}" name="storage_graphed" path="storageMetrics[${rowCounter.index}].graphed" onchange="vmGraphOnChangeCheckbox(this, 'storage')"/>
                                    </c:when>
                                    <c:otherwise>
                                        <form:checkbox id="storage_graphed_${rowCounter.count}" name="storage_graphed" path="storageMetrics[${rowCounter.index}].graphed" onchange="vmGraphOnChangeCheckbox(this, 'storage')"/>
                                    </c:otherwise>
                                    </c:choose>
                                </td>
                                ...
                                <td class="tableRow4">
                                    <form:input type="text"
                                                readonly="${profileBean.enableStorage == false}"
                                ...
                                <td class="tableRow6">
									...
									<form:hidden path="storageMetrics[${rowCounter.index}].expression" value="${metric.expression}"/>
									<form:hidden path="storageMetrics[${rowCounter.index}].format" value="${metric.format}"/>
                                </td>
                            </tr>
                        </c:forEach>
                        </tbody>
