...
<title>Groundwork Cloud Hub for VMware</title>
...
<script>
	...
	function testForm(form, action) {
		postTestConnection(form, action, "VMWare");
	}
...
<body id="dt_example">
	<form:form id="createVMwareConnectionForm" commandName="configBean" method="POST" action="/cloudhub/mvc/vmware2/saveConnectionConfiguration">
		<div id="container">
			<!-- <p align="center">
				<img align="top" src="images/logo.png">
			</p> -->
			<p align="center" class="agent_title">Cloud Hub Configuration Wizard for VMware</p>

            <div id="testConWaitMsg" class='message' style="display:none">
                <p><img src="/cloudhub/resources/images/loader.gif"/>Testing connection to Groundwork and VMWare servers!</p>
            </div>
            <div id='testConResultMsg'>
                <c:if test="${result == 'vmwareerror'}">
                    <div id="messages" class='redMessage'>
                        <p>VMWare server connection failed!</p>
                        <p>${errorMessage}</p>
                    </div>
                </c:if>
                ...
                <c:if test="${result == 'savesuccess'}">
                    <div id="messages" class='message'>
                        <p class='message'>VMware server connection saved successfully. (Remember to test connection to ensure connection validity)</p>
                    </div>
                </c:if>
                <c:if test="${result == 'savefailure'}">
                    <div id="messages" class='redMessage'>
                        <p class='message'>Sorry some problem occurred in saving VMWare server connection.!</p>
                        <p>${errorMessage}</p>
                    </div>
                </c:if>
			...
			<div id="controlbg">
				...
				<div class="controlcontent">
					...
					<table cellpadding="0" cellspacing="0" border="0" class="display"
						...
						<tr>
							<td class="group2">Is SSL enabled on VMware Server?</td>
							<td class="group2">
						...
						<tr>
							<td class="group2">VMware Server Name:</td>
							...
							<td class="group2">VMware-server.yourdomain.com</td>
						</tr>

						<tr>
							<td class="group2">VMware Server URI:</td>
							<td class="group2"><form:input type="text" class="text required"
						...
						<tr>
							<td class="group2">VMware Server Username:</td>
							<td class="group2"><form:input type="text" class="text required"
						...
						<tr>
							<td class="group2">VMware Server Password:</td>
					...
					<p align="center">
						...
						<c:if test="${configBean.common.testConnectionDisabled == false}">
							<input id="testConnectionBtn" value="Test Connection" class="button-l" type="button" onclick="testForm('createVMwareConnectionForm', '/cloudhub/mvc/vmware2/testConnection')"/>
						</c:if>
						...
						<input name="save" id='saveButton' value="Save" class="button-l" type="button" onclick="saveForm('createVMwareConnectionForm', '/cloudhub/mvc/vmware2/saveConnectionConfiguration')"/>


						<c:if test="${configBean.common.createProfileDisabled == false}">
							<input id="next" value="Next" class="button-l" type="button" onclick="profileForm('createVMwareConnectionForm', '/cloudhub/mvc/vmware2/navigateToProfile')"/>
						</c:if>
						<c:if test="${configBean.common.createProfileDisabled == true}">
							<input id="next" alt="You need to have a valid connection to proceed to the next page." name="next" value="Next" class="button-l" type="submit" disabled="disabled"
								   onclick="profileForm('createVMwareConnectionForm', '/cloudhub/mvc/vmware2/navigateToProfile')" />
						</c:if>
					</p>
