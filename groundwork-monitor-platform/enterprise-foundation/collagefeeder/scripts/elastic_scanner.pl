...
my (
    ...
    @elasticsearch_nodes,  $initially_process_everything, $origin,                $health_service,
    $app_type,
) = undef;
...
sub send_events_to_elasticsearch {
    ...
    # $elastic_search_object = Elasticsearch->new( nodes => [ @elasticsearch_nodes ]);
...
sub read_daemon_config_file 
{
    ...
    eval {
	   ...
	   #@elasticsearch_nodes         = $config->get_array('elasticsearch_node');
...
sub initialize_elasticsearch
{
    ...
    eval { 
        $elastic_search_object = Search::Elasticsearch->new( nodes => $feeder->{properties}->{elasticsearch_nodes} );
    };
    if ( $@ ) { 
        chomp $@;
...
sub validate_feeder_specific_options
{
    ...
    if ( not defined $feeder->{properties}->{elasticsearch_nodes} ) { 
