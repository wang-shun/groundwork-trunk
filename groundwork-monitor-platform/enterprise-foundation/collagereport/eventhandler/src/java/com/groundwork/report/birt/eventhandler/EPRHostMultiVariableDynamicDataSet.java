...
public class EPRHostMultiVariableDynamicDataSet extends DataSetEventAdapter
{
	public void beforeOpen(IDataSetInstance dataSet, IReportContext reportContext)
	{
		...
		try
		{
			...
			queryText += "WHERE (h.HostName = '" + (String) reportContext.getParameterValue("HostName") + "' OR h.HostName = '" + (String) reportContext.getParameterValue("HostName_Right") + "') ";
			...
			queryText += "GROUP BY dayofyear(lpd.LastCheckTime), h.HostID ";
