...
public class EPRHostMultiVariableDynamicDataSetForTable extends DataSetEventAdapter
{
	public void beforeOpen(IDataSetInstance dataSet, IReportContext reportContext)
	{
		...
		String 		queryText;
		String		h_name_left, h_name_right, sort_field, sort_order;
		...
		try
		{
			...
			h_name_left = (String) reportContext.getParameterValue("HostName");
			h_name_right = (String) reportContext.getParameterValue("HostName_Right");
			...
			queryText += "where (h.HostName = '" + h_name_left + "' OR h.HostName = '" + h_name_right + "') AND (";
			...
			queryText += "group by dayofyear(lpd.LastCheckTime), pdl.PerformanceName, h.HostID ";
