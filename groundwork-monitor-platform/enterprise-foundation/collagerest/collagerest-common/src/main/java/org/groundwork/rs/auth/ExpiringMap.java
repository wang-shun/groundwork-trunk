...
public class ExpiringMap<K, V> implements Map<K, V> {
    ...
    public Collection<V> values() {
        // quietly read values out of the map, (does not set
        // value last access time), returning collection
        List<V> values = new ArrayList<V>(delegate.size());
        for (ExpiringObject object : delegate.values()) {
    ...
    public Set<Map.Entry<K, V>> entrySet() {
        // quietly read entries out of the map, (does not set
        // value last access time), returning entry set
        Map<K, V> entries = new HashMap<K, V>();
        for (Map.Entry<K, ExpiringObject> entry : delegate.entrySet()) {
            entries.put(entry.getKey(), entry.getValue().getValue());
        }
