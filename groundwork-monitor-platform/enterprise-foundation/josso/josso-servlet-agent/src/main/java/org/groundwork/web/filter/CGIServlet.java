...
package org.groundwork.web.filter;
...
public final class CGIServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse res)
        ...
        if (usePostDataCaching) {
            postData = (String)req.getAttribute(GenericServletSSOAgentFilter.ORG_GROUNDWORK_WEB_FILTERS_POSTDATA);
            int contentLength = -1;
            if (postData != null && "POST".equals(req.getMethod())) {
                contentLength = postData.length();
                req.removeAttribute(GenericServletSSOAgentFilter.ORG_GROUNDWORK_WEB_FILTERS_POSTDATA);
            }
        }
        ...
        if (cgiEnv.isValid()) {
            ...
            if (usePostDataCaching) {
                ...
                if ("POST".equals(req.getMethod()) && postData != null) {
                    if (debug >= 2)
    ...
    protected String retrievePostData(HttpServletRequest request) {
...
//            this.contentLength = contentLength;
        ...
        protected boolean setCGIEnvironment(HttpServletRequest req) throws IOException {
            ...
            envp.put("REQUEST_URI", nullsToBlanks(req.getRequestURI())
                    + nullsToBlanks(query_string == null ? "" : ("?" + query_string)));
