...
import org.groundwork.web.filter.IOTools;
...
public class GenericServletSSOAgentFilter implements Filter {
    ...
    public static final String ORG_GROUNDWORK_WEB_FILTERS_POSTDATA = "org.groundwork.web.filters.postdata";
    /**
     * The servlet context
     */
    ...
    /**
     * was necessary to get around input stream getting invalidated by req.getParameter()
     * no longer necessary: instead JOSSO was modified to only use query parameter api
     */
    protected boolean usePostDataCaching = false;
    ...
    private static String BASE_URL = GateinConfiguration
    ...
    private String gwAuthRoles = null;

    // comma separated list of ignored extensions
    private String[] ignoredExtensions = null;
    public static final String[] DEFAULT_EXCLUDE_EXTENSIONS = {".js", ".css", ".gif", ".png", ".jpg", ".xml", ".csv", ".html", ".htm", ".ico"};
        ...
        try {
            ...
            synchronized (semaphore) {
                if (agent == null) {
                    ...
                    lookup.init("file:/usr/local/groundwork/config/josso-agent-config.xml"); // For spring compatibility ...
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        ...
        resourceName = filterConfig.getInitParameter("RESOURCE_NAME");
        gwAuthRoles = filterConfig.getInitParameter("AUTHORIZED_ROLES");
        String ignored = filterConfig.getInitParameter("IGNORED_EXTENSIONS");
        if (ignored == null)
            ignoredExtensions = DEFAULT_EXCLUDE_EXTENSIONS;
        else {
            ignoredExtensions = ignored.split("\\s*,\\s*");
        }
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain)
        ...
        if (isExcludedExtension(uri)) {
            if (filterConfig != null) {
                if (log.isDebugEnabled())
                    log.debug(">>> excluding " + uri);
                RequestDispatcher defaultDispatcher = filterConfig.getServletContext().getNamedDispatcher("default");
                if (defaultDispatcher != null) {
                    if (log.isDebugEnabled()) {
                        log.debug("Dispatching to excluded resource " + uri);
                    }
                    defaultDispatcher.forward(hreq, hres);
                    return;
                }
            }
        }
        ...
        Map<String, String> queryParameters = parseQueryParameters(hreq.getQueryString());

        if (log.isDebugEnabled())
            log.debug("Processing : " + hreq.getContextPath());

        if (usePostDataCaching) {
            if ("POST".equals(hreq.getMethod()) && request.getContentLength() > 0) {
                ByteArrayOutputStream out = new ByteArrayOutputStream();
                IOTools.flow(request.getInputStream(), out);
                out.close();
                String filterPostData = new String(out.toByteArray(), "UTF-8");
                hreq.setAttribute(ORG_GROUNDWORK_WEB_FILTERS_POSTDATA, filterPostData);
            }
        }
        ...
        try {
            ...
            String nodeId = queryParameters.get("josso_node");
            if (nodeId != null) {
                if (log.isDebugEnabled())
            ...
            // Get our session ...
            final HttpSession session = hreq.getSession(true);
            ...
            if (cookie == null || cookie.getValue().equals("-")) {
                ...
                if (hreq.getRequestURI().endsWith(agent.getJossoSecurityCheckUri()) &&
                        queryParameters.get("josso_assertion_id") == null) {
            ...
            if (hreq.getRequestURI().endsWith(agent.getJossoSecurityCheckUri()) &&
                ...
                if (log.isDebugEnabled())
                    log.debug("josso_security_check received for uri '" + hreq.getRequestURI() + "' assertion id '" +
                                    queryParameters.get("josso_assertion_id")
                    );

                String assertionId = queryParameters.get(Constants.JOSSO_ASSERTION_ID_PARAMETER);
                ...
                if (postAuthURI != null) {
                    ...
                    hres.sendRedirect(hres.encodeRedirectURL(hreq.getScheme() + "://" + hreq.getServerName() + requestURI));
                }
            ...
            final SingleSignOnEntry entry = agent.processRequest(r);
            ...
            if (entry != null) {
                ...
                // Use a simple request wrapper to return the right remote user and principal.
                hreq = new HttpServletRequestWrapper(hreq) {
                    ...
                    public boolean isUserInRole(String role) {
                        ...
                        final String GWOS_AUTH = "com.groundwork.security.auth";
                        Object gwosObj = session.getAttribute(GWOS_AUTH);
                        if (gwosObj != null) {
                            log.debug("Getting " + GWOS_AUTH + " from session!");
                            return (Boolean) gwosObj;
                        } else {
                            boolean gwos_auth = checkUserInRole(username);
                            session.setAttribute(GWOS_AUTH, gwos_auth);
                            // Use the helper to call the portal rest
                            return gwos_auth;
                        } // end if
                    }
                };
                if (!hreq.isUserInRole(resourceName)) {
                    hres.setContentType("text/html");
                    PrintWriter out = hres.getWriter();
                    out.println("<HTML><HEAD><TITLE>Authorization Error</TITLE>"+
                        "</HEAD><BODY style=\"\n" +
                            "    font-family: Helvetica,Arial,sans-serif;\n" +
                            "    font-size: 13px;\n" +
                            "    font-weight: bold;\n" +
                            "\">Application <em>" + resourceName + "</em> not authorized for this membership. Please contact GroundWork Administrator.</BODY></HTML>");
                    out.close();
                    //String loginUrl = agent.buildLoginOptionalUrl(hreq);
                    //hres.sendRedirect(hres.encodeRedirectURL(loginUrl));
                    return;
            ...
            // stub SSL connection attributes if secure and missing
            if (hreq.isSecure()) {
                if (hreq.getAttribute("javax.servlet.request.cipher_suite") == null) {
                    hreq.setAttribute("javax.servlet.request.cipher_suite", "");
                }
                if (hreq.getAttribute("javax.servlet.request.ssl_session") == null) {
                    hreq.setAttribute("javax.servlet.request.ssl_session", "");
                }
                if (hreq.getAttribute("javax.servlet.request.key_size") == null) {
                    hreq.setAttribute("javax.servlet.request.key_size", 0);
                }
            }
    ...
    /**
     * Checks userin role by extended ui role
     *
     * @param username
     * @return
     */
    private boolean checkUserInRole(String username) {
        if (username != null) {
            ExtendedRoleClient roleClient = new ExtendedRoleClient(BASE_URL);
            try {
                com.groundworkopensource.portal.model.ExtendedRoleList extendedRoles = roleClient.findRolesByUser(username);
                List<ExtendedUIRole> roleList = extendedRoles.getList();
                if ((roleList != null) && !roleList.isEmpty()) {
                    for (ExtendedUIRole role : roleList) {
                        ExtendedUIRolePermissionList permList = role.getRolePermissions();
                        Collection<ExtendedUIRolePermission> permCol = permList.getRolePermissions();
                        for (ExtendedUIRolePermission permission : permCol) {
                            if (permission.getResource().equalsIgnoreCase(resourceName) && permission.getAction().equalsIgnoreCase("allow")) {
                                return true;
                            }
                        }
                    }
                ...
                if (gwAuthRoles != null && gwAuthRoles.equalsIgnoreCase("*"))
                    return true;
                String endPoint = BASE_URL + ROLES_ENDPOINT + username;
                String roles = this.httpGet(endPoint);
                log.debug("Output roles ==>" + roles);
                if (roles != null) {
                    StringTokenizer stkn = new StringTokenizer(gwAuthRoles, ",");
                    while (stkn.hasMoreTokens()) {
                        StringTokenizer stkn2 = new StringTokenizer(roles, ",");
                        String gwAuthRole = stkn.nextToken();
                        while (stkn2.hasMoreTokens()) {
                            if (stkn2.nextToken().equalsIgnoreCase(gwAuthRole)) {
                                log.debug("Role matched !!!!");
                                return true;
                            }
                        }
                    }
    ...
    public boolean isExcludedExtension(String uri) {
        if (uri != null) {
            for (int i = 0; i < ignoredExtensions.length; i++) {
                if (uri.endsWith(ignoredExtensions[i])) {
                    return true;
                }
            }
        }
        ...
        if (query.length() > 0) {
            ...
            for (String pair : pairs) {
                ...
                try {
                    ...
                    log.warn("Failed to encode query param: [" + pair + "] - " + e.toString());
                }
            }
        }
        if (log.isDebugEnabled()) {
            if (queryPairs != null) {
                for (String key : queryPairs.keySet()) {
                    log.debug("queryParam: " + key + ":" + queryPairs.get(key));
                }
            }
        }
        return queryPairs;
