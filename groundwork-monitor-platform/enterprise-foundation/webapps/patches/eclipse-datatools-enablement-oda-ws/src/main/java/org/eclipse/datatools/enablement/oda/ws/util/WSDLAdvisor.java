...
public class WSDLAdvisor
{
	...
	public static Definition getDefinition( String wsdlURI ) throws WSDLException
	{
		...
		definition = reader.readWSDL( wsdlURI, wsdlInputSource(wsdlURI) );
    ...
    public static InputSource wsdlInputSource(String wsdlURI) throws WSDLException
    {
        // use simple HttpURLConnection client to get WSDL input
        try
        {
            ...
            // authorization
            String user = WSClientConfiguration.getProperty(WSClientConfiguration.WEBSERVICES_USERNAME);
            String password = WSClientConfiguration.getProperty(WSClientConfiguration.WEBSERVICES_PASSWORD);
            if ((user != null) && (password != null)) {
                // decode and decrypt password if necessary
                String encryptionEnabledProperty = WSClientConfiguration.getProperty(WSClientConfiguration.ENCRYPTION_ENABLED);
                boolean encryptionEnabled = ((encryptionEnabledProperty == null) || Boolean.parseBoolean(encryptionEnabledProperty));
                if (encryptionEnabled) {
                    ...
                    if (mainKey != null) {
                        ...
                        byte[] decodedPasswordBytes = Base58Codec.doDecode(password);
                        // base64 encode encrypted bytes
                        String base64Password = new String(Base64.encodeBase64(decodedPasswordBytes));
                        // decrypt base64 encoded string
                        BasicTextEncryptor textEncryptor = new BasicTextEncryptor();
                        textEncryptor.setPassword(mainKey);
                        password = textEncryptor.decrypt(base64Password);
                    }
                }
                // set HTTP Basic Authorization header
                String basicAuthorization = new String(Base64.encodeBase64((user + ":" + password).getBytes("ASCII")), "ASCII");
                connection.setRequestProperty("Authorization", "Basic " + basicAuthorization);
            }
            // connect and return input source
            connection.connect();
            ...
            inputSource.setSystemId(wsdlURI);
            return inputSource;
        }
        catch (Exception e)
        {
            throw new WSDLException(WSDLException.OTHER_ERROR, "Unable to read WSDL "+wsdlURI+": "+e, e);
        }
    }
