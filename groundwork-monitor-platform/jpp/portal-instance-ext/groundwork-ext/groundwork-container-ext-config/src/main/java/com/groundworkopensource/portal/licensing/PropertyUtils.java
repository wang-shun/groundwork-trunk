...
public class PropertyUtils {
     ...
     * @throws IOException
     */
    public static String getPropertyFromFilePath(String propFilePath, String key)
            throws IOException {
        String value = null;
        if (propFilePath != null) {
            Properties porps = loadPropertiesFromFilePath(propFilePath);
            if (porps != null) {
                value = porps.getProperty(key);
            }
        }
    ...
    public static Properties loadFallbackProperties(
        ...
        if (null != servletContext) {
            ...
            try {
                fallbackFS = new FileInputStream(servletContext
                        .getRealPath(applicationType
                                .getFallbackPropertiesPath()));
                fallbackProps.load(fallbackFS);
            } catch (Exception e) {
                logger
                        .debug("Unable to find fallback properties ["
                ...
                try {
                    ...
                    logger
                            .error("Unable to close the input stream for fallback properties file - "
                                    + ioe.getMessage());
                }
            }
        }
