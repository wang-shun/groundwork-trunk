...
public class HostListDataBean extends HostPagedListDataModel {
    ...
    public List<HostBean> queryForHostsByFilter(Filter filter, int startIndex,
        ...
        try {
            collection = foundFacade.getHostsbyCriteria(filter, sort,
        ...
        if (collection != null) {
            ...
            if (showNonAcknowledgedHosts) {
                for (Host host : hosts) {
                    ...
                    if (propertyTypeBinding != null) {
                        ...
                        if ((acknowledgedProperty != null) && acknowledgedProperty.isValue()) {
                            totalCount--;
                        }
                    }
                }
            ...
            for (Host host : hosts) {
                ...
                if (propertyTypeBinding != null) {
                    BooleanProperty acknowledgedProperty = propertyTypeBinding
                            .getBooleanProperty("isAcknowledged");
                    isAcknowledged = ((acknowledgedProperty != null) && acknowledgedProperty.isValue());
                    // Duration
                    DateProperty lastStateChangeProperty = propertyTypeBinding
                            .getDateProperty("LastStateChange");
                    if (lastStateChangeProperty != null) {
                        duration = DateUtils.computeDuration(lastStateChangeProperty
                                        .getValue());
                    } else {
                    ...
                    if (lastPluginOutputProperty != null) {
                        StringTokenizer stkn = new StringTokenizer(lastPluginOutputProperty.getValue(), "^^^");
                    ...
                    HostBean hostBean = new HostBean(host.getHostID(), host
                            ...
                            isAcknowledged, tooltip, applicationType);
                    hostBean.setTotalCount(totalCount);
                    // set service status as well as tool <></>ip for service status
                    ...
                    String monitorStatus = host.getMonitorStatus().getName();
