...
public class StatisticsModelPopUpListBean extends StatisticsPagedListDataModel {
    ...
    private List<ModelPopUpDataBean> getServicePopUpDataList(Filter filter,
        ...
        List<ModelPopUpDataBean> serviceList = Collections
                .synchronizedList(new ArrayList<ModelPopUpDataBean>());
        WSFoundationCollection serviceCollection = foundFacade
                .getServicesbyCriteria(filter, sort, startRow, pageSize);
        if (null != serviceCollection) {
            ServiceStatus[] serviceArr = serviceCollection
                    .getServiceStatus();

            if (serviceArr != null) {
                for (int i = 0; i < serviceArr.length; i++) {
                    ModelPopUpDataBean modelpopupbean = new ModelPopUpDataBean();
                    modelpopupbean.setTotalCount(serviceCollection
                            .getTotalCount());
                    PropertyTypeBinding propertyTypeBinding = null;
                    if (serviceArr[i] != null) {
                        propertyTypeBinding = serviceArr[i].getPropertyTypeBinding();
                    }
                    if (propertyTypeBinding != null) {
                        ...
                        if (isAcknowledgedValue != null && isAcknowledgedValue.isValue()) {
                        ...
                        String hostName = serviceArr[i].getHost().getName();
                        if (hostName != null) {
                            modelpopupbean.setParentName(hostName);
                        }
                        // Setting service parent sub page URL
                        modelpopupbean.setParentPageURL(NodeURLBuilder
                                .buildNodeURL(NodeType.HOST, serviceArr[i].getHost()
                                        .getHostID(), hostName));
                        // set application type
                        String applicationType = CommonUtils.getApplicationNameByID(serviceArr[i].getApplicationTypeID());
                        modelpopupbean.setApplicationType(applicationType);
    ...
    private List<ModelPopUpDataBean> getHostPopUpDataList(Filter filter,
        ...
        WSFoundationCollection collection = foundFacade
                .getHostsbyCriteria(filter, sort, startRow, pageSize);
        Host[] hostArr = collection.getHost();
        ...
        if (hostArr != null) {
            for (int i = 0; i < hostArr.length; i++) {
                ...
                PropertyTypeBinding propertyTypeBinding = null;
                if (hostArr[i] != null) {
                    propertyTypeBinding = hostArr[i].getPropertyTypeBinding();
                }
                if (propertyTypeBinding != null) {

                    BooleanProperty isAcknowledgedValue = propertyTypeBinding
                            .getBooleanProperty(FilterConstants.IS_ACKNOWLEDGED);
                    if (isAcknowledgedValue != null && isAcknowledgedValue.isValue()) {
                    ...
                    // set application type
                    String applicationType = CommonUtils.getApplicationNameByID(hostArr[i].getApplicationTypeID());
                    modelpopupdatabean.setApplicationType(applicationType);
