...
import com.groundworkopensource.portal.statusviewer.handler.HealthPortletsHandler;
...
public class ActionBean extends ServerPush implements Serializable {
	...
	/**
	 * show.downtime.menu=yes/no
	 */
	private String SHOW_DOWNTIME_MENU = "yes";

	/**
	 *
	 */
	private static final String SHOW_DOWNTIME_MENU_PROPERTY = "show.downtime.menu";
	...
	public ActionBean() throws GWPortalGenericException {
		...
		// get status-viewer.properties show.downtime.menu if not set defaluts to yes
		SHOW_DOWNTIME_MENU = PropertyUtils.getProperty(ApplicationType.STATUS_VIEWER, SHOW_DOWNTIME_MENU_PROPERTY);
		if (SHOW_DOWNTIME_MENU == null)
			SHOW_DOWNTIME_MENU = "yes";
	...
	private void populateHostGroupChildMenus() throws GWPortalGenericException {
		for (MenuItem parentMenu : parentMenuList) {
			...
			if (parentMenu.getChildren() != null) {
				...
				case DOWNTIME:
					// Add the sub/child menu-list to the parent menu list
					if (SHOW_DOWNTIME_MENU.equalsIgnoreCase("yes")) {
						parentMenu.getChildren().addAll(getDownTimeChildMenuForHostGroup());
					}
					break;
				case NOTIFICATIONS:
					// Add the sub/child menu-list to the parent menu list
	...
	private void createParentMenuList() {
        // get selected node application type, (or null)
        HealthPortletsHandler healthPortletsHandler = (HealthPortletsHandler) FacesUtils
                .getManagedBean(Constant.HEALTH_PORTLETS_HANDLER_BEAN);
        String applicationType = ((healthPortletsHandler != null) ?
                healthPortletsHandler.getSelectedNodeApplicationType() : null);

		for (ParentMenuActionEnum menuEnum : ParentMenuActionEnum.values()) {

			if (!SHOW_DOWNTIME_MENU.equalsIgnoreCase("yes") && menuEnum.getMenuString().equalsIgnoreCase("Downtime"))
				continue;

            // match the selected node application type
            if ((menuEnum.getApplicationType() != null) && !menuEnum.getApplicationType().equals(applicationType)) {
                continue;
            }
