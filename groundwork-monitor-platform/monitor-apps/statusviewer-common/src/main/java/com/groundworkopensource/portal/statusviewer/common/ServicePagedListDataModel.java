 ...
 */
public abstract class ServicePagedListDataModel extends DataModel {
     ...
     * data page
     */
    protected ServiceListDataPage page;

    /**
     * Logger
     */
    private static final Logger LOGGER = Logger
            .getLogger(ServicePagedListDataModel.class.getName());
     ...
     * @param pageSize
     */
    public ServicePagedListDataModel(int pageSize) {
        super();
    ...
    /**
     * Not used in this class; data is fetched via a call back to the fetchData
    ...
    /**
     * Specify what the "current row" within the data set is. Note that the
     ...
     */
    private ServiceListDataPage getPage() {
        if (page != null) {
            return page;
        }
    ...
    public Object getRowData() {
        if (rowIndex < 0) {
            LOGGER.debug("Rowindex is less than 0 : " + rowIndex);
            // Ideally RuntimeException should never be thrown.
        ...
        if (rowIndex >= datasetSize) {
            LOGGER.debug("Found rowIndex greater than datasetSize.");
        ...
        /*
         * if list is empty, return a new ServiceBean object to avoid
         * java.lang.ArrayIndexOutOfBoundsException: Array index out of range: 0
         */
        if (page.getData().size() == 0) {
            LOGGER.debug("Found page.getData().size() = 0");
            return new ServiceBean();
        }
        if ((rowIndex - startRow) < 0) {
            LOGGER.debug("Requested row does not exist : "
                    + (rowIndex - startRow));
            return new ServiceBean();
        }
     ...
     * @return DataPage
     */
    public abstract ServiceListDataPage fetchPage(int startRow, int pageSize);

    /**
     * @param page
     */
    public void setPage(ServiceListDataPage page) {
        this.page = page;
    }
