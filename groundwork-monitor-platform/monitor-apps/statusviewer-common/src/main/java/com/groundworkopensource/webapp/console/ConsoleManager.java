...
public class ConsoleManager {
    ...
    public void populateAllOpenEvents(Filter searchFilter) {
        ...
        ConsoleHelper.getFilterBean().setFilter(null);
    ...
    /**
     * Populate events by searchFilter.
     *
     * @param searchFilter the search filter
     */
    public void populateEventsBySearchFilter(Filter searchFilter) {
        logger.debug("Enter populateEventsBySearchFilter method");
        if (searchFilter != null) {
            filterInSession.setFilter(searchFilter);
        }
        this.initializePage(true);
        logger.debug("Exit populateEventsBySearchFilter method");
    }
    /**
     * Populate events by for the public filters.
     *
     * @param pubFilter    the pub filter
     * @param searchFilter the search filter
     */
    ...
    public ActionReturn performAction(int[] messageIds, int actionID,
                                      String appType, String userComment, boolean isConsoleRequester) {
        ...
        try {
            ...
            if (userName == null)
                userName = FacesUtils.getLoggedInUser();
            ...
            parameters[8] = new StringProperty(
                    ...
                    .generateHostString(messageIds, isConsoleRequester));
     ...
     *
     * @param resetDynaColumns the reset dyna columns
     */
     ...
     * @return the string
     */
    private String generateHostString(int[] messageIds, boolean isConsoleRequester) {
        String result = null;
        StringBuffer sb = new StringBuffer();
        if (!isConsoleRequester) {
            EventActionHandler eventActionHandler = (EventActionHandler) FacesUtils
                    .getManagedBean(Constant.EVENT_ACTION_HANDLER_BEAN);
            return eventActionHandler.generateHostString(messageIds);
        } else {
            TabsetBean tabset = ConsoleHelper.getTabSetBean();
            Tab tab = tabset.getTabs().get(tabset.getTabIndex());
            EventBean[] events = tab.getDataTableBean().getEvents();
            // If not console requester then call is coming from SV
    ...
    public HostDetailBean getHostDetails(String hostName) {
        ...
        try {
            ...
            Filter hostFilter = new Filter(ConsoleConstants.FILTER_HOST,
     ...
     *
     * @param alarmButtonText the alarmButtonText to set
     */
     ...
     *
     * @param muteButtonImage the muteButtonImage to set
     */
     ...
     *            the search bean
     */
    void populateData(Tab tab, SearchBean searchBean) {
        String tabLabel = tab.getHiddenLabel();
        Filter searchFilter = ConsoleHelper.createSearchFilter(searchBean, null);
        String filterType = tab.getFilterType();
        if (StringUtils.isBlank(tabLabel) || StringUtils.isBlank(filterType)) {
            populateEventsBySearchFilter(searchFilter);
            return;
        }
        // TODO: this doesn't work this way.  filter won't get highlighted
        ConsoleHelper.hightlightNode(filterType);
        String filter = tabLabel.substring(tabLabel.indexOf("=") + 1, tabLabel.length());
        if (filterType.equals(ConsoleConstants.SYSTEM_FILTER_HOST_GROUPS)) {
            populateEventsByHostGroup(filter, searchFilter);
        } else if (filterType.equals(ConsoleConstants.SYSTEM_FILTER_APPLICATIONS)) {
            populateEventsByApplicationType(filter, searchFilter);
        } else if (filterType.equals(ConsoleConstants.SYSTEM_FILTER_OPERATION_STATUS)) {
            populateEventsByOperationStatus(filter, searchFilter);
        } else if (filterType.equals(ConsoleConstants.SYSTEM_FILTER_SERVICE_GROUPS)) {
            populateEventsByServiceGroup(filter, searchFilter);
        } else {
            for (FilterConfigBean filterConfig : ConsoleHelper.getPublicFilters().getFilterConfigs()) {
                if (filter.equalsIgnoreCase(filterConfig.getLabel())) {
                    populateEventsByCombinedFilters(filterConfig, searchFilter);
                    // TODO: this doesn't work this way.  filter won't get highlighted
