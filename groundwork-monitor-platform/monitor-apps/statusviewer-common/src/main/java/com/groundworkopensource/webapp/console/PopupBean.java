...
public class PopupBean {
    ...
    public boolean isConsoleRequester() {
        return consoleRequester;
    }

    public void setConsoleRequester(boolean consoleRequester) {
        this.consoleRequester = consoleRequester;
    }

    private boolean consoleRequester = true;
	...
	public void closeInputModalPopup(ActionEvent e) {
		...
		if (button.getValue() != null && button.getValue().toString().equalsIgnoreCase("Submit")) {
			...
			ActionReturn actionReturn = action.performAction(messageIds,
					actionId, appType, inputText,consoleRequester);
