...
import com.groundworkopensource.portal.statusviewer.bean.tree.NetworkCustomGroup;
...
public class DynamicNodeUserObject extends IceUserObject {
	...
	public void addHostGroup(ActionEvent event) {
		try {
			...
			if (extendedUIRoleBean != null
				...
				List<CustomGroup> customGroups = rtmm.getRawCustomGroups();
				...
				for (CustomGroup group : customGroups) {
					String concreteEntityType = ConsoleHelper
							.checkConcreteEntityType(group, customGroups);
					if ((group.getParents() == null || group.getParents()
							.size() == 0)
							&& (concreteEntityType != null && concreteEntityType
									.equals(ConsoleConstants.DB_HOSTGROUP))) {
						logger.debug("Root group name=" + group.getGroupName()
								+ ", type=" + concreteEntityType);
						DefaultMutableTreeNode node = this
								.addCGClonedUserObject(null, group);
						this.createCustomGroupBranch(group, customGroups, node);
					} // end if
				} // end for
		...
		if (node != null) {
			...
			if (tempBranchList.size() > 0) {
				aggregatedBubbleUpStatus = this
						.determineBubbleUpStatusForCustomGroupList(tempBranchList);
	 ...
	 * @return
	 */
	private String determineBubbleUpStatusForCustomGroupList(
			ArrayList<String> customGroupList) {
		if (customGroupList != null && customGroupList.size() > 0) {
			...
			for (int i = 0; i < ranking.length; i++) {
				for (int j = 0; j < customGroupList.size(); j++) {
					String customGroupStatus = customGroupList.get(j);
					logger.debug("*****>" + customGroupStatus);
					if (ranking[i].equalsIgnoreCase(customGroupStatus)) {
						return customGroupStatus;// .replaceFirst("host-group",
													// "customgroup");
					} // end if
				} // end for
			} // end if
		...
		// return
		// determineBubbleUpStatusForCustomServiceGroupList(customGroupList);
		return ConsoleConstants.CUSTOM_GROUP_PENDING;
	}
	 ...
	 * @param node
	 */
	private void createCustomGroupBranch(CustomGroup group,
			List<CustomGroup> customGroups, DefaultMutableTreeNode node) {
		...
		for (CustomGroupElement element : elements) {
			...
			if (entityType.equalsIgnoreCase(ConsoleConstants.DB_CUSTOMGROUP)) {
				CustomGroup nextLevel = ConsoleHelper.findCustomGroupById(
						element.getElementId(), customGroups);
				DefaultMutableTreeNode childNode = this.addCGClonedUserObject(
						node, nextLevel);
				createCustomGroupBranch(nextLevel, customGroups, childNode);
			} else {
				...
				if (entityType.equalsIgnoreCase(ConsoleConstants.DB_HOSTGROUP)) {
					NetworkMetaEntity hostGroup = rtmm
							.getHostGroupById((int) element.getElementId());
				...
				if (entityType
						.equalsIgnoreCase(ConsoleConstants.DB_SERVICEGROUP)) {
					NetworkMetaEntity serviceGroup = rtmm
							.getServiceGroupById((int) element.getElementId());
	...
	private DefaultMutableTreeNode addCGClonedUserObject(
			DefaultMutableTreeNode parent, CustomGroup customGroup) {
		DefaultMutableTreeNode clonedWrapper = new DefaultMutableTreeNode();
		DynamicNodeUserObject clonedUserObject = new DynamicNodeUserObject(
				clonedWrapper, treeBean);
			...
			if (extendedUIRoleBean != null
				...
				for (CustomGroup group : customGroups) {
					...
					if ((group.getParents() == null || group.getParents()
							.size() == 0)
							&& (concreteEntityType != null && concreteEntityType
									.equals(ConsoleConstants.DB_SERVICEGROUP))) {
						logger.debug("Root group name=" + group.getGroupName()
								+ ", type=" + concreteEntityType);
						DefaultMutableTreeNode node = this
								.addCGClonedUserObject(null, group);
