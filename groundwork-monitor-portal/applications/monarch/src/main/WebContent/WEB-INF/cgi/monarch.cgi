...
sub parse_query($$) {
    ...
    my %name_vals = ();
    foreach my $name ( $query->param ) {
	my $val = StorProc->sanitize_string_but_keep_newlines( $query->param($name) );
...
"You attempted to save an unsupported configuration. If you wish to turn off all notifications for this $single_obj, do so by turning off <i>Notifications enabled</i> at the template or $single_obj level rather than by disabling all the individual <i>Notification options</i> choices at the $single_obj level. The <i>Notification options</i> choices below have been reset to the selections from the template.";
	...
	my %t = StorProc->fetch_one( 'host_templates', 'name', $query->param('template') );
...
sub build_host_template($) {
    ...
    # FIX MINOR:  GWMON-8146:  add the "Check period" field here
...
"Note: no parent template (recursively) has a check command defined.<br><b><font color=#FF0000>ERROR:  You have a cyclical chain of parents in your service templates, starting with \"$t{'name'}\".</font></b>"
	...
	$form .= Forms->list_box_submit( 'Check command:', 'command', \@commands, $command, '', $docs{'check_command'}, $tab++ );
	$form .= Forms->display_hidden( 'Command definition:', '', HTML::Entities::encode( $cmd{'command_line'} ) );
	$form .= Forms->display_hidden( 'Usage:', '', $usage );
	$form .= Forms->text_area( 'Command line:', 'command_line', $command_line, '3', '80', '', $docs{'command_line'}, '', $tab++ );
	...
	# $help{url} = StorProc->doc_section_url('Managing+Services', 'ManagingServices-Step3ServiceCheck');
...
<data>
  <prop name="command_line"><![CDATA[$command_line]]>
	...
	if ( $task eq 'new' ) {
	    ...
	    $help{url} = StorProc->doc_section_url('Defining+Commands', 'DefiningCommands-CommandLine');
		...
		'Command line:', 'command_line', $command_line, '', $textsize{'command_line'},
	    ...
	    if ( $obj eq 'commands' ) {
		$form .= Forms->hidden( \%hidden );
		$help{url} = StorProc->doc_section_url('Defining+Commands', 'DefiningCommands-Step2NewCommand');
		if ( $task eq 'modify' ) {
...
"Specify individual weekdays (such as \"\u$new_day_rule\") at the top of the page, not as exceptions. Please try again.";
	...
	$form .= Forms->hidden( \%hidden );
	$help{url} = StorProc->doc_section_url('Defining+Time+Periods');
...
sub service_group() {
	...
	if ($got_form) {
	    ...
	    $hidden{'validate'} = 1;
	    $form .= Forms->hidden( \%hidden );
	    $help{url} = StorProc->doc_section_url('Managing+Services', 'ManagingServices-ServiceGroups');
	    $form .= Forms->form_bottom_buttons( \%add, \%cancel, \%help, $tab++ );
	}
    }
	...
	$form .= Forms->hidden( \%hidden );
	$help{url} = StorProc->doc_section_url('Setting+Up+Notifications', 'SettingUpNotifications-ConfiguringHostGroupsandServiceGroups');
...
qq(An existing host wizard session exists for $host{'name'}. Do you wish to continue with this host definition? (No will delete the host.)&nbsp;);
	...
	if ( $step eq 'host_vitals' ) {
	    ...
	    my %values        = ();
	    my @hostgroups    = $query->param('hostgroups');
	    my $coords2d      = StorProc->sanitize_string( $query->param('coords2d') );
	    my $coords3d      = StorProc->sanitize_string( $query->param('coords3d') );
		...
		$service_detail{'command_line'}, '',                     $service_detail{'notes'}
	    );
	    my $id = StorProc->insert_obj_id( 'services', \@values, 'service_id' );
	    if ( $id =~ /^Error/ ) { push @errors, $id }
	    my %where = (
		...
		# FIX MAJOR NOW:  delete $service_detail{'notes'} as well here, so it doesn't carry across to other services?
		# Probably so.
		delete $service_detail{'notes'};
		delete $service_detail{'template'};
		# FIX MAJOR NOW:  delete $service_detail{'inherit'} as well here, so it doesn't carry across to other services?
	...
	if ( $step eq 'host_vitals' ) {
	    ...
	    $form .= Forms->hidden( \%hidden );
	    $help{url} = StorProc->doc_section_url('Managing+Hosts', 'ManagingHosts-CreatingaNewHostusingtheNewHostWizard');
	    ...
	    $form .= Forms->hidden( \%hidden );
	    $help{url} = StorProc->doc_section_url('Managing+Hosts', 'ManagingHosts-HostProperties1');
	    ...
	    $form .= Forms->hidden( \%hidden );
	    $help{url} = StorProc->doc_section_url('Managing+Hosts', 'ManagingHosts-HostProperties2');
	    $form .= Forms->form_bottom_buttons( \%back, \%next, \%cancel, \%help, $tab++ );
	}
	elsif ( $step eq 'service_list' ) {
	    $form .= Forms->header( $page_title, $session_id, $top_menu );
	    ...
	    $form .= Forms->hidden( \%hidden );
	    $help{url} = StorProc->doc_section_url('Managing+Hosts', 'ManagingHosts-SelectServices');
	    $form .= Forms->form_bottom_buttons( \%back, \%next, \%cancel, \%help, $tab++ );
	}
	elsif ( $step eq 'service_detail' ) {
	    $form .= Forms->header( $page_title, $session_id, $top_menu );
	    ...
	    # FIX MAJOR NOW:  if this host service has already been added to the database (i.e., if we got
	    # here via the "<< Back" button), pull notes from there instead, so we have a sensible value.
	    # The same thing goes for the command line and probably other fields as well.
    ...
    "Note: no parent template (recursively) has a check command defined.<br><b><font color=#FF0000>ERROR:  You have a cyclical chain of parents in your service templates, starting with \"$t{'name'}\".</font></b>"
	    ...
	    $form .= Forms->hidden( \%hidden );
	    $help{url} = StorProc->doc_section_url('Managing+Hosts', 'ManagingHosts-SaveasProfile');
...
sub design() {
	...
	if ( $obj eq 'contacts' ) {
		...
		$help{url} = StorProc->doc_section_url('Setting+Up+Notifications', 'SettingUpNotifications-CreatingContacts');
	...
	unless ($got_form) {
		...
		if ( !$template && $task =~ /copy|use/ ) {
		    ...
		    my $help_url =
			( $obj eq 'hostgroups' )
			? StorProc->doc_section_url( 'Setting+Up+Notifications', 'SettingUpNotifications-ConfiguringHostGroupsandServiceGroups' )
			...
			? StorProc->doc_section_url( 'Setting+Up+Dependencies', 'SettingUpDependencies-ServiceDependencies' )
		      : ( $obj eq 'contactgroups' && $task eq 'copy' )
			? StorProc->doc_section_url('Setting+Up+Notifications', 'SettingUpNotifications-CreatingContactGroups')
		      : ( $obj eq 'contactgroups' )
			? StorProc->doc_section_url('Setting+Up+Notifications', 'SettingUpNotifications-NotificationObjects')
		      : ( $obj eq 'contact_templates' )
			? StorProc->doc_section_url('Setting+Up+Notifications', 'SettingUpNotifications-CreatingContactTemplates')
		      : ( $obj eq 'extended_host_info_templates' )
			? StorProc->doc_section_url('Defining+Extended+Info')
		      : '';
...
"Cannot create cyclical dependency. Host \"$dependent_host\" is currently the master to proposed master host \"$master_host\".";
	...
	if ( $query->param('task') eq 'new' ) {
	    ...
	    $page .= Forms->hidden( \%hidden );
	    $help{url} = StorProc->doc_section_url('Setting+Up+Dependencies', 'SettingUpDependencies-HostDependencies');
	    $page .= Forms->form_bottom_buttons( \%add, \%cancel, \%help, $tab++ );
	}
	else {
...
sub delete_hosts() {
	...
	if ($search) {
		...
		my $help_url = StorProc->doc_section_url('Managing+Hosts', 'ManagingHosts-DeletingMultipleHosts');
		$page .= Forms->toggle_delete($help_url);
	    }
	}
	unless (%hosts) {
	    $help{url} = StorProc->doc_section_url('Managing+Hosts', 'ManagingHosts-DeletingMultipleHosts');
...
sub delete_host_services() {
	...
	if ($service) {
		...
		my $help_url = StorProc->doc_section_url('Managing+Services', 'ManagingServices-DeletingMultipleHostServices');
		$page .= Forms->toggle_delete($help_url);
	    }
	}
	unless (@hosts) {
	    $help{url} = StorProc->doc_section_url('Managing+Services', 'ManagingServices-DeletingMultipleHostServices');
	    $page .= Forms->form_bottom_buttons( \%close, \%help, $tab++ );
	}
    }
...
sub parent_child() {
	...
	$page .= Forms->hidden( \%hidden );
	$help{url} = StorProc->doc_section_url('Setting+Up+Dependencies', 'SettingUpDependencies-ParentChildRelationships');
	$page .= Forms->form_bottom_buttons( \%save, \%delete, \%cancel, \%help, $tab++ );
    }
    return $page;
}
...
sub externals($) {
	...
	$form .= Forms->hidden( \%hidden );
	my %cancel = ( 'name' => 'close', 'value' => 'Cancel' );
	$help{url} = StorProc->doc_section_url('Configuring+Externals');
	if ( defined($task) && $task eq 'new' ) {
	    $form .= Forms->form_bottom_buttons( \%add, \%cancel, \%help, $tab++ );
	}
	else {
...
sub escalations() {
	...
	if ( $obj =~ /escalation_templates/ ) {
	    ...
	    $hidden{'type'} = $type;
	    $hidden{'task'} = $task;
	    my $help_url = StorProc->doc_section_url( 'Setting+Up+Notifications', 'SettingUpNotifications-CreatingEscalations' );
...
sub escalation_trees() {
	...
	if ( $obj_view eq 'new' ) {
	    ...
	    $hidden{'obj_view'} = 'new';
	    $page .= Forms->hidden( \%hidden );
	    $help{url} = StorProc->doc_section_url('Setting+Up+Notifications', 'SettingUpNotifications-DefiningEscalationTrees');
	    $page .= Forms->form_bottom_buttons( \%add, \%cancel, \%help, $tab++ );
	}
	    ...
	    $help{url} =
	      ( $type eq 'host' )
	      ? StorProc->doc_section_url( 'Setting+Up+Notifications', 'SettingUpNotifications-Step2AddEscalationstoanEscalationTree' )
	      : StorProc->doc_section_url( 'Setting+Up+Notifications', 'SettingUpNotifications-ConfiguringEscalationTrees' );
	    ...
	    $help{url} =
	      ( $type eq 'host' )
	      ? StorProc->doc_section_url( 'Setting+Up+Notifications', 'SettingUpNotifications-Step3AssigningHostGroups' )
	      : StorProc->doc_section_url( 'Setting+Up+Notifications', 'SettingUpNotifications-ConfiguringEscalationTrees' );
...
sub host_profile() {
  ...
  <prop name="apply_services"><![CDATA[$apply_services]]>
	...
	$form .= Forms->hidden( \%hidden );
	$help{url} = StorProc->doc_section_url('Creating+Profiles', 'CreatingProfiles-HostProfile:HostDetail');
	...
	$form .= Forms->members( 'Parents:', 'parents', \@parents, \@nonmembers, '', 20, $docs{'parents'}, '', $tab++ );
	$form .= Forms->hidden( \%hidden );
	$help{url} = StorProc->doc_section_url('Creating+Profiles', 'CreatingProfiles-HostProfile:Parents');
	...
	$form .= Forms->hidden( \%hidden );
	$help{url} = StorProc->doc_section_url('Creating+Profiles', 'CreatingProfiles-HostProfile:HostGroups');
	$form .= Forms->form_bottom_buttons( \%save, \%help, $tab++ );
    }
    elsif ( $obj_view eq 'escalation_trees' ) {
	my $host_escalation    = $query->param('host_escalation');
	...
	$form .= Forms->hidden( \%hidden );
	$help{url} = StorProc->doc_section_url('Creating+Profiles', 'CreatingProfiles-HostProfile:EscalationTrees');
	$form .= Forms->form_bottom_buttons( \%save, \%help, $tab++ );
    }
    elsif ( $obj_view eq 'externals' ) {
	$form .= Forms->host_profile_top( $name, $session_id, $obj_view, $auth_add{'externals'}, \%obj );
	...
	$form .= Forms->hidden( \%hidden );
	$help{url} = StorProc->doc_section_url('Creating+Profiles', 'CreatingProfiles-HostProfile:ServiceProfiles');
	$form .= Forms->form_bottom_buttons( \%save, \%help, $tab++ );
    }
    elsif ( $obj_view eq 'assign_hosts' ) {
	$form .= Forms->host_profile_top( $name, $session_id, $obj_view, $auth_add{'externals'}, \%obj );
	...
	$form .= Forms->hidden( \%hidden );
	$help{url} = StorProc->doc_section_url('Creating+Profiles', 'CreatingProfiles-HostProfile:AssignHosts');
	$form .= Forms->form_bottom_buttons( \%save, \%help, $tab++ );
    }
    elsif ( $obj_view eq 'assign_hostgroups' ) {
	$form .= Forms->host_profile_top( $name, $session_id, $obj_view, $auth_add{'externals'}, \%obj );
	...
	$form .= Forms->hidden( \%hidden );
	$help{url} = StorProc->doc_section_url('Creating+Profiles', 'CreatingProfiles-HostProfile:AssignHostGroups');
	$form .= Forms->form_bottom_buttons( \%save, \%help, $tab++ );
    }
	...
	$help{url} = StorProc->doc_section_url('Creating+Profiles', 'CreatingProfiles-HostProfile:ApplyTab');
	...
	$hidden{'obj_view'} = 'new';
	$form .= Forms->hidden( \%hidden );
	$help{url} = StorProc->doc_section_url('Creating+Profiles', 'CreatingProfiles-HostProfileDefinition');
	$form .= Forms->form_bottom_buttons( \%add, \%cancel, \%help, $tab++ );
    }
    return $form;
}
...
sub service_profile() {
  ...
  <prop name="apply_services"><![CDATA[$services]]>
	...
	my $path = $query->param('path');
	$form .= Forms->hidden( \%hidden );
	$help{url} = StorProc->doc_section_url('Creating+Profiles', 'CreatingProfiles-ServiceProfile:Services');
	...
	$form .= Forms->hidden( \%hidden );
	$help{url} = StorProc->doc_section_url('Creating+Profiles', 'CreatingProfiles-ServiceProfile:AssignHosts');
	$form .= Forms->form_bottom_buttons( \%save, \%help, $tab++ );
    }
    elsif ( $obj_view eq 'assign_hostgroups' ) {
	$form .= Forms->service_profile_top( $name, $session_id, $obj_view, \%objs, $hidden{'selected'} );
	...
	$form .= Forms->hidden( \%hidden );
	$help{url} = StorProc->doc_section_url('Creating+Profiles', 'CreatingProfiles-ServiceProfile:AssignHostGroups');
	$form .= Forms->form_bottom_buttons( \%save, \%help, $tab++ );
    }
    elsif ( $obj_view eq 'host_profiles' ) {
	$form .= Forms->service_profile_top( $name, $session_id, $obj_view, \%objs, $hidden{'selected'} );
	...
	$form .= Forms->hidden( \%hidden );
	$help{url} = StorProc->doc_section_url('Creating+Profiles', 'CreatingProfiles-ServiceProfile:HostProfiles');
	$form .= Forms->form_bottom_buttons( \%save, \%help, $tab++ );
    }
    elsif ( $obj_view eq 'apply' ) {
	$form .= Forms->service_profile_top( $name, $session_id, $obj_view, \%objs, $hidden{'selected'} );
	...
	$form .= Forms->hidden( \%hidden );
	$form .= Forms->wizard_doc( 'Caution', $docs{'caution'}, 1, 1 );
	$help{url} = StorProc->doc_section_url('Creating+Profiles', 'CreatingProfiles-ServiceProfile:ApplyTab');
	$form .= Forms->form_bottom_buttons( \%apply, \%help, $tab++ );
    }
    elsif ( $obj_view eq 'applied' ) {
	$form .= Forms->service_profile_top( $name, $session_id, $obj_view, \%objs, $hidden{'selected'} );
	...
	$hidden{'obj_view'} = 'new';
	$form .= Forms->hidden( \%hidden );
	$help{url} = StorProc->doc_section_url('Creating+Profiles', 'CreatingProfiles-ServiceProfileDefinition');
	$form .= Forms->form_bottom_buttons( \%add, \%cancel, \%help, $tab++ );
    }
    elsif ( $obj_view eq 'copy' ) {
	my $source = $query->param('source');
...
sub service() {
  ...
  <prop name="apply_service_externals"><![CDATA[checked]]>
	...
	if ( $obj_view eq 'service_detail' ) {
	    ...
	    $form .= Forms->hidden( \%hidden );
	    $help{url} = StorProc->doc_section_url('Managing+Services', 'ManagingServices-Step2ServiceDetail');
    ...
    "Note: no parent template (recursively) has a check command defined.<br><b><font color=#FF0000>ERROR:  You have a cyclical chain of parents in your service templates, starting with \"$t{'name'}\".</font></b>"
	    ...
	    my $usage = $command;
	    my @args  = ();
	    @args = split( /ARG/i, $cmd{'command_line'} ) if defined $cmd{'command_line'};
	    my $args = undef;
	    if ( $args[1] ) {
		$usage .= "!";
		my $cnt = 1;
	    ...
	    $help{url} = StorProc->doc_section_url('Setting+Up+Dependencies', 'SettingUpDependencies-SelectaDependencythroughManageService');
	    ...
	    $hidden{'obj_view'} = 'service_profiles';
	    $form .= Forms->hidden( \%hidden );
	    $help{url} = StorProc->doc_section_url('Creating+Profiles', 'CreatingProfiles-AboutProfiles');
	    $form .= Forms->form_bottom_buttons( \%save, \%help, $tab++ );
	}
	    ...
	    $form .= Forms->hidden( \%hidden );
	    $form .= Forms->wizard_doc( 'Caution', $docs{'caution'}, 1, 1 );
	    $help{url} = StorProc->doc_section_url('Managing+Services', 'ManagingServices-Step4ManagingServicesApplyHosts');
	    $form .= Forms->form_bottom_buttons( \%apply, \%help, $tab++ );
	}
	elsif ( $obj_view eq 'delete' ) {
	    ...
	    $form .= Forms->hidden( \%hidden );
	    $help{url} = StorProc->doc_section_url('Managing+Services', 'ManagingServices-CreatingaNewServiceandManagingExisting');
	    $form .= Forms->form_bottom_buttons( \%add, \%cancel, \%help, $tab++ );
	}
...
sub profile_importer() {
	...
	$help{url} = StorProc->doc_section_url('Importing+Profiles');
	if ( $task eq 'new' ) {
...
sub manage_host() {
	    ...
	    && $range_from <= $range_to )
	{
		...
		unless ( $instances{"_$i"} ) {
		    my $inst   = "_$i";
		    my @vals   = ( \undef, $service_id, $inst, '1', $args );
	...
	if ( $obj_view eq 'host_detail' ) {
	    ...
	    my @parents = $query->param('members');
	    foreach my $parent (@parents) {
		if ( $parent eq $name ) {
		    push @errors, "A host cannot be its own parent.";
		    last;
		...
		if ( $qname =~ /args_(\d+)/ ) {
		    ...
		    ($instances{$1}{'args'} = $query->param($qname)) =~ s/[\n\r]+//g;
		}
...
<data>
	...
	my %save = ( 'name' => 'save', 'value' => 'Save' );
	$form .= Forms->hidden( \%hidden );
	$help{url} = StorProc->doc_section_url('Setting+Up+Notifications', 'SettingUpNotifications-ApplyingEscalationTrees');
	$form .= Forms->form_bottom_buttons( \%save, \%help, $tab++ );
    }
    elsif ( $obj_view eq 'hostgroups' ) {
...
"Note: no parent template (recursively) has a check command defined.<br><b><font color=#FF0000>ERROR:  You have a cyclical chain of parents in your service templates, starting with \"$t{'name'}\".</font></b>"
	...
	$form .= Forms->hidden( \%hidden );
	my %add_dependency = ( 'name' => 'add_dependency', 'value' => 'Add Dependency' );
	$help{url} = StorProc->doc_section_url('Setting+Up+Dependencies', 'SettingUpDependencies-SelectaDependencythroughManageHostService');
	$form .= Forms->form_bottom_buttons( \%add_dependency, \%help, $tab++ );
    }
    elsif ( $obj_view eq 'service_externals' ) {
	...
	$form .= Forms->host_top( $properties{'name'}, $session_id, $obj_view, $auth_add{'externals'},
	  $hidden{'selected'}, $hidden{'form_service'} );
	if (@errors) { $form .= Forms->form_errors( \@errors ) }
	$form .= Forms->text_box( 'Rename to:', 'new_name', '', $textsize{'name'}, $required, '', '', $tab++ );
	$hidden{'name'}   = $name;
	$hidden{'rename'} = $name;
	$form .= Forms->hidden( \%hidden );
...
sub custom_variables($$$$$) {
	...
	if ( $param =~ /^(_\p{IsWord}+)$/ ) {
	    ...
	    # FIX MAJOR:  ensure that non-ISO-8859-1 characters are stripped or cause errors to be displayed
	    $value_variable{$1} = StorProc->sanitize_string( $query->param($param) );
	}
    }
...
sub exported_files($$) {
	...
	if (@errors) {
		...
		$help{url} = StorProc->doc_section_url('Importing+and+Exporting', 'ImportingandExporting-ExporttoFiles');
		$page .= Forms->hidden( \%hidden );
...
sub tools() {
	...
	# tested for once the user decides to move forward.
	    ...
	    my $doc_url = StorProc->doc_section_url('About+System+Maintenance');
	    my $message =
...
The Bookshelf contains instructions for that (see
<a href="$doc_url" target="_blank">SYSTEM MAINTENANCE</a>).
<p>
<hr class=row2>
</p>
<p class=append>
You are about to update or drop all configuration records in the GroundWork "monarch" database.
Should you choose to continue, it is strongly recommended that you first select the backup option.
</p>
<p class=append>
Are you sure you want to continue?
	    ...
	    $help{url} = StorProc->doc_section_url('Importing+and+Exporting', 'ImportingandExporting-ImportfromFiles');
	    $page .= Forms->form_bottom_buttons( \%backup, \%import, \%abort, \%help, $tab++ );
	}
	else {
	    $page .= Forms->header( $page_title, $session_id, $top_menu );
...
"Check the new Monarch group name (name = \"$new_name\"). Group names cannot contain illegal characters (anything other than a-z, A-Z, 0-9, hyphen, and dot), or begin or end with a hyphen or dot.";
	...
	my $location    = StorProc->sanitize_string( $query->param('location') );
	my $nagios_etc  = StorProc->sanitize_string( $query->param('nagios_etc') );
...
<data>
 ...
 <prop name="service_passive_checks_enabled"><![CDATA[$group{'service_passive_checks_enabled'}]]>
	...
	$form .= Forms->hidden( \%hidden );
	$help{url} = StorProc->doc_section_url('Configuring+Groups', 'ConfiguringGroups-ManageGroupDetail');
	$form .= Forms->form_bottom_buttons( \%save, \%delete, \%rename, \%close, \%help, $tab++ );
    }
	...
	my $help_url = StorProc->doc_section_url('Configuring+Groups', 'ConfiguringGroups-ManageGroupHostAssignment');
	...
	my $help_url = StorProc->doc_section_url('Configuring+Groups', 'ConfiguringGroups-ManageGroupsSubGroups');
	...
	$form .= Forms->hidden( \%hidden );
	$help{url} = StorProc->doc_section_url('Configuring+Groups', 'ConfiguringGroups-DefiningGroups');
	$form .= Forms->form_bottom_buttons( \%add, \%cancel, \%help, $tab++ );
    }
...
sub macros() {
	...
	$form .= Forms->hidden( \%hidden );
	$help{url} = StorProc->doc_section_url('Configuring+Groups', 'ConfiguringGroups-ManageGroupsMacros');
...
sub nagios_cgi($$) {
	...
	if ( $nagios_set{'name'} ) {
		...
		if ($gid) {
		    my @values = ( \undef, $gid, $name, 'nagios_cgi', $nagios_defaults{$name} );
		    my $result = StorProc->insert_obj( 'monarch_group_props', \@values );
		    if ( $result =~ /^Error/ ) { push @errors, $result }
		}
		else {
	...
	foreach my $name ( keys %nagios_defaults ) {
	    if ($gid) {
		my @vals = ( \undef, $gid, $name, 'nagios_cgi', $nagios_defaults{$name} );
		my $result = StorProc->insert_obj( 'monarch_group_props', \@vals );
		if ( $result =~ /^Error/ ) { push @errors, $result }
	    }
	    else {
	...
	if ($gid) {
	    ...
	    %upload = ( 'name' => 'load_nagios_cgi', 'value' => 'Load From File' );
	...
	$page .= Forms->text_area(
	    ...
	    '3', '75', '', $docs{'authorized_for_configuration_information'},
	    '', $tab++, 1
	);
	$page .= Forms->text_area(
	    ...
	    '3', '75', '', $docs{'authorized_for_system_information'},
	    '', $tab++, 1
	);
	$page .= Forms->text_area(
	...
	$page .= Forms->text_area(
	    ...
	    '3', '75', '', $docs{'authorized_for_all_service_commands'},
	    '', $tab++, 1
	);
	...
	if ( $nagios_set{'name'} ) {
		...
		unless ( $name =~ /^next$|user_acct|^misc_name$|^misc_value$|^nocache$|^obj$|^step$|^top_menu$|^update_main$|^view$/ ) {
			...
			if ( defined $checks{$name} ) {
			    ...
			    ## FIX MINOR:  Is there any reason not to trim all values, not just a few special ones?
			    $val =~ s/^\s+|\s+$//g if $name =~ /^(use_timezone|temp_path)$/;
			    if ($gid) {
				if ( $name =~ /^\d+$/ ) {
				    my %values = ( 'value'   => $val );
...
sub control() {
	...
	#
		...
		my @setup_props = (
		    ...
		    'enable_externals', 'enable_groups',        'enable_ez'
		);
		...
		my $errors = StorProc->check_version( $p{'value'} );
		push @errors, @$errors if @$errors;
		if (@errors) { $page .= Forms->form_errors( \@errors ) }
		$page .= Forms->display_hidden( 'Monarch version:', 'monarch_version', $p{'value'}, $docs{'monarch_version'} );
		if ($is_portal) {
		    %p = StorProc->fetch_one( 'setup', 'name', 'nagios_version' );
		    $page .= Forms->display_hidden( 'Nagios version:', 'nagios_version', $p{'value'}, $docs{'nagios_version'} );
		...
		$page .= Forms->hidden( \%hidden );
		$help{url} = StorProc->doc_section_url('Configuring+Externals', 'ConfiguringExternals-ConfiguringExternals');
		$page .= Forms->form_bottom_buttons( \%save, \%cancel, \%help, $tab++ );
	    }
	}
	...
	#
		...
		if ( $query->param('refreshed') ) {
		    my ($errors, $results, $timings) =
		      StorProc->synchronized_commit( $user_acct, $nagios_ver, $nagios_etc, $nagios_bin, $monarch_home, 'html' );
		    $page .= Forms->header( $page_title, $session_id, $top_menu );
		    if (@$errors) {
		...
		$help{url} = StorProc->doc_section_url('Pre+flight%2C+Backup%2C+Commit');
		$page .= Forms->form_bottom_buttons( \%abort, \%backup, \%commit, \%help, $tab++ );
	    }
	    else {
		$page .= Forms->header( $page_title, $session_id, $top_menu );
		$page .= Forms->form_top( 'Nagios Commit', '' );
