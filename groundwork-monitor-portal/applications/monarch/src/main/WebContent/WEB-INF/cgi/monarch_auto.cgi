...
sub discover_home() {
    ...
    my $traceroute_max_hops = StorProc->sanitize_string( $query->param('traceroute_max_hops') );
    my $traceroute_timeout  = StorProc->sanitize_string( $query->param('traceroute_timeout') );
...
sub commit() {
    ...
    my $message =
'The commit process is reported in three stages: 1. Pre-flight, 2. Backup, and 3. Commit and Foundation sync. Please review the results carefully before closing. A pre-flight or backup failure will abort the process.';
...
sub show_data() {
    my $data_source = StorProc->sanitize_string( $query->param('data_source') );
...
sub edit_schema() {
    ...
    my $description     = StorProc->sanitize_string_but_keep_newlines( $query->param('description') );
    my $type            = $query->param('type');
    ...
    my $other_delimiter = StorProc->sanitize_string( $query->param('other_delimiter') );
...
"There are no records to process. Did the discovery find anything new? Check the $automation_name schema to see if there is a rule to discard records that match existing hosts.";
		...
		unlink "$automation_data_dir/$import_file" || push @errors, "error: cannot unlink $automation_data_dir/$import_file ($!)";
	    }
	    if (defined $processed_file) {
		unlink "$automation_data_dir/$processed_file" || push @errors, "error: cannot unlink $automation_data_dir/$processed_file ($!)";
	    }
	    foreach my $err (@errors) {
		$errstr .= "<br>&bull;&nbsp;$err";
	    }
...
sub discovery_delete_group_if_confirmed {
	...
	unless (@$errors) {
	    ...
	    delete $hidden->{'obj_view'};
	    $$discover_name = undef;
	    my @method_ids = $query->param('method_id');
...
sub discovery_add_method {
    ...
    my $new_method             = StorProc->sanitize_string( $query->param('new_method') );
    my $new_type               = $query->param('new_type');
    my $new_method_description = StorProc->sanitize_string( $query->param('new_method_description') );
...
sub discovery_add_filter {
    ...
    my $filter_name  = StorProc->sanitize_string( $query->param('filter_name') );
    my $filter_type  = $query->param('filter_type');
    my $filter_value = StorProc->sanitize_string( $query->param('filter_value') );
...
<data>
 <prop name="wmi_type"><![CDATA[$wmi_type]]>
	...
	my $command_line = StorProc->sanitize_string( $query->param('command_line') );
...
<data>
 ...
 <prop name="command_line"><![CDATA[$command_line]]>
	...
	if ( $result =~ /error/i ) {
		...
		my $add_filter = StorProc->sanitize_string( $query->param('filter_name') );
