...
my $josso_config = "/usr/local/groundwork/config/resources/josso-gateway-db-stores.xml";
my $jpp_config = "/usr/local/groundwork/foundation/container/jpp/standalone/configuration/standalone.xml";

# old jbossportal default DB config
my $old_user = "postgres";
my $old_pass = "groundwork";
my $old_host = "localhost";
my $old_port = 5432;
my $old_dbname = "jbossportal";

# new jboss-idm default DB config
my $new_user = "postgres";
my $new_pass = "groundwork";
my $new_host = "localhost";
my $new_port = 5432;
my $new_dbname = "jboss-idm";

# load connection options
if( -e $josso_config ){
	my $ref = XMLin($josso_config);
	
	if( exists $ref->{'db-istore:jdbc-store'}){
		
		$old_user = $ref->{'db-istore:jdbc-store'}->{'connectionName'} if exists $ref->{'db-istore:jdbc-store'}->{'connectionName'};
		$old_pass = $ref->{'db-istore:jdbc-store'}->{'connectionPassword'} if exists $ref->{'db-istore:jdbc-store'}->{'connectionPassword'};
		
		if( $ref->{'db-istore:jdbc-store'}->{'connectionURL'} =~ /jdbc:postgresql:\/\/([\w\.\-\_]+):(\d+)\/([\w\.\-\_]+)/ ){
			$old_host = $1 if $1 ne "";
			$old_port = $2 if $2 ne "";
			$old_dbname = $3 if $3 ne "";
		}
	}
}
if( -e $jpp_config ){
		...
		if( $subsystem->{'xmlns'} eq "urn:jboss:domain:datasources:1.1" ){
			foreach my $datasource ( @{$subsystem->{'datasources'}->{'datasource'}} ){
				if( $datasource->{'pool-name'} eq "IDMPortalDS" ){
					...
					$new_user = $datasource->{'security'}->{'user-name'} if exists $datasource->{'security'}->{'user-name'};
					$new_pass = $datasource->{'security'}->{'password'} if exists $datasource->{'security'}->{'password'};
...
my $old_dbh = DBI->connect("dbi:Pg:dbname=$old_dbname;host=$old_host;port=$old_port;", $old_user, $old_pass, { AutoCommit => 1 }) or die $DBI::errstr;
...
my $new_dbh = DBI->connect("dbi:Pg:dbname=$new_dbname;host=$new_host;port=$new_port;", $new_user, $new_pass, { PrintError => 0, AutoCommit => 1 } ) or die $DBI::errstr;
...
my $perms_sth = $old_dbh->prepare("SELECT nv_mod, nv_act, nv_obj FROM jbp_nagvis_perms WHERE nv_mod != 'AutoMap' ORDER BY nv_mod, nv_obj;");
...
# add new permissions
@perm = ('User','setOption','*');
if( $rows > 0 && $new_perm_sth->execute(@perm) > 0 && $debug ){
	printf "Added perm MOD: %-10s ACT: %-10s OBJ: %s\n", @perm;
}
