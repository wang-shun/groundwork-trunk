...
public class NetworkObjectTree {
	...
	private List<CustomGroup> rootHGCustomGroups = new ArrayList<CustomGroup>();

	private List<CustomGroup> rootSGCustomGroups = new ArrayList<CustomGroup>();
	...
	public NetworkObjectTree(NodeType nodeType,
		...
		NetworkMetaEntity rootEntity = new NetworkMetaEntity(0, rootNodeText,
				NetworkObjectStatusEnum.ENTIRE_NETWORK_STATUS,
				NodeType.NETWORK, null, null, null);
		List<CustomGroup> customGroups = referenceTreeModel
				.getRootCustomGroups();
		for (CustomGroup cusGroup : customGroups) {
			// Load only groups that are in published status
			if (cusGroup.getGroupState().equalsIgnoreCase("P")) {
				String concreteType = referenceTreeModel
						.checkConcreteEntityType(cusGroup);
				if (treeType == NodeType.HOST_GROUP) {
					if (concreteType.equalsIgnoreCase(Constant.DB_HOST_GROUP)) {
						rootHGCustomGroups.add(cusGroup);
					} // end if
				}
				...
				if (treeType == NodeType.SERVICE_GROUP) {
					if (concreteType
						...
						rootSGCustomGroups.add(cusGroup);
					}
				}
				NetworkCustomGroup rootObject = new NetworkCustomGroup(
		...
		if (treeType == NodeType.HOST_GROUP) {
			...
			synchronized (rootHGCustomGroups) {
				for (int i = 0; i < rootHGCustomGroups.size(); i++) {
					CustomGroup group = rootHGCustomGroups.get(i);
					NetworkMetaEntity custGroup = referenceTreeModel
							.getCustomGroupById((int) group.getGroupId());
					if (custGroup != null) {
			...
			synchronized (rootSGCustomGroups) {
				for (int i = 0; i < rootSGCustomGroups.size(); i++) {
					CustomGroup group = rootSGCustomGroups.get(i);
					NetworkMetaEntity custGroup = referenceTreeModel
							.getCustomGroupById((int) group.getGroupId());
					if (custGroup != null) {
						DefaultMutableTreeNode node = this
