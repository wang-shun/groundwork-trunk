...
public class NetworkTreeBean extends UIEventsServerPush {
	...
	private void createHostGroupTreeModel() {
		try {
			// build Host tree
            if (!referenceTreeModel.isInitialized()) {
                referenceTreeModel.rebuildModel();
            }
			hostTree = new NetworkObjectTree(NodeType.HOST_GROUP,
					referenceTreeModel);
			...
			LOGGER.info("Empty Host tree found. Enabling retry button on UI in createTreeModels()");
			hostTreeOk = false;
		}
	}
				...
				if (child != null) {
					if (child.getUserObject() instanceof NetworkCustomGroup) {
						NetworkCustomGroup custGroup = (NetworkCustomGroup) child
								.getUserObject();
						// This is the right place to bubble up the custom group
						// as MSP is already applied here
	...
	private void createServiceGroupTreeModel() {
		try {
			...
			// error!! enable "retry" button on UI
			LOGGER.info("Empty Service tree found. Enabling \"retry\" button on UI in createTreeModels()");
			serviceTreeOk = false;
		}
	}
