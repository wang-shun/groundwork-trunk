...
public class SeuratHandler extends ServerPush {
     ...
     */
    private boolean populateList() throws PreferencesException,
            GWPortalException, WSDataUnavailableException {
                ...
                if (host != null) {
                    ...
                    if (status == SeuratStatusEnum.SEURAT_HOST_UP) {
                        ...
                        if (status != SeuratStatusEnum.SEURAT_HOST_RECENTLY_RECOVERED) {
                            ...
                            // services
                            double serviceAvailability = host
                                    .getServiceAvailablityForHost();
                            ...
                            if (serviceAvailability > 0) {
                                ...
                                for (SeuratStatusEnum availabilityStatus : serviceAvailabilityStates) {
                                    if (serviceAvailability > availabilityStatus
                                            .getServiceLowerBound()
                                            && serviceAvailability <= availabilityStatus
                    ...
                    SeuratBean seuratBean = new SeuratBean(host.getObjectId(),
                            ...
                            host.isAcknowledged(), host.getChildNodeList(),
                            referenceTreeModel);
    ...
    public void showHostDetailsPopup(ActionEvent event) {
        ...
        try {
            if (hostNameParam != null) {
                SimpleHost simpleHostByName = foundationWSFacade
                        .getSimpleHostByName(hostNameParam, true);
                if (simpleHostByName != null) {
                    int hostId = simpleHostByName.getHostID();
                    SeuratStatusEnum status = getSeuratStatusFromMonitorStatus(simpleHostByName
                            .getMonitorStatus());
                    boolean isAcknowledged = simpleHostByName.isAcknowledged();
                    if (status == SeuratStatusEnum.SEURAT_HOST_UP
                    ...
                    selectedHost = new SeuratBean(hostId, hostNameParam,
                            status, isAcknowledged, hostGroupName);
                    selectedHost.setType(NodeType.HOST);

                    // populate pop-up with data
                    populatePopup(hostId, simpleHostByName);
     ...
     * @param hostId
     *            the host id
     * @param simpleHostByName
     *            the simple host by name
     * 
     * @throws WSDataUnavailableException
     *             the WS data unavailable exception
     * @throws GWPortalException
     *             the GW portal exception
     */
    private void populatePopup(int hostId, SimpleHost simpleHostByName)
            throws GWPortalException, WSDataUnavailableException {
        // remove all elements from list
        serviceList.clear();
        ackAllServicesButtonEnabled = false;

        SimpleServiceStatus[] simpleServicesUnderHost = simpleHostByName
                .getSimpleServiceStatus();
        if (simpleServicesUnderHost != null
                && simpleServicesUnderHost.length != 0) {
            for (SimpleServiceStatus simpleService : simpleServicesUnderHost) {
                // Construct new SeuratService object and insert into list
                int serviceId = simpleService.getServiceStatusID();
                NetworkMetaEntity serviceEntity = referenceTreeModel
                        .getServiceById(serviceId);

                if (serviceEntity != null) {
                    // update service entity with latest acknowledgment status
                    serviceEntity.setAcknowledged(simpleService
                            .isAcknowledged());
                    ...
                    serviceEntity.setParentListString(hostGroupName
                            + Constant.COMMA + simpleHostByName.getName());
