...
int main(int argc, char **argv){
	...
	if(result!=OK || show_help==TRUE){
		printf("Usage: %s -H <host_address> [-p port] [-to to_sec] [-d delim] [-c config_file] [-od]\n",argv[0]);
		printf("\n");
		printf("Options:\n");
		...
		printf("The input will the have to be specified as:\n");
		printf("\n");
		printf("Service Checks:\n");
	...
	while(fgets(input_buffer,sizeof(input_buffer)-1,stdin)){
		if(custom_timestamp==TRUE){
			...
			packet_timestamp=(time_t)atol(ptr1);
			strncpy(host_name,ptr2,sizeof(host_name)-1);
			...
			if(ptr5==NULL){
				...
				strncpy(svc_description,ptr3,sizeof(svc_description)-1);
				return_code=atoi(ptr4);
				strncpy(plugin_output,ptr5,sizeof(plugin_output)-1);
				}
			}
		else{
			if(feof(stdin))
