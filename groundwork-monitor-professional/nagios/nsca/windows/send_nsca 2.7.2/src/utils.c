...
int encrypt_init(char *password,int encryption_method,char *received_iv,struct crypt_instance **CIptr){
        ...
        *CIptr=CI;
        ...
        if(CI==NULL){
                printf("Could not allocate memory for crypt instance\n");
                return ERROR;
                }
	...
	/* client recieves IV from server */
        ...
        if((CI->key=(char *)malloc(CI->keysize))==NULL){
                printf("Could not allocate memory for encryption/decryption key\n");
                return ERROR;
                }
...
#ifdef DEBUG
        printf("Encrypting with algorithm #%d\n",encryption_method);
#endif
...
#ifdef DEBUG
        printf("Decrypting with algorithm #%d\n",encryption_method);
#endif
