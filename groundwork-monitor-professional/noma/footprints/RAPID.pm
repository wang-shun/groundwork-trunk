...
#   (Although this is probabl not necessary given that the careful use of Coro
...
use JSON;                               # For encoding messages to the API, and decoding responses
...
# FIX MAJOR:  Convert to using TLSv12 or TLSv2 ?
# FIX MAJOR:  We probably want "SSL_version => 'TLSv12'" here, or something like it (at least 'TLSv11'),
# within %standard_ssl_opts, instead of the current 'TLSv1' setting (which implies the TLS1.0 protocol).
...
# A critical issue might be that some browsers such as Firefox don't yet support TLS1.2 (this needs to
# be verified), so we won't be able to protect the entire GroundWork site at that level yet.  We need to
# see if we can protect just the URLs that a GW::RAPID client will access, using various Apache facilities.
# Also, we must be running a version of OpenSSL on the server, underneath Apache, that supports TLS1.2.
#
# FIX MAJOR:  Review the cipher list; maybe strengthen it (RC4 is now considered to be insecure).
# Consider whether we should upgrade our copy of the IO::Socket::SSL package, and see what it now
# uses as default ciphers.
#
...
# FIX MAJOR:  Support some combination of "cert", "key", and "ca" options to provide proper SSL configurability.
sub new {
    my $invocant        = $_[0];
    my $protocol        = $_[1];
    my $hostname        = $_[2];
	...
	if ( defined $options ) {
		...
		if ( $valid_options{$key} eq 'integer' ) {
			...
			$logger->logcroak("ERROR:  Invalid REST API option '$key':  \"$options->{$key}\" is not a valid readable file.");
		    }
		}
...
sub get_hosts {
	...
	$self->{logger}->logcroak("ERROR:  Undefined arg(s). $usage_get_hosts")
	  if any { !defined $_ } $self, $hostnames, $options, $outcome, $results;
...
sub get_categories {
	...
	$self->{logger}->logcroak("ERROR:  Undefined arg(s). $usage_get_categories") if any { !defined $_ } $self, $categorynames, $options, $outcome, $results;
	...
	$status = $self->_API_GET( "categories", $categorynames, $options, $outcome, $results );
    };
    set_outcome( $@, $outcome );
...
sub upsert_hosts {
	...
	$self->{logger}->logcroak("ERROR:  Undefined arg(s). $usage_upsert_hosts")
	  if any { !defined $_ } $self, $hosts, $options, $outcome, $results;
...
# NOTE:  At the time of this writing, the only part of the API that uses PUT is updating events.
...
sub update_events {
	...
	$self->{logger}->logcroak("ERROR:  Undefined arg(s). $usage_update_events")
	  if any { !defined $_ } $self, $eventids, $options, $outcome, $results;
...
sub delete_hosts {
	...
	$self->{logger}->logcroak("ERROR:  Undefined arg(s). $usage_delete_hosts")
	  if any { !defined $_ } $self, $hostnames, $options, $outcome, $results;
...
sub _API_AUTH {
	...
	if ( $self->{logger} && ( $is_stats || $self->{logger}->is_trace() ) ) {
	    ...
	    ## my $json_readable_instructions = to_json( { $api_post_objects{$api_method} => $objects }, { utf8 => 1, pretty => 1 } );
	    ## $self->{logger}->$level("...$full_post_url POST JSON:\n$json_readable_instructions");
	}
	## $json_encoded_instructions = encode_json( { $api_post_objects{$api_method} => $objects } );
...
sub _API_GET {
	...
	# With the case of a GET, the only response code that indicates the host was found is a 200,
	# otherwise process the code and produce info on that
	if ( $http_response_code == 200 ) {
	    my $status = 1;
	    $self->{logger}->trace("$api_method GET JSON response:\n$response_content");
	    $ref_decoded_response = decode_json($response_content);    # decode the JSON object back into a Perl structure
	    ...
	    #
   	    ...
   	    # If that approach does fail, the logic here will drop through to being a more generalized error but still handled tho.

	    if ( $self->response_content_matched( $api_method, $response_content ) or $http_response_code == 404 ) {
		    $self->{logger}->warn("WARNING:  $api_method object / query \"$thing_to_get\" returned 'not found'");    # simply wasn't found
	    }
	    else {
...
sub _API_POST {
	...
	my $level = $is_stats ? 'stats' : 'trace';
	my $json_readable_instructions = to_json( $api_post_objects{$api_method} ? { $api_post_objects{$api_method} => $objects } : $objects,
	    { utf8 => 1, convert_blessed => 1, pretty => 1 } );
	$self->{logger}->$level("...$full_post_url POST JSON:\n$json_readable_instructions");
    }

    $json_encoded_instructions =
      to_json( $api_post_objects{$api_method} ? { $api_post_objects{$api_method} => $objects } : $objects,
	...
	if ( $http_response_code == 200 ) {
	    ...
	    $ref_decoded_response = decode_json($response_content);    # decode JSON back into Perl data structure
...
sub _API_DELETE {
    ...
    # Validate arguments.
    $self->{logger}->logcroak("ERROR:  unrecognized API command root '$api_method'")
      if not defined $api_delete_methods{$api_method};
	...
	if ( $is_stats || $self->{logger}->is_trace() ) {
	    my $level = $is_stats ? 'stats' : 'trace';
	    my $json_readable_instructions = to_json( { 'services' => \@services }, { utf8 => 1, convert_blessed => 1, pretty => 1 } );

	    # If you want to display what came into this routine as opposed to what we did with it, use this instead:
	    # my $json_readable_instructions = to_json( { 'services' => $objects }, { utf8 => 1, convert_blessed => 1, pretty => 1 } );
	...
	$json_encoded_instructions = to_json( { 'services' => \@services }, { utf8 => 1, convert_blessed => 1 } );
    }
    ...
    elsif ( $api_method eq 'servicegroups' || $api_method eq 'hosts' || $api_method eq 'hostgroups' ) {
	## FIX MINOR:  ensure that @$objects != 0, if that would cause a problem (test that case)
	my @object_ids = map {
	    { $api_delete_ids{$api_method} => $_ }
	} @$objects;
	$full_delete_url = "/$api_method";
	...
	if ( $is_stats || $self->{logger}->is_trace() ) {
	    ...
	    my $json_readable_instructions =
	      to_json( { $api_delete_objects{$api_method} => \@object_ids }, { utf8 => 1, convert_blessed => 1, pretty => 1 } );

	    # If you want to display what came into this routine as opposed to what we did with it, use this instead:
	    # my $json_readable_instructions =
	    # to_json( { $api_delete_objects{$api_method} => $objects }, { utf8 => 1, convert_blessed => 1, pretty => 1 } );

	    # FIX MINOR:  $full_delete_url here doesn't contain "?clear=true" if that is appended below.
	    $self->{logger}->$level("...$full_delete_url DELETE JSON:\n$json_readable_instructions");
	}

	$json_encoded_instructions = to_json( { $api_delete_objects{$api_method} => \@object_ids }, { utf8 => 1, convert_blessed => 1 } );
    }
    else {
    ...
    if ( ( caller(2) )[3] eq "GW::RAPID::clear_hostgroups" ) {
...
sub _API_PUT {
    ...
    $self->{logger}->logcroak("ERROR:  Undefined arg(s). $usage__API_PUT")
      if any { !defined $_ } $self, $api_method, $objects, $options, $outcome, $results;
    ...
    my %api_put_methods = ( "events" => 1 );

    # Validate arguments.
    $self->{logger}->logcroak("ERROR:  unrecognized API command root '$api_method'")
      if not defined $api_put_methods{$api_method};
...
Documentation regarding the REST API can be found here: <link coming soon>
...
Required constructor arguments are:
	...
	timeout         => integer (seconds; seems to be ignored in spite of its intended action)
...
=over
	...
	* We use the %outcome and %results hashes in a manner which is
	...
	if ($status) {
	    foreach my $version ( keys %results ) {
		if ( $version eq '7.0.2' ) {
		    ## We are operating against version 7.0.2 of the REST API.
...
Copyright (c) 2013-2015 GroundWork Open Source, Inc.;
