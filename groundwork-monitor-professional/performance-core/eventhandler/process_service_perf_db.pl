...
my $debug = 0; # set to 1 = on and 0 = off
...
if ($perfidstring) {
	if ($perfdata =~ /PLUGINID=(\S+)/i) {
		if ($perfidstring ne $1) {
			print FP "Plugin ID $1 doesn't match expected plugin ID string $perfidstring \n" if $debug;
			close FP if $debug;
			exit;		# No perf handling for this host service
		}
		print FP "Plugin ID $1 matches plugin ID string $perfidstring \n" if $debug;

	} else {
		print FP "No PLUGINID string $perfidstring in performance string $perfdata \n" if $debug;
		close FP if $debug;
		exit;		# No perf handling for this host service
	}
}
...
sub replace_macros {
	...
	$string =~ s/\$RRDNAME\$/$$macros_ref{'\$RRDNAME\$'}/;		# RRDNAME may contain other macros, ie $HOST$, $SERVICE$, so substitute this first
	foreach my $macro (keys %$macros_ref) {
		$string =~ s/$macro/$$macros_ref{$macro}/;
	}
	$i=1;
