...
# Modified Aug 29 2017 to fix output to condform to RRDtool DS limit and 
# to fix trigger logic for warning and critical thresholds. 
# - Thomas Stocking, Laura Horsky, GroundWork Open Source, Inc. 
...
make perfdata fit RRD ds formats
...
my (
  ...
  $opt_tmpdir,   $opt_url, $opt_screenshots, $opt_rrd,  %opt_casperopts,  
...
# collect data from xmlFile (modified by GroundWork Open Source Inc. Aug 2017 to support shorter perf data lables)

if ($$ref{'testsuite'}{'tests'} == 1) {
  if (defined $$ref{'testsuite'}{'testcase'}{'time'}) {
    if ($opt_rrd) {
       $perfdata .= "TransactionTime=$$ref{'testsuite'}{'testcase'}{'time'}s;$opt_warning;$opt_critical;0;$opt_critical ";
    } else {
  ...
  foreach my $caseRef( @{$$ref{'testsuite'}{'testcase'}} ) {
    if (defined $$caseRef{'time'}) {
      if ($opt_rrd) {
         my $dsname = "Step".$stepcount;
         $perfdata .= "$dsname=$$caseRef{'time'}s;$opt_warning;$opt_critical;0;$opt_critical ";
         $stepcount++;
      } else {
         $$caseRef{'name'}=~s/'//g;
...
# Calculate output state (modified by GroundWork Open Source Inc. Aug 2017 to fix errant logic)
if ( $$ref{'time'} > $opt_warning && $$ref{'time'} < $opt_critical && $out_state != 2 ) {
  $out_state = 1;
