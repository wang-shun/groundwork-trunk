# This is the groundwork-alertsite-integration spec file needed to construct
# the Groundwork Monitor AlertSite Integration RPM.
...
%define	name		groundwork-alertsite-integration
...
%define	alertsitepath	%{gwpath}/alertsite
...
Summary: GroundWork Monitor AlertSite Integration Software
...
# We require no RPM pre-requisites for the groundwork-alertsite-integration RPM
...
supplying software to integrate with AlertSite web monitoring.
...
# make alertsite_integration_install
...
if [ `env | egrep -c '^MYSQL_PASS=' || true` -ne 1 ]; then
    ...
    echo "        See the ALERTSITE_INTEGRATION_INSTALL_NOTES file."
    exit 1
fi
...
if [ "$1" = 1 ]; then
    ...
    # echo "       no attempt will be made to kill a previous AlertSite Integration."
...
if [ `expr match "$1" '[0-9]*'` -gt 0 ] && [ "$1" -gt 1 ]; then
    ...
    # We shut down the AlertSite Integration daemons if they are running, so the upgrade
    # can go smoothly.
    ...
    %{alertsitepath}/bin/control_alertsite_integration stop
...
if [ "$1" = 1 -o "$1" = "configure" -o "$1" = "abort-remove" ]; then
    ...
    # Shut down any old daemons associated with the AlertSite Integration software,
...
# While developing this specfile, we wanted to install the exec_rrdgraph.pl script
# directly in place, overwriting the existing file from the GWMEE base product.
# That runs a danger of undesirable deletion of the file under some conditions
# (e.g., when this package is removed).  We tried a possible workaround using a fake
# %%config(noreplace) declaration, but that makes the problem more complex without
# fully solving the issue.  So to avoid unexpected surprises that might occur under
# conditions we forgot to test, we are instead using RPM to install the file under
# our own extension-specific directory and then manually copying it into place.

# Regardless of whether this is a fresh install or an upgrade, back up the current
# copy of exec_rrdgraph.pl, which we will replace with our own copy.

timestamp=`date +"%%Y-%%m-%%d.%%H_%%M_%%S"`
old_file=%{gwpath}/common/bin/exec_rrdgraph.pl
sav_file=%{gwpath}/common/bin/exec_rrdgraph.pl.$timestamp.pre_alertsite

if [ -f $old_file ]; then
    cp -p $old_file $sav_file
    if [ $? -ne 0 ]; then
        echo "FATAL:  AlertSite Integration RPM could not back up the exec_rrdgraph.pl file." 1>&2
        exit 1
    fi
fi
...
rpm_file=%{alertsitepath}/bin/exec_rrdgraph.pl

if [ -f $rpm_file ]; then
    cp -p $rpm_file $old_file
    if [ $? -ne 0 ]; then
        echo "FATAL:  AlertSite Integration RPM could not install the exec_rrdgraph.pl file." 1>&2
        exit 1
    fi
else
    echo "FATAL:  AlertSite Integration RPM could not find the new exec_rrdgraph.pl file." 1>&2
    exit 1
fi

# Make sure the AlertSite Integration nagios cron job is in place.  We carry out this action
...
if [ $? -ne 0 ]; then
    echo "FATAL:  AlertSite Integration RPM could not fetch the old nagios crontab" 1>&2
    exit 1
fi
if [ "`echo "$old_crontab" | sed -e 's/#.*//' | fgrep -c '/control_alertsite_integration '`" -eq 0 ]; then
    ...
    new_cron_job='*/5 * * * * %{alertsitepath}/bin/control_alertsite_integration start > /dev/null 2>&1'
    echo "$old_crontab$newline$new_cron_job" | crontab -u nagios -
    if [ $? -ne 0 ]; then
	echo "FATAL:  AlertSite Integration RPM could not install its nagios cron job" 1>&2
	exit 1
    fi
else
    echo "NOTICE:  The AlertSite Integration nagios cron job looks like it was already in place."
fi

# Put the expected database objects in place, whether this is a fresh install or an
# upgrade.  (The prepare_databases_for_alertsite script is idempotent, so that's not
...
# (inside the prepare_databases_for_alertsite script, it will default to localhost),
...
# from environment variables and fail if the script doesn't get what it needs via
# that route, rather than prompting the user for them if they are not so provided.
# FIX MINOR:  Make sure the -e option here actually does what we want it to.
%{alertsitepath}/db/prepare_databases_for_alertsite -e
if [ $? -ne 0 ]; then
    # Although correct operation of the AlertSite Integration depends on the
    ...
    echo "ERROR:  AlertSite Integration RPM could not make required database changes." 1>&2
fi
...
if [ "$1" = 0 -o "$1" = "remove" ]; then
    ...
    # Make sure the AlertSite Integration nagios cron job is removed.  We carry out this action
    # in small steps to minimize the likelihood of leaving the system in a mangled state,
    # having corrupted the existing content and not having installed the new content.
	...
	echo "NOTICE:  The AlertSite Integration nagios cron job looks like it was already commented out or removed."
    else
	# new_cron_job='*/5 * * * * %{alertsitepath}/bin/control_alertsite_integration start > /dev/null 2>&1'
	new_crontab=`echo "$old_crontab" | sed -e '/\/control_alertsite_integration /d'`
	if [ $? -ne 0 ]; then
	    echo "FATAL:  AlertSite Integration RPM could not deal with its nagios cron job" 1>&2
	    exit 1
	fi
	echo "$new_crontab" | crontab -u nagios -
	if [ $? -ne 0 ]; then
	    echo "FATAL:  AlertSite Integration RPM could not remove its nagios cron job" 1>&2
	    exit 1
	fi
    fi
...
# Stop the AlertSite Integration daemons.  We do this only after the cron job has been uninstalled,
...
# Either way, all outstanding copies of the AlertSite Integration daemons
...
%dir %attr(0755,nagios,nagios) %{alertsitepath}
%dir %attr(0755,nagios,nagios) %{alertsitepath}/bin
%dir %attr(0755,nagios,nagios) %{alertsitepath}/config
%dir %attr(0755,nagios,nagios) %{alertsitepath}/db
%dir %attr(0755,nagios,nagios) %{alertsitepath}/doc
%dir %attr(0755,nagios,nagios) %{alertsitepath}/info
%dir %attr(0755,nagios,nagios) %{alertsitepath}/logs
%dir %attr(0755,nagios,nagios) %{alertsitepath}/perl

%attr(0444,nagios,nagios) %{alertsitepath}/db/create_alertsite_database.sql
%attr(0754,nagios,nagios) %{alertsitepath}/db/prepare_databases_for_alertsite

%attr(0444,nagios,nagios) %{alertsitepath}/info/build_info
...
%config(noreplace) %attr(0600,nagios,nagios) %{alertsitepath}/config/query_alertsite.conf

%dir %attr(0755,nagios,nagios) %{alertsitepath}/doc/%{name}-%{version}
%doc %attr(0444,nagios,nagios) %{alertsitepath}/doc/%{name}-%{version}/ALERTSITE_INTEGRATION_INSTALL_NOTES
# FIX THIS:  create these files and enable their inclusion here
# %doc %attr(0444,nagios,nagios) %{alertsitepath}/doc/%{name}-%{version}/ALERTSITE_INTEGRATION_RELEASE_NOTES
# %doc %attr(0444,nagios,nagios) %{alertsitepath}/doc/%{name}-%{version}/groundwork-alertsite-integration.1.0.pdf

%attr(0754,nagios,nagios) %{alertsitepath}/bin/control_alertsite_integration
%attr(0755,nagios,nagios) %{alertsitepath}/bin/exec_rrdgraph.pl
%attr(0754,nagios,nagios) %{alertsitepath}/bin/query_alertsite.pl

%attr(0444,nagios,nagios) %{gwpath}/core/profiles/host-profile-alertsite-resource.xml
%attr(0444,nagios,nagios) %{gwpath}/core/profiles/service-profile-alertsite-daemons.xml
%attr(0444,nagios,nagios) %{gwpath}/core/profiles/service-profile-alertsite-probes.xml
...
# log files within the query_alertsite.pl script.
# %attr(0644,root,root) /etc/logrotate.d/groundwork-alertsite-integration
...
# %dir %attr(0755,nagios,nagios) %{alertsitepath}/perl/bin
# %dir %attr(0755,nagios,nagios) %{alertsitepath}/perl/lib
# %dir %attr(0755,nagios,nagios) %{alertsitepath}/perl/man
# %dir %attr(0755,nagios,nagios) %{alertsitepath}/perl/share
...
%changelog
* Wed Jun 22 2011 Glenn Herteg <support@groundworkopensource.com> 1.0.0
