...
NAGIOS_VERIFY="/usr/local/groundwork/nagios/bin/nagios -v /usr/local/groundwork/nagios/etc/nagios.cfg"
...
start_nagios() {
    ...
    pids=`ps -o pid,args --no-headers -C .perl.bin | egrep $PERFDATA_PATH'$' | awk '{print $1}'`
    while [ -n "$pids" ]; do
	kill -TERM $pids
	...
	preflight_results=`$NAGIOS_VERIFY`
	if [ $? -ne 0 ]; then
...
stop_nagios() {
    ...
    # Also clean up the Nagios temp_path directory.
    rm -f /usr/local/groundwork/nagios/tmp/*
    shopt -u nullglob
