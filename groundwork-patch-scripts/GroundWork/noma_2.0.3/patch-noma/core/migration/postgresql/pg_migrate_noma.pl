...
# the siblink dump_sqlite_data_for_postgresql script for more informaion on that kind
# of NoMa migration.
...
if ( $running_status =~ /noma is already running/) {
    $wait_status = system('service groundwork stop noma');
    if ($wait_status) {
